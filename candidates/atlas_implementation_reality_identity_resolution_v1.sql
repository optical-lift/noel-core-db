...
begin
  ...
  if p_kind='organization' then
    ...
    -- v1 is deliberately limited to the Person reachability substrate that is
    -- live in production today. Organization Membership remains a compatibility
    -- carrier; when Institutional Person Record is released, a later migration
    -- may upgrade this resolver without changing the case-scoped authority law.
    select coalesce(jsonb_agg(item order by match_rank,label,canonical_id),'[]'::jsonb)
    into v_items
    from (
      select *

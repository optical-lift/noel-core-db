...
begin
  ...
  elsif k='institutional_destination' then
    if nullif(btrim(p_body->>'sourceInstitutionalAnchorProposalId'),'') is null then
      issues:=issues||'"institutional_anchor_proposal_id_required"'::jsonb;
    else
      begin perform (p_body->>'sourceInstitutionalAnchorProposalId')::uuid;
      exception when others then issues:=issues||'"institutional_anchor_proposal_id_invalid"'::jsonb; end;
    end if;
    if coalesce(p_body->>'destinationKind','') not in ('principal_ledger','organization_access','implementation_case','unresolved') then
      issues:=issues||'"institutional_destination_kind_invalid"'::jsonb;
    end if;
    if coalesce(p_body->>'destinationKind','')='unresolved' then
      if nullif(btrim(p_body->>'destinationId'),'') is not null then
        issues:=issues||'"unresolved_destination_must_not_have_id"'::jsonb;
      end if;
    else
      if nullif(btrim(p_body->>'destinationId'),'') is null then
        issues:=issues||'"institutional_destination_id_required"'::jsonb;
      else
        begin perform (p_body->>'destinationId')::uuid;
        exception when others then issues:=issues||'"institutional_destination_id_invalid"'::jsonb; end;
      end if;
    end if;
    if p_body?'destinationLabel'
       and (
         jsonb_typeof(p_body->'destinationLabel')<>'string'
         or nullif(btrim(p_body->>'destinationLabel'),'') is null
       ) then
      issues:=issues||'"institutional_destination_label_invalid"'::jsonb;
    end if;
...
begin
  ...
  for x in select value from jsonb_array_elements(p_effects) loop
    ...
    if kind not in ('claim','capacity_block','capacity_adjustment','one_off_action','fact_only','institutional_anchor','institutional_destination') then raise exception 'Unsupported effectKind.' using errcode='22023'; end if;
...
begin
  ...
  if q.effect_kind in ('one_off_action','capacity_block','capacity_adjustment','institutional_anchor','institutional_destination') and fn is null then
...
create or replace function atlas.apply_personal_institutional_destination_self_api_v1(
  p_proposal_id uuid
)
returns jsonb
language plpgsql
...
declare
  ...
  v_old atlas.personal_reality_effect_proposals%rowtype;
  v_old_decision text;
  v_old_route text;
...
begin
  ...
  if v_proposal.id is null
     or v_proposal.effect_kind<>'institutional_destination' then
    raise exception 'Institutional Destination proposal required.'
      using errcode='P0002';
  end if;
  ...
  if v_proposal.decision_state<>'confirmed'
     or v_proposal.decision_receipt_id is null
     or v_proposal.readiness_state<>'ready_for_confirmation'
     or v_proposal.route_state not in ('ready','destination_unavailable') then
    raise exception 'Confirmed Institutional Destination required before application.'
      using errcode='42501';
  end if;

  select * into v_anchor
  from atlas.personal_reality_effect_proposals p
  where p.id=(v_proposal.proposal->>'sourceInstitutionalAnchorProposalId')::uuid
    and p.owner_user_id=v_uid
    and p.principal_id=v_proposal.principal_id
    and p.capture_id=v_proposal.capture_id
  ...
  v_kind := v_proposal.proposal->>'destinationKind';
  v_label := nullif(btrim(v_proposal.proposal->>'destinationLabel'),'');
  v_id := nullif(v_proposal.proposal->>'destinationId','')::uuid;

  if v_kind='unresolved' then
    v_destination_kind := 'institutional_unresolved';
  else
    v_resolution := atlas.resolve_personal_institutional_anchor_self_api_v1(
      v_anchor.id,
      25
    );

    if v_kind='principal_ledger' then
      select exists(
        select 1
        from jsonb_array_elements(coalesce(v_resolution->'existingLedgers','[]'::jsonb)) item
        where nullif(item->>'ledgerId','')::uuid=v_id
      ) into v_found;
      v_destination_kind := 'institutional_ledger';
    elsif v_kind='organization_access' then
      select exists(
        select 1
        from jsonb_array_elements(coalesce(v_resolution->'accessibleOrganizations','[]'::jsonb)) item
        where nullif(item->>'organizationId','')::uuid=v_id
      ) into v_found;
      v_destination_kind := 'institutional_organization';
    elsif v_kind='implementation_case' then
      select exists(
        select 1
        from jsonb_array_elements(coalesce(v_resolution->'implementationCases','[]'::jsonb)) item
        where nullif(item->>'implementationCaseId','')::uuid=v_id
      ) into v_found;
      v_destination_kind := 'implementation_case';
    else
      raise exception 'Unsupported Institutional Destination kind.'
        using errcode='22023';
    end if;

    if not v_found then
      raise exception 'Selected destination is not currently visible through the governed Institutional Anchor resolver.'
        using errcode='42501';
    end if;
  end if;
  ...
  if v_proposal.supersedes_proposal_id is not null then
    ...
    if v_old.id is null then
      raise exception 'Institutional Destination correction target missing.'
        using errcode='P0002';
    end if;

    v_old_decision := v_old.decision_state;
    v_old_route := v_old.route_state;

    update atlas.personal_reality_effect_proposals
    set decision_state='revoked',
        route_state=case when route_state='applied' then 'superseded' else 'not_ready' end,
        revoked_at=now(),
        updated_at=now()
    where id=v_old.id
    ...
    insert into atlas.personal_reality_effect_events(
      proposal_id,capture_id,principal_id,actor_user_id,authority_receipt_id,
      event_kind,state_axis,from_state,to_state,detail
    ) values (
      v_old.id,v_old.capture_id,v_old.principal_id,v_uid,
      v_proposal.decision_receipt_id,
      'superseded','decision',v_old_decision,'revoked',
      jsonb_build_object('supersededByProposalId',v_proposal.id)
    );

    if v_old_route is distinct from v_old.route_state then
      insert into atlas.personal_reality_effect_events(
        proposal_id,capture_id,principal_id,actor_user_id,authority_receipt_id,
        event_kind,state_axis,from_state,to_state,detail
      ) values (
        v_old.id,v_old.capture_id,v_old.principal_id,v_uid,
        v_proposal.decision_receipt_id,
        'superseded','route',v_old_route,v_old.route_state,
        jsonb_build_object('supersededByProposalId',v_proposal.id)
      );
    end if;
  end if;
  ...
  set route_state='applied',
      ...
      destination_kind=v_destination_kind,
      destination_id=case when v_id is null then null else v_id::text end,
      metadata=metadata||jsonb_build_object(
        'destinationLabel',v_label,
        'destinationAppliedBy','apply_personal_institutional_destination_self_api_v1'
      ),
      updated_at=now()
  ...
  insert into atlas.personal_reality_effect_events(
    ...
    v_proposal.id,v_capture.id,v_capture.principal_id,v_uid,
    v_proposal.decision_receipt_id,
    'applied','route','ready','applied',
    jsonb_build_object(
      'destinationKind',v_destination_kind,
      'destinationId',v_proposal.destination_id,
      'sourceInstitutionalAnchorProposalId',v_anchor.id
    )
  );

  v_capture_state := atlas.refresh_personal_reality_capture_state_v1(v_capture.id);

  return jsonb_build_object(
    'ok',true,
    'changed',true,
    'contractVersion','personal_institutional_destination_v1',
    'proposalId',v_proposal.id,
    'sourceInstitutionalAnchorProposalId',v_anchor.id,
    'routeState',v_proposal.route_state,
    'destinationKind',v_proposal.destination_kind,
    ...
    'truthBoundary',jsonb_build_object(
      'humanDestinationChoiceDoesNotCreateInstitutionalTruth',true,
      'existingDestinationMustRemainCurrentlyVisible',true,
      'unresolvedDoesNotAuthorizeNewOrganization',true,
...
revoke all on function atlas.apply_personal_institutional_destination_self_api_v1(uuid)
  from public,anon,authenticated,service_role;
grant execute on function atlas.apply_personal_institutional_destination_self_api_v1(uuid)
  to authenticated;

create or replace function public.apply_personal_institutional_destination_self_api_v1(
  p_proposal_id uuid
)
returns jsonb
language sql
...
as $function$
  select atlas.apply_personal_institutional_destination_self_api_v1(p_proposal_id);
$function$;

revoke all on function public.apply_personal_institutional_destination_self_api_v1(uuid)
  from public,anon,service_role;
grant execute on function public.apply_personal_institutional_destination_self_api_v1(uuid)
  to authenticated;
...
insert into atlas.authenticated_rpc_registry(
  ...
  'atlas.apply_personal_institutional_destination_self_api_v1(uuid)',
  'app_endpoint','verified','active',
  true,true,false,1,1,
  jsonb_build_object(
    'source','personal_institutional_destination_v1',
    'purpose','Apply a separately human-confirmed Institutional Destination only after re-verifying the selected existing destination through the governed resolver.',
    'truthBoundary','Routing selection only. It does not create or mutate Organization, Ledger, Implementation, commercial, membership, responsibility, or task truth.',

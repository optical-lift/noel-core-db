...
-- Applies external-supply observation, qualification, composition, pricing,
-- Commercial Order -> Company Work, coverage position, pooled/break-bulk planning,
-- pooled price protection, and governed External Acquisition Commitment authority.
...
commit;
 then
            v_violations:=v_violations||jsonb_build_array(jsonb_build_object(
              'key','invalid_known_cost_component',
              'componentKey',v_cost->>'componentKey'
            ));
          elsif v_currency is not null
             and upper(btrim(v_cost->>'currency'))<>v_currency then
            v_violations:=v_violations||jsonb_build_array(jsonb_build_object(
              'key','cost_component_currency_mismatch',
              'componentKey',v_cost->>'componentKey'
            ));
          end if;
        elsif v_cost_state='unresolved' then
          if v_cost ? 'amount' and v_cost->'amount'<>'null'::jsonb then
            v_violations:=v_violations||jsonb_build_array(jsonb_build_object(
              'key','unresolved_cost_component_has_amount',
              'componentKey',v_cost->>'componentKey'
            ));
          end if;
          if v_cost_required then
          ...
          if v_cost ? 'amount' and v_cost->'amount'<>'null'::jsonb then
            v_violations:=v_violations||jsonb_build_array(jsonb_build_object(
              'key','not_applicable_cost_component_has_amount',
              'componentKey',v_cost->>'componentKey'
            ));
          end if;
        end if;
      ...
      if v_economic_state='known' and v_unresolved_required_cost_count>0 then
        v_violations:=v_violations||jsonb_build_array(jsonb_build_object(
          'key','known_economics_has_unresolved_required_component'
        ));
      elsif v_economic_state='partially_known' and v_unresolved_required_cost_count=0 then
        v_violations:=v_violations||jsonb_build_array(jsonb_build_object(
          'key','partially_known_requires_unresolved_component'
        ));
      end if;
    end if;
  end if;

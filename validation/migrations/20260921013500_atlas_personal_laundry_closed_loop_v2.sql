...
-- ============================================================================
-- Product RPC membrane postconditions.
-- ============================================================================
...
begin
  for v_signature in
    select x.signature
    from (values
      ('public.record_current_household_claim_evidence_api_v1(jsonb)'::text),
      ('public.current_household_claim_evidence_state_api_v1()'::text),
      ('public.calibrate_personal_laundry_kernel_self_api_v2(jsonb)'::text),
      ('public.personal_laundry_kernel_self_api_v2()'::text),
      ('public.apply_personal_reality_household_claim_effect_self_api_v1(uuid)'::text),
      ('public.ensure_personal_laundry_consequence_definition_self_api_v1(uuid)'::text),
      ('public.evaluate_personal_laundry_consequence_from_household_evidence_self_api_v1(uuid,jsonb)'::text),
      ('public.record_person_life_consequence_clock_characterization_self_api_v1(uuid,jsonb)'::text),
      ('public.person_life_consequence_clock_admission_self_api_v1(uuid)'::text),
      ('public.reconcile_personal_laundry_consequence_axes_self_api_v1(uuid,jsonb)'::text),
      ('public.principal_clock_api_v2(date,timestamp with time zone)'::text),
      ('public.record_personal_laundry_actual_self_api_v1(jsonb)'::text),
      ('public.resolve_personal_laundry_consequence_from_actual_self_api_v1(uuid,uuid)'::text),
      ('public.propose_personal_laundry_weekly_pattern_self_api_v1()'::text)
    ) x(signature)
  loop
    v_oid := to_regprocedure(v_signature)::oid;
    if v_oid is null then
      raise exception 'Required Laundry public RPC membrane is missing: %',v_signature;
    end if;
    ...
    if not has_function_privilege('authenticated',v_oid,'EXECUTE')
      ...
      raise exception 'Laundry public RPC privilege boundary is incorrect: %',v_signature;
    end if;

    if not exists (
      select 1 from pg_proc p where p.oid=v_oid and p.prosecdef
    ) then
      raise exception 'Laundry public RPC membrane must remain SECURITY DEFINER: %',v_signature;
    end if;

    if position('atlas.' in lower(v_def))=0 then
      raise exception 'Laundry public RPC membrane must delegate to canonical Atlas authority: %',v_signature;
    end if;
  end loop;

  v_signature := 'public.issue_personal_reality_human_decision_receipt_serv_v1(uuid,uuid,text,text,jsonb)';
  v_oid := to_regprocedure(v_signature)::oid;

  if v_oid is null then
    raise exception 'Trusted Personal Reality receipt public service membrane is missing.';
  end if;
  ...
  if has_function_privilege('authenticated',v_oid,'EXECUTE')
    ...
    raise exception 'Trusted Personal Reality receipt membrane must remain service-role-only.';
  end if;

  if not exists (
    select 1 from pg_proc p where p.oid=v_oid and p.prosecdef
  )
     or position('atlas.issue_personal_reality_human_decision_receipt_serv_v1' in lower(v_def))=0 then
    raise exception 'Trusted Personal Reality receipt membrane must delegate only to canonical Atlas receipt authority.';
  end if;

  -- None of the public membranes may own domain behavior. Their SQL bodies are
  -- deliberately one-hop delegations into the canonical Atlas functions.
  select x.signature
  into v_bad
  from (values
    ('public.record_current_household_claim_evidence_api_v1(jsonb)'::text),
  ...
  where (
    select
      (length(lower(pg_get_functiondef(to_regprocedure(x.signature)::oid)))
       - length(replace(lower(pg_get_functiondef(to_regprocedure(x.signature)::oid)),'atlas.','')))
      / length('atlas.')
  ) <> 1
  limit 1;

  if v_bad is not null then
    raise exception 'Laundry public RPC membrane must remain a one-hop Atlas delegation: %',v_bad;
  end if;
end
$$;

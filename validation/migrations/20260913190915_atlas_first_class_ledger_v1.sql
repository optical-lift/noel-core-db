...
begin
  ...
  if to_regprocedure('atlas.primary_ledger_for_organization_v1(uuid)') is null
     ...
     or to_regprocedure('atlas.principal_ledgers_self_api_v1()') is null
     or to_regprocedure('atlas.establish_governing_ledger_on_organization_insert_v1()') is null then
  ...
  -- Prove the invariant also applies to Organizations born after the migration, including
  -- legacy creators that do not yet know how to create Ledger explicitly.
  insert into atlas.organizations(
    id, stable_key, name, status, metadata, onboarding_state
  ) values (
    '66666666-6666-4666-8666-666666666666'::uuid,
    'ledger_post_migration_birth_fixture',
    'Ledger Post-Migration Birth Fixture',
    'active',
  ...
  if not exists (
    ...
    where o.id = '66666666-6666-4666-8666-666666666666'::uuid
      and l.stable_key = o.stable_key
      and l.ledger_kind = 'organization_governing'
      and l.status = 'active'
      and l.metadata->>'scope_state' = 'canonical'
      and l.metadata->>'establishment_basis' = 'organization_birth'
  ) then
    raise exception 'A newly inserted Organization did not atomically receive its governing Ledger.';
  end if;
  ...
  if not exists (
    ...
    where o.stable_key = 'ledger_validation_company'
      and l.metadata->>'scope_state' = 'canonical'
      and l.metadata->>'establishment_basis' = 'legacy_organization_scope_backfill'
  ) then
    raise exception 'Ordinary pre-existing Organization Ledger was not established as canonical backfilled scope.';
  end if;
  ...
  select pg_get_functiondef('atlas.establish_governing_ledger_on_organization_insert_v1()'::regprocedure)
  into v_def;
  if position('organization_birth' in v_def) = 0
     or position('insert into atlas.ledgers' in lower(v_def)) = 0 then
    raise exception 'Organization birth does not canonically establish Ledger.';
  end if;
  ...
  if not exists (
    ...
    where n.nspname = 'atlas'
      ...
      and t.tgname = 'organizations_establish_governing_ledger_v1'
      and not t.tgisinternal
      and t.tgenabled <> 'D'
  ) then
    raise exception 'Organization -> governing Ledger birth trigger is missing or disabled.';
  end if;
  ...
  if has_function_privilege('anon','atlas.primary_ledger_for_organization_v1(uuid)','EXECUTE')
     ...
     or has_function_privilege('authenticated','atlas.principal_has_ledger_authority_v1(uuid,uuid)','EXECUTE')
     or has_function_privilege('anon','atlas.establish_governing_ledger_on_organization_insert_v1()','EXECUTE')
     or has_function_privilege('authenticated','atlas.establish_governing_ledger_on_organization_insert_v1()','EXECUTE') then
    raise exception 'Internal Ledger authority/birth helpers became directly browser-callable.';
  end if;

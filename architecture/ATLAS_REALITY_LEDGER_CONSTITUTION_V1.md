...
## Company Work collaboration receiver uptake

Bounded Work participation is receiver-established.

A current exact responsible Person may offer `participant` or `approver` participation, but no collaboration allocation exists until the target Reality Person accepts. Acceptance creates an allocation whose establishment basis identifies the exact participation offer.

Collaboration never transfers exact responsibility. Source-system visibility remains a separate fact and may constrain a domain adapter without becoming Work authority.

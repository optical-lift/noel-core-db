-- Atlas Organization Ledger v1 — rollback proof only.
--
-- This file is intentionally NOT a production migration.
-- Run as one transaction ending in ROLLBACK against the live PostgreSQL engine.
-- The proof establishes a generic organization-scoped projection beside the
-- legacy farm Journal without changing canonical domain ownership.

begin;

-- Fail rather than collide with a future independently released contract.
do $preflight$
begin
  if to_regclass('atlas.organization_ledger_entries') is not null
    ...
    raise exception 'Organization Ledger objects already exist; review the released contract instead of running this proof.';
  end if;
end;
$preflight$;
...
-- Acceptance fixture: Atlas Reference Company proves the kernel has no farm dependency.
do $proof$
declare
  v_org uuid;
  v_user uuid;
...
begin
  select id into v_org from atlas.organizations where stable_key='atlas_reference_company' limit 1;
  if v_org is null then raise exception 'Atlas Reference Company fixture is required.'; end if;
  select id into v_user from auth.users order by created_at limit 1;
  if v_user is null then raise exception 'One auth fixture user is required for owner-read proof.'; end if;
  ...
  v_one:=atlas.project_organization_ledger_event_internal_v1(
    v_org,null,'proof:resource-lot-ready','operations','resource_ready','resource-lot:alpha:ready',
    '2026-09-05 14:00:00+00','Resource lot Alpha became ready',
    'Destination has not been established yet.','established','unresolved',
    jsonb_build_object('quantity',12),jsonb_build_object('proof','organization_ledger_v1'),
    jsonb_build_object('chain','reference_company_resource'),null
  );
  if v_one->>'state'<>'admitted' then raise exception 'First Ledger event was not admitted.'; end if;
  v_entry:=(v_one->>'entryId')::uuid;
  v_before:=(v_one->>'revision')::bigint;

  perform atlas.link_organization_ledger_subject_internal_v1(
    v_entry,'operations','resource_lot','alpha','about',jsonb_build_object('proof','v1'),'{}'::jsonb
  );

  v_replay:=atlas.project_organization_ledger_event_internal_v1(
    v_org,null,'proof:resource-lot-ready','operations','resource_ready','resource-lot:alpha:ready',
    '2026-09-05 14:00:00+00','Resource lot Alpha became ready',
    'Destination has not been established yet.','established','unresolved',
    jsonb_build_object('quantity',12),jsonb_build_object('proof','organization_ledger_v1'),
  ...
  if v_replay->>'state'<>'unchanged' then raise exception 'Exact Ledger replay must be idempotent.'; end if;

  v_revised:=atlas.project_organization_ledger_event_internal_v1(
    v_org,null,'proof:resource-lot-ready','operations','resource_ready','resource-lot:alpha:ready',
    '2026-09-05 14:00:00+00','Resource lot Alpha became ready',
    'Destination line Beta is now established.','established','designated',
    jsonb_build_object('quantity',12,'destination','line_beta'),jsonb_build_object('proof','organization_ledger_v1'),
    jsonb_build_object('chain','reference_company_resource'),null
  );
  if v_revised->>'state'<>'revised' then raise exception 'Designation correction must revise the projection.'; end if;
  v_after:=(v_revised->>'revision')::bigint;
  if v_after<=v_before then raise exception 'Projection revision did not advance.'; end if;

  v_work:=atlas.project_organization_ledger_event_internal_v1(
    v_org,null,'proof:work-allocated','work','responsibility_allocated','company-work:beta:allocated',
    '2026-09-05 14:05:00+00','Preparation work assigned','A responsible institutional carrier is established.',
    'established','designated','{}'::jsonb,jsonb_build_object('proof','organization_ledger_v1'),
    jsonb_build_object('resourceEventKey','proof:resource-lot-ready'),null
  );
  perform atlas.link_organization_ledger_subject_internal_v1(
    (v_work->>'entryId')::uuid,'work','work_item','beta','about',jsonb_build_object('proof','v1'),'{}'::jsonb
  );

  v_payment:=atlas.project_organization_ledger_event_internal_v1(
    v_org,null,'proof:payment-observed','money','payment_observed','provider:event:123',
    '2026-09-05 14:10:00+00','Payment provider reported receipt','Economic obligation match is unresolved.',
    'observed','unresolved',jsonb_build_object('amountMinor',3500),jsonb_build_object('provider','proof_provider'),
    jsonb_build_object('customerCommitment','proof-order-1'),null
  );
  perform atlas.link_organization_ledger_subject_internal_v1(
    (v_payment->>'entryId')::uuid,'commerce','customer_commitment','proof-order-1','evidence_for',
    jsonb_build_object('proof','v1'),'{}'::jsonb
  );

  if (select count(*) from atlas.organization_ledger_entries where organization_id=v_org)<>3 then
    raise exception 'Expected exactly three reference-company Ledger entries.';
  end if;
  if (select count(*) from atlas.organization_ledger_entries where organization_id=v_org and designation_status='unresolved')<>1 then
    raise exception 'Unresolved designation did not remain explicit.';
  end if;

  begin
    perform atlas.project_organization_ledger_event_internal_v1(
      v_org,null,'proof:resource-lot-ready','operations','different_semantic_identity','different-source',
      '2026-09-05 14:00:00+00','Illegal rebind',null,'established','designated','{}','{}','{}',null
    );
  exception when check_violation then
    v_denied:=true;
  end;
  if not v_denied then raise exception 'Event identity rebind did not fail closed.'; end if;

  if has_table_privilege('authenticated','atlas.organization_ledger_entries','select')
     or has_table_privilege('authenticated','atlas.organization_ledger_entries','insert')
     or has_table_privilege('authenticated','atlas.organization_ledger_subjects','select') then
    raise exception 'Authenticated role received unintended direct Ledger table privilege.';
  end if;
  if not (select c.relrowsecurity from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='atlas' and c.relname='organization_ledger_entries') then
    raise exception 'Ledger entries RLS is not enabled.';
  end if;

  perform set_config('request.jwt.claim.sub',v_user::text,true);
  v_page:=atlas.organization_ledger_owner_window_api_v1(
    v_org,'2026-09-05 13:00:00+00','2026-09-05 15:00:00+00',0,50
  );
  if jsonb_array_length(v_page->'items')<>3 then raise exception 'Owner Ledger read did not return proof entries.'; end if;
  ...
  if not v_denied then raise exception 'Non-owner Ledger read did not fail closed.'; end if;
end;
$proof$;

-- The transaction must leave no proof objects or temporary membership changes.
rollback;

...
alter table atlas.money_receipt_reversal_events
  ...
  add constraint money_receipt_reversal_events_receipt_fkey
    ...
    references atlas.money_receipts(id,organization_id,currency)
    on delete restrict,
  add constraint money_receipt_reversal_evidence_check
    check (
...
begin
  ...
  where r.provider_event_id=v_event.id;
  ...
  if exists(
    select 1 from atlas.money_receipts r
    where r.organization_id=v_attempt.organization_id and r.idempotency_key=v_key
  ) then
  ...
  insert into atlas.money_receipts(
    ...
    v_attempt.organization_id,v_amount,v_currency,v_received_at,'provider_event',v_attempt.connected_source_id,
    v_event.id,null,v_key,coalesce(p_metadata,'{}'::jsonb)
...
begin
  ...
  insert into atlas.money_receipts(
    ...
    p_organization_id,v_amount,v_currency,p_received_at,'manual',null,
    null,p_recorded_by_user_id,v_key,coalesce(p_metadata,'{}'::jsonb)
  ) returning * into v_existing;
...
begin
  ...
  -- Deterministic row locking prevents concurrent allocations from both seeing
  -- the same available/open amount.
  perform 1
  from (
    select 'o'::text as kind,o.id from atlas.money_obligations o where o.id=p_obligation_id
    union all
    select 'r'::text,r.id from atlas.money_receipts r where r.id=p_receipt_id
  ) q
  ...
  insert into atlas.money_receipt_allocations(
    ...
    v_receipt.organization_id,v_receipt.id,v_obligation.id,v_amount,v_receipt.currency,v_key,
...
begin
  ...
  from atlas.money_receipt_position_v1 p
  ...
  -- Lock affected allocations deterministically and verify that the supplied
  -- allocation reversals exactly reopen the paid position displaced by the
  -- receipt reversal. This prevents net allocated money from exceeding net
  -- received money after a refund/reversal.
  for v_allocation_id in
    select a.id
    from atlas.money_receipt_allocations a
    where a.id in (
      select (value->>'allocationId')::uuid from jsonb_array_elements(p_allocation_reversals)
    )
    order by a.id
    for update
  loop
  ...
  if (
    select count(*) from atlas.money_receipt_allocations a
  ...
  loop
    ...
    select greatest(
      v_allocation.amount-coalesce(sum(ar.amount),0),0
    )::numeric(14,2)
  ...
  insert into atlas.money_receipt_reversal_events(
    ...
    v_receipt.organization_id,v_receipt.id,v_amount,v_receipt.currency,v_kind,p_provider_event_id,
    p_recorded_by_user_id,p_occurred_at,v_key,coalesce(p_metadata,'{}'::jsonb)
  ) returning * into v_existing;
  ...
  loop
    ...
    insert into atlas.money_receipt_allocation_reversal_events(
      ...
      v_reverse_amount,v_receipt.currency,
      v_key||':allocation:'||v_allocation_id::text,
...
-- Strengthen the receipt projection so impossible state cannot be hidden by
-- greatest(...,0). The command layer prevents this condition; the read layer
-- still exposes it if custody is ever bypassed.

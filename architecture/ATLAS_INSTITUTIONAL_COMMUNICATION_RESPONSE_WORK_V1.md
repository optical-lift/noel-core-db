...
Collaboration uptake remains separate from responsibility transfer and is implemented through receiver-accepted participant/approver offers.

## Collaboration receiver uptake now implemented

Direct collaborator assignment is no longer merely fail-closed.

The current exact responsible Person may offer `participant` or `approver` participation through the generic Company Work collaboration receiver-uptake primitive. The target Reality Person must independently accept before an allocation exists.

Acceptance creates receiver-established participation and leaves exact responsibility unchanged.

For communication response Work, the compatibility adapter additionally requires that the target already be able to view the source Communication Endpoint. That source-visibility condition restricts the adapter but does not create Work authority.

See `architecture/ATLAS_COMPANY_WORK_COLLABORATION_RECEIVER_UPTAKE_V1.md`.

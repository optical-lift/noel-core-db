...
begin
  select pg_get_functiondef('atlas.record_flower_preparation_directive_result_for_member_v2(uuid,jsonb,jsonb,jsonb,text)'::regprocedure)
  into v_definition;

  if position(v_old in v_definition) = 0 then
    raise exception 'Expected directed flower final-tally metadata update was not found.';
  end if;

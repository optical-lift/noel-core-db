create or replace function public.wnph_publication_web_build_v2(p_expression_key text)
returns jsonb
language plpgsql
...
begin
  ...
  if v_expr.id is null then raise exception 'WNPH web build v2: Expression not found' using errcode='P0002'; end if;
  ...
  if coalesce((v_snapshot->>'reproducible_build_ready')::boolean,false) is not true then
    raise exception 'WNPH web build v2: publication master has % unreceipted media placements',v_snapshot->>'unreceipted_media_count' using errcode='55000';
  end if;
  ...
  loop
    if v_row.semantic_role='front_matter' then
      ...
      if not v_front_emitted then
        ...
        join public.v_wnph_expression_selected_publication_raster_v1 r
  ...
  return jsonb_build_object(
    'contract_version','wnph_publication_web_build_v2',
    ...
    'html',v_html,
    'placement_resolution_note','Source-surface-boundary plates use a chapter-opening fallback in web build v2 until paragraph-span anchoring is available.'
  );
end;
$function$;

revoke all on function public.wnph_publication_web_build_v2(text) from public, anon, authenticated;
grant execute on function public.wnph_publication_web_build_v2(text) to service_role;
...
begin
  ...
  where communication_conversation_id=v_common.id;
...
create or replace function atlas.require_common_communication_conversation_for_institutional_v1(
...
drop trigger if exists communication_email_draft_common_root_guard_v1
...
drop trigger if exists communication_outbound_operation_common_root_guard_v1
...
begin
  ...
  if v_common_id is not null then
    ...
    return jsonb_build_object(
      'contractVersion','organization_communication_command_pair_v1',
      'state','resolved',
      'communicationConversationId',v_common.id,
  ...
  if not p_create_if_missing then
    return jsonb_build_object(
      'contractVersion','organization_communication_command_pair_v1',
      'state','unbound',
      'communicationConversationId',null,
      'institutionalCompatibilityId',null,
      'communicationEndpointId',v_endpoint.id,
      'created',false
    );
  end if;
  ...
  insert into atlas.communication_conversations(
    ...
    jsonb_build_object(
      'createdFromOutboundIntent',true,
      'initiatedByMembershipId',v_actor.id,
      'communicationEndpointId',v_endpoint.id
    )
  ...
  insert into atlas.communication_conversation_endpoints(
    ...
    v_common_id,v_endpoint.id,'primary',jsonb_build_object('source','outbound_intent')
  );

  insert into atlas.institutional_conversations(
    organization_id,organization_unit_id,stable_key,subject,
    opened_at,last_activity_at,metadata
  ) values(
    v_endpoint.organization_id,v_endpoint.organization_unit_id,v_pair_key,
    nullif(btrim(coalesce(p_subject,'')),''),now(),now(),
    jsonb_build_object(
      'compatibilityForCommunicationConversationId',v_common_id,
      'createdFromOutboundIntent',true,
      'initiatedByMembershipId',v_actor.id
    )
...
-- Common-root outbound authorization. Existing transport and response authority
-- remains inside v2; this layer guarantees common Conversation custody first.
...
begin
  ...
  if v_existing.id is not null then
    ...
    if p_communication_conversation_id is not null
       and p_communication_conversation_id is distinct from v_common_id then
  ...
  return v_result || jsonb_build_object(
    'contractVersion','communication_email_send_intent_v3',
    'communicationConversationId',v_common_id,
    'institutionalCompatibilityId',v_institutional_id,
    'commandRoot','communication_conversation'
  );
...
begin
  ...
  return atlas.prepare_communication_email_send_internal_v3(
    ...
    'legacy_institutional_send_compatibility'
  );
end;
$function$;
...
begin
  ...
  return atlas.prepare_communication_email_send_internal_v3(
    ...
    p_to_recipients,p_cc_recipients,p_bcc_recipients,p_subject,p_body_text,p_body_html,
    p_attachment_refs,p_reply_to_communication_event_id,p_idempotency_key,
    'common_conversation_authenticated_send'
  );
end;
$function$;
...
begin
  ...
  v_result:=atlas.save_communication_email_draft_compatibility_self_api_v1(
    p_draft_id,p_communication_endpoint_id,p_institutional_conversation_id,
    p_reply_to_communication_event_id,p_to_recipients,p_cc_recipients,p_bcc_recipients,
    p_subject,p_body_text,p_body_html,p_attachment_refs,p_signature_id,p_send_after,p_metadata
  );
  ...
  set communication_conversation_id=v_common_id,
      metadata=metadata||case when v_common_id is null then '{}'::jsonb else jsonb_build_object(
        'communicationConversationId',v_common_id,
        'institutionalCompatibilityId',p_institutional_conversation_id,
        'commandRoot','communication_conversation'
      ) end,
      updated_at=now()
  where id=v_draft_id;
...
begin
  ...
  v_result:=atlas.save_communication_email_draft_compatibility_self_api_v1(
    p_draft_id,p_communication_endpoint_id,v_institutional_id,
    p_reply_to_communication_event_id,p_to_recipients,p_cc_recipients,p_bcc_recipients,
    p_subject,p_body_text,p_body_html,p_attachment_refs,p_signature_id,p_send_after,p_metadata
  );
  v_draft_id=(v_result->>'draftId')::uuid;
  ...
  return v_result || jsonb_build_object(
    'contractVersion','communication_email_draft_v3',
    'communicationConversationId',v_common_id,
    'institutionalCompatibilityId',v_institutional_id,
...
begin
  ...
  if v_row.draft_state='authorized' then
    return jsonb_build_object(
      'contractVersion','communication_email_draft_authorize_v2',
      'draftId',v_row.id,
  ...
  v_result:=atlas.prepare_communication_email_send_internal_v3(
    v_member.id,v_row.communication_endpoint_id,v_row.communication_conversation_id,
    v_row.institutional_conversation_id,v_row.to_recipients,v_row.cc_recipients,
    v_row.bcc_recipients,v_row.subject,v_body,v_row.body_html,v_row.attachment_refs,
    v_row.reply_to_communication_event_id,'draft:'||v_row.id::text,'draft_send_now'
  );
  v_operation_id=(v_result->>'outboundOperationId')::uuid;
  ...
  set draft_state='authorized',send_after=null,
      ...
      metadata=metadata||jsonb_build_object('commandRoot','communication_conversation'),
      updated_at=now()
  where id=v_row.id;
...
begin
  ...
  loop
    begin
      ...
      set draft_state='authorized',send_after=null,
          ...
          metadata=((metadata-'lastScheduleError')-'lastScheduleAttemptAt')||jsonb_build_object('commandRoot','communication_conversation'),
          updated_at=now()
      where id=v_row.id;
  ...
  return jsonb_build_object(
    'contractVersion','communication_email_draft_release_v3',
    'released',v_released,
    'failed',v_failed,
    'commandRoot','communication_conversation'
  );
...
begin
  ...
  return v_result||jsonb_build_object(
    'contractVersion','communication_conversation_claim_v1',
...
create or replace function atlas.handoff_communication_conversation_self_api_v1(
  p_communication_conversation_id uuid,
...
begin
  ...
  return v_result||jsonb_build_object(
    'contractVersion','communication_conversation_handoff_v1',
    'communicationConversationId',p_communication_conversation_id,
    'institutionalCompatibilityId',v_institutional_id
  );
end;
...
create or replace function atlas.add_communication_conversation_collaborator_self_api_v1(
  p_communication_conversation_id uuid,
  p_target_membership_id uuid,
...
begin
  ...
  return v_result||jsonb_build_object(
    'contractVersion','communication_conversation_collaborator_v1',
    'communicationConversationId',p_communication_conversation_id,
    'institutionalCompatibilityId',v_institutional_id
  );
end;
...
create or replace function atlas.remove_communication_conversation_collaborator_self_api_v1(
  p_communication_conversation_id uuid,
  p_target_membership_id uuid,
  p_allocation_role text default 'participant'
)
...
begin
  ...
  return v_result||jsonb_build_object(
    'contractVersion','communication_conversation_collaborator_remove_v1',
    'communicationConversationId',p_communication_conversation_id,
    'institutionalCompatibilityId',v_institutional_id
  );
end;
...
create or replace function atlas.set_communication_conversation_response_state_self_api_v1(
  p_communication_conversation_id uuid,
  p_to_state text,
  p_reason text default null
)
...
create or replace function atlas.communication_work_context_candidates_self_v2(
  p_communication_conversation_id uuid,
...
begin
  ...
  return v_result||jsonb_build_object(
    'contractVersion','communication_work_context_candidates_v2',
    'communicationConversationId',p_communication_conversation_id,
    'institutionalCompatibilityId',v_institutional_id
  );
end;
...
begin
  ...
  v_result:=atlas.create_communication_derived_work_self_api_v2(
    v_institutional_id,p_communication_event_id,p_excerpt,p_title,p_instructions,
    p_assignee_membership_id,p_due_at,p_handling_mode,p_semantic_frame,p_context_links,
    p_related_work,p_idempotency_key
  );
  ...
  return v_result||jsonb_build_object(
    'contractVersion','communication_derived_work_create_v3',
    'communicationConversationId',p_communication_conversation_id,
    'institutionalCompatibilityId',v_institutional_id,
    'communicationEventId',p_communication_event_id
  );

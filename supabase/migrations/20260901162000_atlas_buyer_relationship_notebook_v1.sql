...
--   1. projects exact linked facts into one read-only relationship position and
--      event-true timeline;
...
--
-- Execution adapter note:
-- Buyer follow-up currently enters the legacy Task execution membrane with
-- task_scope='farm_operation' because that is the present operating-unit Task
-- carrier. That value is an execution adapter, not relationship-domain or CRM
-- authority. Company Work remains durable responsibility authority.
...
create unique index if not exists tasks_buyer_next_action_idempotency_v1
on atlas.tasks (farm_id, ((metadata ->> 'relationship_next_action_idempotency_key')))
where (metadata ->> 'relationship_action_kind') = 'buyer_follow_up'
  and nullif(metadata ->> 'relationship_next_action_idempotency_key','') is not null;
...
select
  ...
  latest_contact.last_contact_event_id,
  latest_contact.last_contacted_at,
  latest_contact.last_contact_method,
  latest_contact.last_contact_outcome,
  p.next_action as remembered_next_action,
  p.buying_stage,
  p.buying_cadence,
  ...
  p.last_sale_order_id,
  p.last_purchase_amount,
  p.active_standing_order_count,
  p.next_standing_due_date,
  p.pursuit_recommendation,
  p.access_notes,
  r.source_person,
  r.source_date,
  r.metadata as relationship_metadata,
  p.buying_profile_metadata,
  r.created_at,
  r.updated_at,
  ...
  (nullif(btrim(coalesce(p.next_action,'')),'') is not null and action_task.id is null) as needs_next_action_scheduling
from atlas.v_flower_buyer_position_v1 p
join atlas.buyer_relationship_reconstruction r
  on r.id=p.buyer_relationship_id
 and r.farm_id=p.farm_id
left join lateral (
  select
    e.id as last_contact_event_id,
    e.occurred_at as last_contacted_at,
    e.contact_method as last_contact_method,
    e.outcome as last_contact_outcome
  from atlas.buyer_contact_events e
  where e.farm_id=p.farm_id
    and e.buyer_relationship_id=p.buyer_relationship_id
...
left join lateral (
  select t.*
  from atlas.tasks t
  where t.farm_id=p.farm_id
    and t.metadata ->> 'relationship_action_kind'='buyer_follow_up'
    and t.metadata ->> 'buyer_relationship_id'=p.buyer_relationship_id::text
    and t.status in ('open','blocked')
  order by t.due_date nulls last,t.created_at,t.id
  limit 1
) action_task on true
left join lateral (
  select cw.work_item_id,cw.work_state
  from atlas.company_work_position_v2 cw
  where action_task.id is not null
    and cw.source_object_type='legacy_task'
    and cw.source_object_id=action_task.id
  order by cw.updated_at desc,cw.work_item_id
  limit 1
...
comment on view atlas.buyer_relationship_position_v1 is
  'Current buyer relationship position composed from existing buyer memory, exact buyer facts, and current Task -> Company Work responsibility. It creates no CRM lifecycle authority.';
...
select
  ...
  jsonb_strip_nulls(jsonb_build_object(
    ...
    'note',d.note,
    'commercialTruthAuthority','flower_demand_order'
  ))
...
select
  ...
  d.buyer_relationship_id,
  c.created_at,
  'flower_demand_committed'::text,
  'flower_demand_commitment_event'::text,
  c.id,
  'Flower demand committed'::text,
  coalesce(nullif(c.note,''),coalesce(nullif(d.customer_label,''),'Flower customer') || ' · commitment accepted')::text,
  c.to_strength,
  jsonb_strip_nulls(jsonb_build_object(
    'demandOrderId',d.id,
    'fromStrength',c.from_strength,
    'toStrength',c.to_strength,
...
join atlas.flower_demand_orders d
  on d.id=c.demand_order_id
 and d.farm_id=c.farm_id
where d.buyer_relationship_id is not null
...
select
  ...
  d.buyer_relationship_id,
  c.created_at,
  'flower_demand_cancelled'::text,
  'flower_demand_order_cancellation_event'::text,
  c.id,
  'Flower demand cancelled'::text,
  coalesce(nullif(c.note,''),replace(c.reason_kind,'_',' '),'Demand cancelled')::text,
  'cancelled'::text,
  jsonb_strip_nulls(jsonb_build_object(
    'demandOrderId',d.id,
    'reasonKind',c.reason_kind,
    'recordedByMembershipId',c.recorded_by_membership_id,
    'note',c.note,
    'idempotencyKey',c.idempotency_key,
    'saleTruth',false,
...
from atlas.flower_demand_order_cancellation_events c
join atlas.flower_demand_orders d
  on d.id=c.demand_order_id
 and d.farm_id=c.farm_id
where d.buyer_relationship_id is not null
...
select
  ...
  coalesce(nullif(s.customer_label,''),'Flower customer') || ' · ' || trim(to_char(s.total_amount,'FM999999990.00')) || ' ' || s.currency,
  'recorded'::text,
  jsonb_strip_nulls(jsonb_build_object(
...
select
  ...
  s.buyer_relationship_id,
  c.created_at,
  'flower_sale_cancelled'::text,
  'flower_sale_order_cancellation_event'::text,
  c.id,
  'Flower Sale cancelled'::text,
  coalesce(nullif(c.note,''),replace(c.reason_kind,'_',' '),'Sale cancelled')::text,
  'cancelled'::text,
  jsonb_strip_nulls(jsonb_build_object(
    'saleOrderId',s.id,
    'reasonKind',c.reason_kind,
...
join atlas.flower_sale_orders s
  on s.id=c.sale_order_id
 and s.farm_id=c.farm_id
where s.buyer_relationship_id is not null
...
select
  ...
  jsonb_strip_nulls(jsonb_build_object(
    ...
    'workStateAtReadTime',work.work_state,
    'idempotencyKey',t.metadata ->> 'relationship_next_action_idempotency_key',
    'executionAdapter',t.metadata ->> 'relationship_execution_adapter'
  ))
from atlas.tasks t
left join lateral (
...
select
  ...
  (t.metadata ->> 'buyer_relationship_id')::uuid,
  tr.created_at,
  'next_action_transition'::text,
  'task_transition'::text,
  tr.id,
  concat('Follow-up · ',replace(coalesce(tr.transition,'updated'),'_',' '))::text,
  coalesce(nullif(tr.note,''),nullif(tr.reason,''),nullif(t.note,''),'Buyer follow-up updated')::text,
  coalesce(tr.next_status,tr.transition),
  jsonb_strip_nulls(jsonb_build_object(
    'taskId',t.id,
    'transition',tr.transition,
    'previousStatus',tr.previous_status,
    'nextStatus',tr.next_status,
    'previousDueDate',tr.previous_due_date,
    'targetDate',tr.target_date,
    'actionKey',tr.action_key,
    'workClass',tr.work_class,
    'reason',tr.reason,
    'actorMembershipId',tr.actor_membership_id,
    'actorRole',tr.actor_role,
    'idempotencyKey',tr.idempotency_key
  ))
from atlas.task_transitions tr
join atlas.tasks t
  on t.id=tr.task_id
 and t.farm_id=tr.farm_id
where t.metadata ->> 'relationship_action_kind'='buyer_follow_up'
  and t.metadata ->> 'buyer_relationship_id' ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$';
...
comment on view atlas.buyer_relationship_timeline_v1 is
  'Event-true buyer relationship chronology. Original rows keep their birth state; later commitment, cancellation, and Task transitions appear as later events rather than rewriting history.';
...
begin
  ...
  -- Serialize the idempotency namespace even when two different relationship rows
  -- race with the same key. The unique partial index remains the database backstop.
  perform pg_advisory_xact_lock(hashtextextended(v_relationship.farm_id::text||':buyer-next-action:'||v_key,0));
  ...
  if v_existing.id is not null then
    if v_existing.metadata ->> 'buyer_relationship_id' is distinct from v_relationship.id::text
       or v_existing.note is distinct from v_action
       or v_existing.due_date is distinct from p_due_on
       or v_existing.assigned_membership_id is distinct from v_assignee.id then
      raise exception 'Buyer next-action idempotency key replay conflicts with the original request.' using errcode='22023';
    end if;
    ...
    return jsonb_build_object(
      ...
      'workState',v_work_state,
      'assignedMembershipId',v_existing.assigned_membership_id,
      'dueOn',v_existing.due_date,
      'deduplicated',true,
      'relationshipMemoryCopied',false,
      'paymentTruth',false,
      'executionAdapter','legacy_farm_operation_task_v1'
    );
  end if;
  ...
  insert into atlas.tasks(
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'relationship_execution_adapter','legacy_farm_operation_task_v1',
      'relationship_execution_adapter_boundary','task_scope_is_operating_unit_execution_adapter_not_relationship_authority',
  ...
  return jsonb_build_object(
    ...
    'workItemId',v_work_item_id,
    'workState',v_work_state,
    'assignedMembershipId',v_assignee.id,
    'dueOn',v_task.due_date,
    'deduplicated',false,
    'relationshipMemoryCopied',false,
    'paymentTruth',false,
    'executionAdapter','legacy_farm_operation_task_v1'
...
comment on function atlas.create_buyer_relationship_next_action_core_v1(uuid,text,date,uuid,uuid,text,text,boolean) is
  'Converts one explicit remembered buyer follow-up into canonical Task -> Company Work responsibility. Idempotent replay must match relationship, action, due date, and assignee exactly.';
...
(
  ...
  jsonb_build_object(
    ...
    'executionAdapter','task_scope=farm_operation is the current operating-unit Task adapter, not relationship authority.',
    'commercialTruth','Creates no Demand, Sale, fulfillment, inventory, or payment truth.',
    'idempotency','Exact replay deduplicates; divergent replay of the same key is rejected.',

...
begin
  ...
  if v_expr is null then
    raise exception 'WNPH expression snapshot v4: Expression not found' using errcode='P0002';
  end if;
  ...
  select
    ...
    count(*) filter(
      where r.raster_sha256 is not null
        and coalesce(q.quality_status,'unreviewed') <> 'accepted'
    ),
    string_agg(
      lpad(m.sequence_ordinal::text,6,'0')||'|'||m.placement_key||'|'||m.media_role||'|'||m.anchor_kind||'|'||coalesce(m.anchor_block_key,'')||'|'||m.source_asset_key||'|'||m.anchor_data::text||'|'||m.placement_policy::text||'|'||m.accessibility::text||'|'||coalesce(r.receipt_key,'MISSING')||'|'||coalesce(r.fetch_uri,'MISSING')||'|'||coalesce(r.receipt_media_type,'MISSING')||'|'||coalesce(r.byte_length::text,'MISSING')||'|'||coalesce(r.raster_sha256,'MISSING')||'|QUALITY|'||coalesce(q.quality_status,'unreviewed'),
      E'\n' order by m.sequence_ordinal,m.placement_key
    )
  into v_media,v_receipts,v_missing,v_unaccepted,v_media_payload
  ...
  left join public.v_wnph_selected_publication_raster_quality_v1 q
    on q.expression_key=m.expression_key and q.placement_key=m.placement_key
  where m.expression_key=p_expression_key;

  v_hash:=encode(extensions.digest(convert_to(
    'WNPH_PUBLICATION_EXPRESSION_SNAPSHOT_V4'||E'\nTEXT\n'||coalesce(v_text_payload,'')||E'\nSEMANTIC_MEDIA_WITH_SELECTED_RASTER_RECEIPTS_AND_VISUAL_QUALITY\n'||coalesce(v_media_payload,''),
    'UTF8'
  ),'sha256'),'hex');

  return jsonb_build_object(
    'contract_version','wnph_publication_expression_snapshot_v4',
    ...
    'unaccepted_media_count',coalesce(v_unaccepted,0),
    'reproducible_build_ready',coalesce(v_missing,0)=0 and coalesce(v_unaccepted,0)=0,
    'publication_raster_contract','explicit-selected-governed-raster-with-visual-quality:v2',
    'publication_raster_quality_contract','publication_media_raster_quality_review_v1',
    'semantic_media_fields_hashed',jsonb_build_array('sequence_ordinal','placement_key','media_role','anchor_kind','anchor_block_key','source_asset_key','anchor_data','placement_policy','accessibility','selected_raster_receipt','visual_quality_status'),

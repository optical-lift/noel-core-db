...
begin
  ...
  select vault_secret_id into v_existing_secret_id from atlas.connected_source_secret_refs where connected_source_id=v_source.id and credential_kind=v_kind for update;

  if v_existing_secret_id is null then
    select vault.create_secret(p_secret,v_name,coalesce(nullif(btrim(p_description),''),'Atlas provider credential'),null) into v_secret_id;
    insert into atlas.connected_source_secret_refs(connected_source_id,credential_kind,vault_secret_id) values(v_source.id,v_kind,v_secret_id);
  else
    ...
    update atlas.connected_source_secret_refs set updated_at=now() where connected_source_id=v_source.id and credential_kind=v_kind;
  end if;

  return jsonb_build_object('contractVersion','connected_source_secret_custody_v1','connectedSourceId',v_source.id,'credentialKind',v_kind,'stored',true);
end;
$function$;
...
begin
  ...
  select count(*),(array_agg(id order by id))[1] into v_count,v_source_id
  ...
  select * into v_source from atlas.connected_sources where id=v_source_id;
  return jsonb_build_object('contractVersion','provider_webhook_source_resolution_v1','connectedSourceId',v_source.id,'providerKey',v_source.provider_key,'providerAccountKey',v_source.provider_account_key,'custodianKind',case when v_source.custodian_user_id is not null then 'human' else 'organization' end,'custodianUserId',v_source.custodian_user_id,'organizationId',v_source.custodian_organization_id);
end;
$function$;
...
returns jsonb language sql set search_path=pg_catalog,atlas,public as $function$ select atlas.validate_provider_connection_callback_service_v1(p_session_id,p_provider_key,p_state_nonce_digest); $function$;
...
returns jsonb language sql set search_path=pg_catalog,atlas,public as $function$ select atlas.store_connected_source_secret_service_v1(p_connected_source_id,p_credential_kind,p_secret,p_description); $function$;
...
returns jsonb language sql set search_path=pg_catalog,atlas,public as $function$ select atlas.resolve_provider_webhook_source_service_v1(p_provider_key,p_provider_account_key); $function$;

...
-- Gate 1 continuation: complete the employee-entry membrane needed by ENT-03
-- and by re-entry routing. This remains access/identity infrastructure only.
-- It does not create position, responsibility, Principal authority, execution
-- authority, semantic exposure, or Personal Atlas ownership.
...
begin
  ...
  if v_email is null then
    return jsonb_build_object('ok',false,'reason','authenticated_email_unavailable','items','[]'::jsonb);
  end if;
  ...
  return jsonb_build_object(
    'ok',true,
    'contractVersion','organization_access_invitation_v1',
    'items',v_items
  );
end;
$function$;

create or replace function atlas.decline_organization_employee_invitation_self_api_v1(
...
begin
  ...
  if v_email is null then
    raise exception 'Authenticated email unavailable.' using errcode='42501';
  end if;
  ...
  if v_invite.status='declined' and v_invite.declined_by_user_id=v_uid then
    return jsonb_build_object('ok',true,'replayed',true,'invitationId',v_invite.id,'status','declined');
  end if;

  if v_invite.status<>'issued' then
    raise exception 'Invitation is not open for decline.' using errcode='55000';
  end if;
  ...
  update atlas.organization_access_invitations
    set status='declined',declined_by_user_id=v_uid,declined_at=now(),
        metadata=metadata||jsonb_build_object('declineContractVersion','organization_access_invitation_v1'),
        updated_at=now()
  where id=v_invite.id;
...
create or replace function atlas.mark_organization_employee_invitation_not_me_self_api_v1(
  p_invitation_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  if v_invite.status<>'issued' then
    return jsonb_build_object('ok',false,'reason','invitation_not_open','invitationId',v_invite.id,'status',v_invite.status);
  end if;

  update atlas.organization_access_invitations
  set metadata=metadata||jsonb_build_object(
        'recipientDispute',jsonb_build_object(
          'kind','not_me',
          'reportedByUserId',v_uid,
          'reportedAt',now()
        )
      ),
      updated_at=now()
...
('atlas.list_pending_organization_employee_invitations_self_api_v1()','app_endpoint','verified','active',true,true,true,1,1,
 jsonb_build_object('source','atlas_organization_access_entry_completion_v1','purpose','List only open organization employee invitations addressed to the signed-in verified email.','truthBoundary','Read only; invitation visibility does not create access.','classificationRuleVersion',3),false),
('atlas.decline_organization_employee_invitation_self_api_v1(uuid)','app_endpoint','verified','active',true,true,true,1,1,
 jsonb_build_object('source','atlas_organization_access_entry_completion_v1','purpose','Decline an organization employee invitation addressed to the signed-in verified email.','truthBoundary','Invitation state only; does not erase institutional identity/history.','classificationRuleVersion',3),false),
('atlas.mark_organization_employee_invitation_not_me_self_api_v1(uuid)','app_endpoint','verified','active',true,true,true,1,1,
 jsonb_build_object('source','atlas_organization_access_entry_completion_v1','purpose','Flag that the verified recipient disputes being the intended human without accepting or silently merging identity.','truthBoundary','Invitation evidence only; invitation remains unaccepted.','classificationRuleVersion',3),false),
('atlas.organization_access_self_api_v1()','app_endpoint','verified','active',true,true,true,1,1,

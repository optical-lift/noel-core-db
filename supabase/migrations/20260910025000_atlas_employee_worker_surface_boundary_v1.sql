...
-- Gate 2 continuation: employee Worker Day reads and commands must be carried
-- by one current institutional relationship. A farm membership by itself is
-- not Atlas access and must never be enough to reopen worker-facing authority.
...
begin
  ...
  if p_farm_id is null or p_delivery_membership_id is null then
    return jsonb_build_object('ok',false,'reason','worker_context_required');
  end if;
  ...
  where fm.id=p_delivery_membership_id
    and fm.farm_id=p_farm_id
    and fm.user_id=v_uid
    and fm.active=true
    and fm.role='farm_hand'
    and fm.identity_subject_id is not null;
  if v_farm_member.id is null then
    return jsonb_build_object('ok',false,'reason','active_worker_membership_required');
  end if;
  ...
  where m.organization_id=v_farm.organization_id
    and m.user_id=v_uid
    and m.identity_subject_id=v_farm_member.identity_subject_id
    and m.active=true
    and m.role='member'
  order by m.created_at
  limit 1;
  if v_org_member.id is null then
    return jsonb_build_object('ok',false,'reason','employee_organization_membership_required');
  end if;
  ...
  where s.organization_id=v_farm.organization_id
  ...
  if v_seat.id is null then
    return jsonb_build_object('ok',false,'reason','active_employee_seat_required');
  end if;

  if not exists(
    select 1 from atlas.organization_member_credentials c
    where c.organization_id=v_farm.organization_id
    ...
    return jsonb_build_object('ok',false,'reason','active_employee_credential_required');
  end if;

  select a.*,p.* into v_appointment,v_position
  from atlas.organization_position_appointments a
  join atlas.organization_positions p
    on p.id=a.position_id
   and p.organization_id=a.organization_id
   and p.status='active'
  where a.organization_id=v_farm.organization_id
    and a.organization_membership_id=v_org_member.id
    and a.identity_subject_id=v_org_member.identity_subject_id
    and a.status='active'
    and a.begins_at<=now()
  ...
  limit 1;
  if v_appointment.id is null or v_position.id is null then
    return jsonb_build_object('ok',false,'reason','active_worker_appointment_required');
  end if;

  return jsonb_build_object(
    'ok',true,
    'contractVersion','organization_employee_worker_context_v1',
    ...
    'deliveryMembershipId',v_farm_member.id,
    'identitySubjectId',v_org_member.identity_subject_id,
...
-- A temporary Work Pass is a credential carrier, not a way around the paid
-- employee relationship. Keep the pilot available only while its membership,
-- seat, identity, and institutional appointment all remain live.
  ...
  select coalesce(
    (
      ...
      join atlas.organization_memberships om
        on om.id=c.organization_membership_id
       and om.active=true
       and om.role='member'
      join atlas.farm_memberships fm
        on fm.id=c.delivery_membership_id
       and fm.active=true
       and fm.role='farm_hand'
       and fm.user_id=om.user_id
       and fm.identity_subject_id=om.identity_subject_id
      join atlas.farms f
        on f.id=fm.farm_id
       and f.organization_id=om.organization_id
       and f.organization_unit_id is not null
      join atlas.organization_employee_seats seat
        on seat.organization_id=om.organization_id
       and seat.organization_membership_id=om.id
       and seat.identity_subject_id=om.identity_subject_id
       and seat.seat_class='employee'
       and seat.status='active'
       and seat.billing_state in ('active','waived')
      join atlas.organization_position_appointments appt
        on appt.organization_id=om.organization_id
       and appt.organization_membership_id=om.id
       and appt.identity_subject_id=om.identity_subject_id
       and appt.status='active'
       and appt.begins_at<=clock_timestamp()
       and (appt.ends_at is null or appt.ends_at>clock_timestamp())
      join atlas.organization_positions pos
        on pos.id=appt.position_id
       and pos.organization_id=om.organization_id
       and pos.organization_unit_id=f.organization_unit_id
...
-- Pilot history previously required every event to belong to a Work Pass
-- session. Authenticated employee-seat actions need an explicit human actor
-- instead. Exactly one credential carrier owns each event.
...
alter table atlas.worker_delivery_pilot_events
  drop constraint if exists worker_delivery_pilot_events_actor_check;

alter table atlas.worker_delivery_pilot_events
  add constraint worker_delivery_pilot_events_actor_check
  check (num_nonnulls(session_id,actor_user_id)=1);

create index if not exists worker_delivery_pilot_events_actor_user_idx
  on atlas.worker_delivery_pilot_events(actor_user_id,event_seq);

-- Same projection-event semantics as the temporary pilot, but the credential
-- is now the authenticated employee account. The command cannot be reached by
-- a mere farm membership: it must pass the full employee context above first.
...
begin
  ...
  if v_action not in ('start','stop','done','reopen','switch_finish','switch_stop','report_unscheduled') then
  ...
  if not coalesce((v_context->>'ok')::boolean,false) then
    return jsonb_build_object('ok',false,'code','employee_access_required','reason',v_context->>'reason');
  end if;

  v_organization_id:=(v_context->>'organizationId')::uuid;
  v_organization_membership_id:=(v_context->>'organizationMembershipId')::uuid;

  perform pg_advisory_xact_lock(hashtextextended(p_delivery_membership_id::text,0));

  if v_action='report_unscheduled' then
    if nullif(btrim(p_reported_title),'') is null or length(btrim(p_reported_title))>240 then
    ...
    insert into atlas.worker_delivery_pilot_events(
      ...
      projection_id,session_id,actor_user_id,event_kind,effective_at,reported_title,metadata
    ) values(
      v_organization_id,v_organization_membership_id,p_delivery_membership_id,
      null,null,v_uid,'unscheduled_work_reported',v_effective,btrim(p_reported_title),
      jsonb_build_object('source','organization_employee_seat','employeeSeatId',v_context->>'employeeSeatId')
  ...
  where e.projection_id=p_projection_id
    and e.delivery_membership_id=p_delivery_membership_id
    and e.organization_membership_id=v_organization_membership_id
  ...
  where delivery_membership_id=p_delivery_membership_id
  for update;
  ...
  if v_action='start' then
    ...
    if v_active.delivery_membership_id is not null then
      ...
      select p.title into v_active_title from atlas.worker_week_projection p
      where p.id=v_active.projection_id and p.membership_id=p_delivery_membership_id
        and p.organization_id=v_organization_id and p.organization_membership_id=v_organization_membership_id;
    ...
    insert into atlas.worker_delivery_pilot_events(
      organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata
    ) values(v_organization_id,v_organization_membership_id,p_delivery_membership_id,p_projection_id,null,v_uid,'start',v_effective,
      jsonb_build_object('source','organization_employee_seat','employeeSeatId',v_context->>'employeeSeatId')) returning id into v_event_id;
    insert into atlas.worker_delivery_pilot_active_attention(delivery_membership_id,projection_id,start_event_id,started_effective_at,updated_at)
    values(p_delivery_membership_id,p_projection_id,v_event_id,v_effective,v_now);
  ...
  if v_action='stop' then
    ...
    insert into atlas.worker_delivery_pilot_events(
      organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata
    ) values(v_organization_id,v_organization_membership_id,p_delivery_membership_id,p_projection_id,null,v_uid,'stop',v_effective,
      jsonb_build_object('source','organization_employee_seat','employeeSeatId',v_context->>'employeeSeatId'));
  ...
  if v_action='done' then
    ...
    insert into atlas.worker_delivery_pilot_events(
      organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata
    ) values(v_organization_id,v_organization_membership_id,p_delivery_membership_id,p_projection_id,null,v_uid,'done_reported',v_effective,
      jsonb_build_object('source','organization_employee_seat','employeeSeatId',v_context->>'employeeSeatId'));
  ...
  if v_action='reopen' then
    ...
    insert into atlas.worker_delivery_pilot_events(
      organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata
    ) values(v_organization_id,v_organization_membership_id,p_delivery_membership_id,p_projection_id,null,v_uid,'completion_reopened',v_effective,
      jsonb_build_object('source','organization_employee_seat','employeeSeatId',v_context->>'employeeSeatId'));
    return jsonb_build_object('ok',true,'status','reopened');
  end if;

  if v_action in ('switch_finish','switch_stop') then
    if v_latest_completion='done_reported' then return jsonb_build_object('ok',false,'code','already_completed'); end if;
    if v_active.delivery_membership_id is null then
      insert into atlas.worker_delivery_pilot_events(
        organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata
      ) values(v_organization_id,v_organization_membership_id,p_delivery_membership_id,p_projection_id,null,v_uid,'start',v_now,
        jsonb_build_object('source','organization_employee_seat','employeeSeatId',v_context->>'employeeSeatId')) returning id into v_event_id;
      insert into atlas.worker_delivery_pilot_active_attention(delivery_membership_id,projection_id,start_event_id,started_effective_at,updated_at)
      values(p_delivery_membership_id,p_projection_id,v_event_id,v_now,v_now);
      return jsonb_build_object('ok',true,'status','started');
    end if;
    if v_active.projection_id=p_projection_id then return jsonb_build_object('ok',true,'status','already_active'); end if;
    if v_action='switch_stop' and (v_effective>v_now+interval '1 minute' or v_effective<v_active.started_effective_at) then
      return jsonb_build_object('ok',false,'code','invalid_stop_time');
    end if;

    insert into atlas.worker_delivery_pilot_events(
      organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata
    ) values(v_organization_id,v_organization_membership_id,p_delivery_membership_id,v_active.projection_id,null,v_uid,'stop',
      case when v_action='switch_stop' then v_effective else v_now end,
      jsonb_build_object('source','organization_employee_seat','employeeSeatId',v_context->>'employeeSeatId'));
    if v_action='switch_finish' then
      insert into atlas.worker_delivery_pilot_events(
        organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata
      ) values(v_organization_id,v_organization_membership_id,p_delivery_membership_id,v_active.projection_id,null,v_uid,'done_reported',v_now,
        jsonb_build_object('source','organization_employee_seat','employeeSeatId',v_context->>'employeeSeatId'));
    end if;
    insert into atlas.worker_delivery_pilot_events(
      organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata
    ...
    set projection_id=p_projection_id,start_event_id=v_event_id,started_effective_at=v_now,updated_at=v_now
    where delivery_membership_id=p_delivery_membership_id;
...
insert into atlas.authenticated_rpc_registry(
  ...
  'atlas.worker_delivery_employee_transition_self_api_v1(uuid, text, uuid, timestamp with time zone, text)',
  'app_endpoint','verified','active',true,true,true,1,1,
  jsonb_build_object(
    'source','atlas_employee_worker_surface_boundary_v1',
    'purpose','Record current Worker Day attention and reporting events for the signed-in organization employee.',
    'truthBoundary','Requires the same institutional person across active farm membership, organization membership, employee seat, authenticated credential, and current position appointment. Projection assignment limits action scope; no new responsibility or authority is created.',

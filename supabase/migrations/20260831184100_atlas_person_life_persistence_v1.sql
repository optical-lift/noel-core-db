...
--
-- Adds first-party persistence around the generic Life Signal / Goal / Rhythm /
-- State Consequence reducers without inventing farm or organization identity.
-- Definitions preserve the source-custodied signal; evolving state is append-only
-- evidence/evaluation history; consequence instances are a current projection.
--
-- This migration does not create Clock placements, tasks, practitioner sharing,
-- diagnoses, or domain results.
...
create table atlas.person_life_definitions (
  ...
  constraint person_life_definitions_source_key_not_blank check (btrim(source_key) <> ''),
  constraint person_life_definitions_subject_domain_not_blank check (btrim(subject_domain) <> ''),
  constraint person_life_definitions_subject_kind_not_blank check (btrim(subject_kind) <> ''),
  constraint person_life_definitions_subject_id_not_blank check (btrim(subject_id) <> ''),
  constraint person_life_definitions_source_domain_not_blank check (btrim(source_domain) <> ''),
  constraint person_life_definitions_source_kind_not_blank check (btrim(source_kind) <> ''),
  constraint person_life_definitions_source_id_not_blank check (btrim(source_id) <> ''),
  constraint person_life_definitions_source_identity unique (owner_user_id, signal_kind, source_key)
);

comment on table atlas.person_life_definitions is
  'First-party persistence envelope for person-owned Goal, explicit Rhythm, and State Consequence definitions. The canonical Life Signal and derived pure-engine packet are retained together; no farm or organization identity is fabricated.';
...
create table atlas.person_life_relations (
  ...
  created_at timestamptz not null default now(),
  constraint person_life_relations_kind_not_blank check (btrim(relation_kind) <> ''),
  constraint person_life_relations_target_domain_not_blank check (btrim(target_domain) <> ''),
  constraint person_life_relations_target_kind_not_blank check (btrim(target_kind) <> ''),
  constraint person_life_relations_target_id_not_blank check (btrim(target_id) <> ''),
  constraint person_life_relations_identity unique (definition_id, relation_kind, target_domain, target_kind, target_id)
);

comment on table atlas.person_life_relations is
  'Queryable neutral relations carried by a person-owned Life Signal. Relation custody is preserved separately and never promotes a generic relation into causal truth.';
...
create table atlas.person_life_state_events (
  ...
  created_at timestamptz not null default now(),
  constraint person_life_state_events_source_key_not_blank check (btrim(source_key) <> ''),
  constraint person_life_state_events_source_identity unique (owner_user_id, source_key)
);

comment on table atlas.person_life_state_events is
  'Append-oriented evidence/evaluation history for person-owned life definitions. Engine evaluations remain derived from explicit inputs; this history is not Clock or task truth.';
...
create table atlas.person_life_consequence_instances (
  ...
  constraint person_life_consequence_instances_stable_key_not_blank check (btrim(stable_key) <> ''),
  constraint person_life_consequence_instances_identity unique (definition_id, stable_key)
);

comment on table atlas.person_life_consequence_instances is
  'Current projection of explicitly evaluated person-owned State Consequences. Requirement, carrier, execution readiness, and placement remain independent fields; absence from a later evaluation does not silently resolve an instance.';
...
begin
  ...
  if new.life_signal->'scope'->>'kind' <> 'person'
    ...
    raise exception 'Person-owned life definition scope must be the owner user.' using errcode = '23514';
  end if;
...
begin
  ...
  if v_owner_user_id is null or v_owner_user_id is distinct from new.owner_user_id then
...
declare
  ...
  v_signal_kind text;
begin
  select d.owner_user_id, d.signal_kind
  into v_owner_user_id, v_signal_kind
  from atlas.person_life_definitions d
  where d.id = new.definition_id;
  ...
  if (v_signal_kind = 'goal' and new.event_kind <> 'goal_evaluation')
     or (v_signal_kind = 'rhythm' and new.event_kind not in ('rhythm_satisfaction','rhythm_evaluation'))
     or (v_signal_kind = 'consequence' and new.event_kind not in ('consequence_evaluation','consequence_resolution')) then
...
begin
  ...
  if v_owner_user_id is null
     or v_owner_user_id is distinct from new.owner_user_id
     or v_signal_kind <> 'consequence' then
    raise exception 'Consequence-instance custody requires the same owner and a consequence definition.' using errcode = '23514';
  end if;
...
on atlas.person_life_relations
for select
to authenticated
using (owner_user_id = auth.uid());
...
begin
  ...
  if v_source_key = '' then
    raise exception 'sourceKey is required for idempotent person-life definitions.' using errcode = '22023';
  end if;
  ...
  if v_signal->'scope'->>'kind' <> 'person'
     or v_signal->'scope'->>'id' <> v_user_id::text then
    raise exception 'Person life signal scope must be the authenticated user.' using errcode = '42501';
  end if;

  v_signal_kind := v_signal->>'signalKind';
  if v_signal_kind not in ('goal','rhythm','consequence') then
  ...
  if v_signal_kind = 'goal' then
    v_packet := atlas.life_signal_to_goal_packet_v1(v_signal);
    v_initial_evaluation := atlas.evaluate_life_goal_state_v1(v_packet, '[]'::jsonb);
  elsif v_signal_kind = 'rhythm' then
    v_packet := atlas.life_signal_to_rhythm_packet_v1(v_signal);
    if v_packet->>'strategyState' <> 'supported' then
      raise exception 'Person Rhythm persistence v1 requires an explicit supported rhythm strategy.' using errcode = '22023';
    end if;
    v_initial_evaluation := atlas.evaluate_life_lease_rhythm_v1(v_packet, null, now());
  else
    v_packet := atlas.life_signal_to_consequence_packet_v1(v_signal);
    v_initial_evaluation := null;
  end if;
  ...
  values (
    ...
    v_signal_kind,
    v_source_key,
    v_signal->'subject'->>'domain',
    v_signal->'subject'->>'kind',
    v_signal->'subject'->>'id',
    v_signal->'source'->>'domain',
    v_signal->'source'->>'kind',
    v_signal->'source'->>'id',
    v_signal,
    v_packet,
  ...
  if v_definition_id is not null then
    ...
    select d.id, d.life_signal, d.engine_packet
    into v_definition_id, v_existing_signal, v_existing_packet
    from atlas.person_life_definitions d
    where d.owner_user_id = v_user_id
      and d.signal_kind = v_signal_kind
      and d.source_key = v_source_key;

    if v_definition_id is null
       or v_existing_signal is distinct from v_signal
       or v_existing_packet is distinct from v_packet then
      raise exception 'sourceKey retry does not match the existing person-life definition.' using errcode = '23505';
    end if;
  end if;

  if v_created then
    for v_relation in
      select value from jsonb_array_elements(coalesce(v_signal->'relations','[]'::jsonb))
    loop
      ...
      if jsonb_typeof(v_target) = 'object'
        ...
        values (
          v_definition_id,
          v_user_id,
          v_relation->>'relationKind',
          v_target->>'domain',
          v_target->>'kind',
          v_target->>'id',
          nullif(v_relation->>'relationBasis',''),
          nullif(v_relation->>'relationStatus',''),
          jsonb_build_object(
            'source', v_signal->'source',
            'epistemic', v_signal->'epistemic',
            'relation', v_relation
          )
        )
...
comment on function atlas.create_person_life_definition_api_v1(jsonb) is
  'Authenticated first-party persistence adapter for Goal, explicit Rhythm, and State Consequence Life Signals. It fixes custody to the caller, preserves source/provenance, and invokes only pure shared reducers; no task or Clock authority is created.';

create or replace function atlas.record_person_life_state_api_v1(
...
begin
  ...
  select d.signal_kind, d.engine_packet
  into v_signal_kind, v_packet
  from atlas.person_life_definitions d
  ...
  v_source_key := btrim(coalesce(p_payload->>'sourceKey',''));
  if v_source_key = '' then
    raise exception 'sourceKey is required for idempotent person-life state evidence.' using errcode = '22023';
  end if;

  v_event_kind := btrim(coalesce(p_payload->>'eventKind',''));
  if v_event_kind = '' then
    raise exception 'eventKind is required.' using errcode = '22023';
  end if;
  ...
  if v_event_id is not null then
    if v_existing_definition_id is distinct from p_definition_id
      ...
      raise exception 'sourceKey retry does not match the existing person-life state event.' using errcode = '23505';
    end if;
  ...
  if v_event_kind = 'goal_evaluation' then
    if v_signal_kind <> 'goal' then
      raise exception 'goal_evaluation requires a Goal definition.' using errcode = '22023';
    end if;

    v_requirement_results := coalesce(p_payload->'requirementResults','[]'::jsonb);
    if jsonb_typeof(v_requirement_results) <> 'array' then
      raise exception 'requirementResults must be an array.' using errcode = '22023';
    end if;
    v_occurred_at := coalesce(nullif(p_payload->>'observedAt','')::timestamptz, now());
    v_evaluation := atlas.evaluate_life_goal_state_v1(v_packet, v_requirement_results);
    v_evidence := jsonb_build_object(
      'requirementResults', v_requirement_results,
      'source', coalesce(p_payload->'evidence','{}'::jsonb)
    );

  elsif v_event_kind = 'rhythm_satisfaction' then
    if v_signal_kind <> 'rhythm' then
      raise exception 'rhythm_satisfaction requires a Rhythm definition.' using errcode = '22023';
    end if;
    if nullif(p_payload->>'satisfiedAt','') is null then
      raise exception 'satisfiedAt is required for rhythm satisfaction evidence.' using errcode = '22023';
    end if;

    v_occurred_at := (p_payload->>'satisfiedAt')::timestamptz;
    v_evaluation := atlas.evaluate_life_lease_rhythm_v1(
      v_packet,
      v_occurred_at,
      coalesce(nullif(p_payload->>'asOf','')::timestamptz, now())
    );
    v_evidence := jsonb_build_object(
      'satisfiedAt', v_occurred_at,
      'source', coalesce(p_payload->'evidence','{}'::jsonb)
    );

  elsif v_event_kind = 'rhythm_evaluation' then
    if v_signal_kind <> 'rhythm' then
      raise exception 'rhythm_evaluation requires a Rhythm definition.' using errcode = '22023';
    end if;

    v_occurred_at := coalesce(nullif(p_payload->>'asOf','')::timestamptz, now());
    select max(e.occurred_at)
    into v_last_satisfied_at
    from atlas.person_life_state_events e
    where e.definition_id = p_definition_id
    ...
    if v_signal_kind <> 'consequence' then
      raise exception 'consequence_evaluation requires a Consequence definition.' using errcode = '22023';
    end if;
    ...
    if v_signal_kind <> 'consequence' then
      raise exception 'consequence_resolution requires a Consequence definition.' using errcode = '22023';
    end if;

    v_stable_key := btrim(coalesce(p_payload->>'stableKey',''));
    if v_stable_key = '' then
      raise exception 'stableKey is required for explicit consequence resolution.' using errcode = '22023';
    end if;
    ...
    if not exists (
      ...
      where i.definition_id = p_definition_id
        ...
        and i.stable_key = v_stable_key
        and i.status = 'open'
    ) then
      raise exception 'Open consequence instance not found for this user and stableKey.' using errcode = '22023';
    end if;
  ...
  if v_event_kind = 'consequence_evaluation' then
    for v_consequence in
      select value from jsonb_array_elements(coalesce(v_evaluation->'openConsequences','[]'::jsonb))
    loop
      ...
      on conflict (definition_id, stable_key)
      do update set
    ...
    set
      status = 'resolved',
      resolved_by_event_id = v_event_id,
...
comment on function atlas.record_person_life_state_api_v1(uuid,jsonb) is
  'Authenticated first-party evidence/evaluation writer for person-owned Goal, explicit lease Rhythm, and State Consequence state. Consequence absence never auto-resolves an instance; resolution requires explicit evidence.';
...
begin
      ...
      jsonb_build_object(
        ...
        'relations', coalesce((
          select jsonb_agg(jsonb_build_object(
            ...
            'relationBasis',r.relation_basis,
            'relationStatus',r.relation_status,
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'consequenceRole',i.consequence_role,
    ...
    'placementState',i.placement_state,
    'executionReadiness',i.execution_readiness,
    'actionSpec',i.action_spec,
    'evidence',i.evidence,
    'status',i.status,
...
comment on function atlas.person_life_state_api_v1() is
  'First-party read membrane for person-owned Life definitions, latest evidence/evaluation state, neutral relations, and explicit State Consequence instances.';
...
values
  (
    ...
    true,
    false,
    'First-party person-owned Goal/Rhythm/State Consequence definition writer. Custody fixed to auth.uid(); no task, Clock, practitioner, farm, or organization authority.',
    now()
  ),
  (
    ...
    true,
    false,
    'First-party person-life evidence/evaluation writer. Reuses pure shared reducers and requires explicit consequence resolution evidence.',
    now()
  ),
  (
    ...
    true,
    false,
    'First-party read membrane for person-owned life definitions, evidence/evaluation state, relations, and consequence instances.',
    now()
  )

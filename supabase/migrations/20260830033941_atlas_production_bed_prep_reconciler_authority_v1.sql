...
begin
  ...
  loop
    ...
    v_next:=atlas.author_production_work_occurrence_v1(
      ...
      'Weed, clear, prepare, and confirm water access for '||v_assignment.quantity_assigned::text||' bed-feet.',v_metadata,
      jsonb_build_object(
    ...
    if v_occurrence_id is not null then v_authored:=v_authored+1; end if;
    ...
    set metadata=coalesce(metadata,'{}'::jsonb)||jsonb_strip_nulls(jsonb_build_object(
      'bed_preparation_occurrence_id',v_occurrence_id,'bed_preparation_task_id',v_task_id,'bed_preparation_synced_at',now()
    )),updated_at=now()

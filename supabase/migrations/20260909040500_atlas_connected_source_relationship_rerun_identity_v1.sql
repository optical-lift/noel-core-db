...
begin
  ...
  -- A transaction-scoped provider identifier is not evidence that two different
  -- transactions belong to the same person. It is only a stable rerun key for the
  -- same source transaction when stronger party identifiers are missing or ambiguous.
  ...
  if v_source.provider_key = 'stripe' and v_checkout_session_id is not null then
    v_identifiers := v_identifiers || jsonb_build_array(jsonb_build_object(
      ...
      'value',v_checkout_session_id,
      'normalized',v_checkout_session_id
    ));
  end if;
  ...
  for v_identifier in select value from jsonb_array_elements(v_identifiers)
  loop
    ...
    if v_type is not null and v_value is not null and v_normalized is not null then
      insert into atlas.identity_subject_external_identifiers(
        ...
        v_normalized, true, case when v_type='checkout_session_id' then 9 else 5 end,
        jsonb_build_object('source', 'connected_source_commercial_interpretation', 'connectedSourceId', v_source.id,
          'transactionScoped', v_type='checkout_session_id')

...
declare
  ...
  profile_id uuid; bed1 uuid; bed2 uuid; bed3 uuid; v_run_id uuid; token text;
  today date:=(now() at time zone 'America/Chicago')::date;
  program_id uuid; lot_id uuid; requirement_id uuid; a1 uuid; a2 uuid; a3 uuid;
  r1 jsonb; r2 jsonb; ids1 jsonb; ids2 jsonb; occurrence_count integer; duplicate_count integer; early_task_count integer; linked_count integer;
...
begin
  ...
  if s.id is null or f.id is null or not atlas.is_system_fixture_farm_v1(f.id) then raise exception 'Reference bed-prep fixture unavailable.' using errcode='23514'; end if;
  ...
  if profile_id is null or bed1 is null or bed2 is null or bed3 is null then raise exception 'Reference bed-prep fixture objects incomplete.' using errcode='23514'; end if;
  ...
  values(s.id,f.id,token,'transactional','running',p_source_revision,jsonb_build_object('fixtureVersion',s.fixture_version,'systemFixture',true,'runner','run_reference_company_production_bed_preparation_sync_v1')) returning id into v_run_id;
  begin
    ...
    values(f.id,'reference_bed_prep_sync_'||replace(v_run_id::text,'-',''),extract(year from today)::int,'Reference generic bed-prep sync','reference_fixture','Prove generic capacity assignments author governed preparation work.',array['conformance'],'active',jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id,'synthetic_truth',true)) returning id into program_id;
    ...
    values(f.id,program_id,profile_id,'reference_bed_prep_lot_'||replace(v_run_id::text,'-',''),'Reference Generic Production Lot',1,720,'seedlings',720,'seedlings','transplant_ready','active',today+5,today+8,array['conformance'],jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id,'synthetic_truth',true)) returning id into lot_id;
    ...
    values(f.id,lot_id,'reference_bed_prep_req','transplant','bed_feet',60,'bed_ft',today+5,today+5,today+8,today+3,'confirmed','reference_company_runner',jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id,'synthetic_truth',true)) returning id into requirement_id;
    insert into atlas.production_bed_assignments(farm_id,production_lot_id,requirement_id,object_id,quantity_assigned,unit,planned_transplant_date,assignment_status,source,metadata) values
      (f.id,lot_id,requirement_id,bed1,20,'bed_ft',today+5,'assigned','reference_company_runner',jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id,'synthetic_truth',true)),
      (f.id,lot_id,requirement_id,bed2,20,'bed_ft',today+5,'assigned','reference_company_runner',jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id,'synthetic_truth',true)),
      (f.id,lot_id,requirement_id,bed3,20,'bed_ft',today+5,'assigned','reference_company_runner',jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id,'synthetic_truth',true));
    ...
    r1:=atlas.sync_production_bed_preparation_tasks_v1(program_id);
    select coalesce(jsonb_agg(id order by source_id),'[]'::jsonb) into ids1 from atlas.planned_work_occurrences where farm_id=f.id and source_kind='production_bed_assignment' and source_id in (a1,a2,a3) and state<>'cancelled';
    r2:=atlas.sync_production_bed_preparation_tasks_v1(program_id);
    select coalesce(jsonb_agg(id order by source_id),'[]'::jsonb) into ids2 from atlas.planned_work_occurrences where farm_id=f.id and source_kind='production_bed_assignment' and source_id in (a1,a2,a3) and state<>'cancelled';
    ...
    select count(*) into duplicate_count from (select occurrence_key from atlas.planned_work_occurrences where farm_id=f.id and source_kind='production_bed_assignment' and source_id in (a1,a2,a3) group by occurrence_key having count(*)>1)d;
    ...
    select count(*) into linked_count from atlas.production_bed_assignments where id in (a1,a2,a3) and nullif(metadata->>'bed_preparation_occurrence_id','') is not null;
    assertions:=jsonb_build_array(
      jsonb_build_object('key','one_occurrence_per_assignment','passed',occurrence_count=3 and linked_count=3,'expected',jsonb_build_object('occurrences',3,'linkedAssignments',3),'actual',jsonb_build_object('occurrences',occurrence_count,'linkedAssignments',linked_count)),
      jsonb_build_object('key','standalone_sync_has_reconciler_authority','passed',coalesce((r1->>'occurrencesAuthored')::int,0)=3 and jsonb_array_length(ids1)=3,'expected',3,'actual',jsonb_build_object('authored',r1->>'occurrencesAuthored','ids',ids1)),
      jsonb_build_object('key','rerun_creates_no_duplicates','passed',ids1=ids2 and duplicate_count=0,'expected',jsonb_build_object('stableIds',true,'duplicates',0),'actual',jsonb_build_object('stableIds',ids1=ids2,'duplicates',duplicate_count,'secondResult',r2)),
      jsonb_build_object('key','future_prep_does_not_materialize_early','passed',early_task_count=0,'expected',0,'actual',early_task_count)
    );
  ...
  if not fixture_completed then update atlas.reference_company_runs set status='failed',completed_at=now(),error_text=err,result=jsonb_build_object('fixtureRolledBack',true,'runnerError',err) where id=v_run_id; return jsonb_build_object('runId',v_run_id,'scenarioKey','production_bed_preparation_sync_v1','status','failed','error',err,'fixtureRolledBack',true); end if;
  ...
  return jsonb_build_object('contractVersion','run_reference_company_production_bed_preparation_sync_v1','runId',v_run_id,'scenarioKey','production_bed_preparation_sync_v1','status',case when all_passed then 'passed' else 'failed' end,'fixtureRolledBack',true,'assertions',assertions);
end;$function$;
...
declare
  ...
  today date:=(now() at time zone 'America/Chicago')::date; cycle_id uuid; batch_id uuid; observation_id uuid;
...
begin
  ...
  begin
    ...
    values(f.id,today,'reference-harvest-bucket:'||v_run_id::text,jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id,'synthetic_truth',true)) returning id into batch_id;
    ...
    values(f.id,batch_id,cycle_id,today,'half',0.5,1,'reference-harvest-bucket-observation:'||v_run_id::text,jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id,'synthetic_truth',true,'quantityExactness','bucket_only')) returning id into observation_id;

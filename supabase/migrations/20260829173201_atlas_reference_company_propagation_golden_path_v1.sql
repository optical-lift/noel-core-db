...
begin
  ...
  begin
    ...
    insert into atlas.crop_cycles(
      farm_id,crop_profile_id,crop_cycle_key,crop_label,variety,cycle_state,lifecycle_status,
      sown_date,germination_checked_date,coverage_kind,coverage_amount,coverage_unit,note,metadata
    ) values(
      v_farm.id,v_profile_id,'reference_cycle_'||replace(v_run_id::text,'-',''),
    ...
    insert into atlas.production_tray_batches(
      ...
      germinated_date,viable_seedlings,current_quantity,current_unit,idempotency_key,metadata
    ) values(
      ...
      v_hardening_date-50,720,720,'seedlings','reference-tray:'||v_run_id::text,
...
revoke all on function atlas.run_reference_company_scenario_v1(text,text) from public,anon,authenticated;
grant execute on function atlas.run_reference_company_scenario_v1(text,text) to service_role;

update atlas.reference_company_scenarios
set metadata=metadata||jsonb_build_object('implementation_state','executable','runner','run_reference_company_scenario_v1'),updated_at=now()
where stable_key='propagation_golden_path_v1';

comment on function atlas.run_reference_company_scenario_v1(text,text) is 'Runs an Atlas Reference Company conformance scenario against real Atlas tables/functions inside a rollback subtransaction, then persists only the assertion ledger. Synthetic fixture truth must never be interpreted as customer operating truth.';
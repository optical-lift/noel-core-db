...
-- Qualifies and selects the exact Eggeling witness passage without admitting text or source surfaces.
...
(
  ...
  'Internet Archive','https://archive.org/details/satapathabrhma01eggeuoft',
...
(
  ...
  'Internet Sacred Text Archive','https://www.sacred-texts.com/hin/sbr/sbe12/sbe1212.htm',
  null,now(),
  ...
  jsonb_build_object('translation_year',1882,'translator','Julius Eggeling',
                     'section','I.3.1','role','comparison_transcription','printed_page_span','67-71')
)
on conflict(canonical_key) do nothing;

do $$
declare
  v_case uuid; v_work uuid; v_expression uuid; v_manifestation uuid; v_item uuid; v_surrogate uuid;
  v_ia uuid; v_meta uuid; v_ol uuid; v_compare uuid; v_usco uuid;
  v_ssa uuid; v_ssm uuid; v_rights uuid; v_component uuid;
  v_audit_open uuid; v_audit uuid; v_finding uuid; v_condition uuid; v_obs uuid;
  v_brief uuid; v_mode uuid; v_decision uuid;
  v_primary_target uuid; v_expression_target uuid; v_package uuid; v_root uuid; v_section uuid; v_leaf uuid;
begin
  ...
  select id into strict v_ia from wnph.evidence_sources where canonical_key='internetarchive:satapathabrhma01eggeuoft';
  select id into strict v_meta from wnph.evidence_sources where canonical_key='internetarchive:satapathabrhma01eggeuoft:verified-item-metadata';
  select id into strict v_ol from wnph.evidence_sources where canonical_key='openlibrary:OL6283265M';
  select id into strict v_compare from wnph.evidence_sources where canonical_key='sacred-texts:sbe12:1-3-1';
  ...
  values(v_meta,v_surrogate,'high','Direct IA metadata identifies the digitization and reported 552-page extent; exact scan-image mapping for printed pp. 67-71 remains unresolved.');

  insert into wnph.source_sufficiency_assessments(recovery_case_id,result,confidence,rationale,recorded_by)
  values(v_case,'sufficient','high',
    ...
    'wnph:satapatha-vessel-cleansing-qualification-v1') returning id into v_ssa;
  ...
  values(v_ssa,v_surrogate,'primary','substantially_complete','usable','sufficient','usable',
    'Exact scan-image IDs for printed pp. 67-71 are not yet fixed.',
    'Sufficient to continue governed recovery; not sufficient to bypass page-image verification.') returning id into v_ssm;
  ...
  values(v_case,'US','cleared','high',
    'The underlying Śatapatha Brāhmaṇa is ancient. Eggeling''s English translation and this Clarendon Press Part I publication date to 1882; Eggeling died in 1918. The historical Work and translation/publication are public domain in the United States. Internet Archive also marks the governed item NOT_IN_COPYRIGHT for the U.S.',
    'wnph:satapatha-vessel-cleansing-qualification-v1') returning id into v_rights;
  insert into wnph.rights_components(determination_id,component_type,component_status,work_id,use_scope,rationale)
  values(v_rights,'underlying_work','public_domain',v_work,'U.S. source-faithful recovery and derived scholarly/semantic editions','Ancient underlying Work.') returning id into v_component;
  ...
  values(v_rights,'prior_translation','public_domain',v_expression,'U.S. recovery and publication of Eggeling''s 1882 English translation Expression','Published 1882; Eggeling died 1918.') returning id into v_component;
  insert into wnph.evidence_links(source_id,rights_component_id,confidence,note)
  ...
  values(v_rights,'source_images','reuse_permitted',v_surrogate,'Use of IA page images as recovery/verification inputs',
  ...
  values(v_case,'in_progress','Bounded audit of existing Eggeling 1882 Part I facsimile and transcription access; no meaningful-gap claim yet.')
  ...
  values(v_meta,v_leaf,'high','Direct repository evidence supports SOURCE_SUFFICIENT.');
  insert into wnph.recovery_case_events(recovery_case_id,prior_event_id,from_state,to_state,event_kind,rationale,recorded_by)
  values(v_case,v_leaf,'SOURCE_SUFFICIENT','RIGHTS_RESEARCH','state_transition','Evaluate component-specific U.S. rights for the bounded recovery inputs.','wnph:satapatha-vessel-cleansing-qualification-v1') returning id into v_leaf;
  insert into wnph.recovery_case_events(recovery_case_id,prior_event_id,from_state,to_state,event_kind,rationale,recorded_by)
  values(v_case,v_leaf,'RIGHTS_RESEARCH','RIGHTS_CLEARED','state_transition','U.S. rights are cleared for the bounded recovery inputs.','wnph:satapatha-vessel-cleansing-qualification-v1') returning id into v_leaf;
  insert into wnph.evidence_links(source_id,recovery_case_event_id,confidence,note)
  ...
  values(v_case,v_leaf,'RIGHTS_CLEARED','RECOVERY_AUDIT','state_transition',
    'Begin governed comparison of surviving facsimile and public transcription. Existing access does not itself settle WNPH recovery value.',
    'wnph:satapatha-vessel-cleansing-qualification-v1') returning id into v_leaf;

  insert into wnph.existing_recovery_audits(recovery_case_id,audit_status,scope_note,supersedes_audit_id)
  values(v_case,'complete',
    'Internet Archive already supplies the 1882 facsimile/OCR, and Sacred Texts supplies readable HTML. The remaining WNPH gap is not access: it is an exact page-image-verified, source-linked passage object whose later functional semantics remain distinct from the historical wording.',v_audit_open)
  ...
  values(v_audit,'internet_archive','present','FACSIMILE','raw','satapathabrhma01eggeuoft','Facsimile, PDF and OCR/full-text channels exist; WNPH source-surface verification remains open.') returning id into v_finding;
  ...
  insert into wnph.existing_recovery_findings(audit_id,channel,availability_status,manifestation_type,competence_state,competing_identifier,notes)
  values(v_audit,'other','present','SEARCHABLE_TEXT','competent','Internet Sacred Text Archive SBE12 I.3.1','Readable HTML of Eggeling''s translation exists and preserves paragraph/page cues; comparison only.') returning id into v_finding;
  insert into wnph.evidence_links(source_id,existing_recovery_finding_id,confidence,note)
  ...
  values(v_case,'bounded_complete','Condition of the exact Eggeling 1882 I.3.1.1-11 witness, corrected to printed pp. 67-71 and kept distinct from the ancient Sanskrit/Mādhyandina Work.','high') returning id into v_condition;
  insert into wnph.recovery_condition_observations(
    assessment_id,condition_type_id,condition_state,epistemic_status,expression_id,item_id,surrogate_id,observation_text,confidence)
  select v_condition,t.id,x.state,x.epistemic,
         ...
         x.text,x.confidence
  from (values
    ('identified_surviving_witness'::text,'adequate'::text,'evidence'::text,'item'::text,'Specific Robarts/University of Toronto copy identified as IA satapathabrhma01eggeuoft, call number AAB-1369.'::text,'high'::text),
    ('digital_facsimile_survival','adequate','evidence','surrogate','The 1882 Part I witness survives digitally with page-image/PDF/OCR access.','high'),
    ('modern_reading_edition','adequate','evidence','expression','A readable public HTML transcription exists; ordinary reading access is not the gap.','high'),
    ('text_integrity','unverified','evidence','surrogate','Exact IA scan-image identifiers for printed pp. 67-71 are not yet fixed and the passage has not yet been verified image-by-image in WNPH.','high'),
    ('edition_relationship_clarity','adequate','interpretation','expression','The recoverable object is Eggeling''s 1882 English Expression, not the Sanskrit Work itself.','high'),
    ('modern_recovery_adequacy','limited','interpretation','expression','Generic access is adequate, but WNPH lacks source-linked page verification plus separately governed function-first semantics for this passage.','high')
  ...
  select case when t.canonical_key='modern_reading_edition' then v_compare else v_meta end,
  ...
  values(v_case,
    ...
    'verified_1882_translation_witness_passage','high') returning id into v_brief;
  for v_mode in
    select m.id from wnph.recovery_case_modes m
    where m.recovery_case_id=v_case and m.recovery_mode in ('text','transcription','witness')
      and m.intent_status='proposed' and not exists(select 1 from wnph.recovery_case_modes n where n.supersedes_mode_id=m.id)
  loop
    ...
    select recovery_case_id,recovery_mode,'committed',
      case recovery_mode
        ...
        else 'Preserve the 1882 translation witness distinctly from Sanskrit and later English witnesses.' end,id
    from wnph.recovery_case_modes where id=v_mode;
  end loop;

  insert into wnph.recovery_case_events(recovery_case_id,prior_event_id,from_state,to_state,event_kind,rationale,recorded_by)
  values(v_case,v_leaf,'CONDITION_ASSESSED','DECISION_REVIEW','state_transition','Review the narrow source-linked recovery value, not generic ebook availability.','wnph:satapatha-vessel-cleansing-qualification-v1') returning id into v_leaf;
  insert into wnph.recovery_decisions(recovery_case_id,decision_outcome,decision_scope,decision_summary)
  values(v_case,'qualify',
  ...
  select o.id into strict v_obs from wnph.recovery_condition_observations o
  join wnph.recovery_condition_types t on t.id=o.condition_type_id
  ...
  select v_decision,'mode',m.id from wnph.recovery_case_modes m
  where m.recovery_case_id=v_case and m.intent_status='committed' and m.recovery_mode in ('text','transcription','witness')
    and not exists(select 1 from wnph.recovery_case_modes n where n.supersedes_mode_id=m.id);

  select t.id into strict v_primary_target from wnph.recovery_case_targets t
  where t.recovery_case_id=v_case and t.target_role='primary_source' and t.surrogate_id=v_surrogate
  ...
  select t.id into strict v_expression_target from wnph.recovery_case_targets t
  where t.recovery_case_id=v_case and t.target_role='candidate' and t.expression_id=v_expression
    and not exists(select 1 from wnph.recovery_case_targets n where n.supersedes_target_id=t.id);
  insert into wnph.recovery_decision_plan_members(recovery_decision_id,member_role,recovery_case_target_id)
  values(v_decision,'source_target',v_expression_target);

  insert into wnph.recovery_case_events(recovery_case_id,prior_event_id,from_state,to_state,event_kind,rationale,recorded_by)
  values(v_case,v_leaf,'DECISION_REVIEW','QUALIFIED','decision','Qualify the exact Eggeling 1882 pp. 67-71 witness passage for source-linked recovery.','wnph:satapatha-vessel-cleansing-qualification-v1') returning id into v_leaf;

  insert into wnph.recovery_case_targets(recovery_case_id,target_role,surrogate_id,rationale)
  values(v_case,'preferred_source',v_surrogate,'Preferred historical source for this qualified passage. Exact IA source-surface IDs for printed pp. 67-71 remain a blocking verification task.');
  ...
  values(
    'satapatha-brahmana:eggeling-1882-part1-i-3-1-canonical-source:v1',v_case,v_expression,v_decision,
    'canonical_master','semantic_single_source','1','planned',
    jsonb_build_object('single_source_publishing',true,'manifestation_agnostic',true,
      'expression_identity','Eggeling 1882 English translation, Part I','printed_page_span','67-71',
      'source_surface_assets_required_before_text_admission',true,
      'functional_semantics_separate_from_source_text',true,'ancient_work_may_not_be_collapsed_into_translation',true,
      'supported_by_design',jsonb_build_array('responsive_web','reflowable_epub','print_pdf','paperback','future_output_families')),
    'Structure-only canonical source package. No OCR, modern web transcription or remembered quotation is canonical text until exact IA source surfaces for printed pp. 67-71 are fixed and verified.') returning id into v_package;
  ...
  values(v_case,v_leaf,'QUALIFIED','SELECTED_FOR_RECOVERY','selection','Select the exact Eggeling 1882 I.3.1.1-11 witness and its source package. Text remains blocked on source-image verification.','wnph:satapatha-vessel-cleansing-qualification-v1',true) returning id into v_leaf;
  ...
  values(v_package,'satapatha-eggeling1882:i-3-1:document',0,'document','book_root',
    jsonb_build_object('translation_year',1882,'part','I','books','I-II'),
    jsonb_build_object('structure_authority','governed_1882_witness','source_surrogate_key','satapatha-brahmana:ia-1882-part1-surrogate')) returning id into v_root;
  ...
  values(v_package,'satapatha-eggeling1882:i-3-1:section',v_root,1,'section','cleaning_of_spoons',
    jsonb_build_object('canonical_section_locator','I.3.1','printed_page_start',67,'printed_page_end',71,
      'paragraph_start',1,'paragraph_end',11,'source_surface_mapping_complete',false),
    jsonb_build_object('structure_authority','1882_contents_and_printed_page_headers','comparison_transcription_key','sacred-texts:sbe12:1-3-1')) returning id into v_section;
  insert into wnph.publication_source_blocks(source_package_id,block_key,parent_block_id,ordinal,block_type,semantic_role,properties,source_provenance)
  values(v_package,'satapatha-eggeling1882:i-3-1:reading-stream',v_section,1,'reading_stream','witness_passage',
    jsonb_build_object('printed_page_span','67-71','paragraph_span','1-11','completion',false,

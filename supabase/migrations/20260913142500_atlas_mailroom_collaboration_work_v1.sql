...
--   communication evidence != personal attention != response responsibility != derived work
--   one conversation may contain many message participants and create many work items
--   handing off a conversation does not hand off work derived from it
...
create table atlas.communication_derived_work_links (
  ...
  constraint communication_derived_work_links_creator_org_fk
    foreign key (organization_id, created_by_membership_id)
    references atlas.organization_memberships(organization_id, id) on delete restrict,
  constraint communication_derived_work_links_work_org_fk
    foreign key (organization_id, work_item_id)
    references atlas.work_items(organization_id, id) on delete restrict,
  constraint communication_derived_work_links_metadata_object_check
    check (jsonb_typeof(metadata) = 'object'),
  constraint communication_derived_work_links_idempotency_nonblank_check
    check (btrim(idempotency_key) <> ''),
  constraint communication_derived_work_links_excerpt_bounds_check
    check (
...
create unique index communication_derived_work_links_org_idempotency_uq
  on atlas.communication_derived_work_links(organization_id, idempotency_key);

create unique index communication_derived_work_links_work_uq
  on atlas.communication_derived_work_links(work_item_id);

create index communication_derived_work_links_conversation_idx
  on atlas.communication_derived_work_links(institutional_conversation_id, created_at);

create index communication_derived_work_links_event_idx
...
declare
  ...
  v_excerpt_pos integer;
  v_excerpt_start integer;
  v_excerpt_end integer;
  v_excerpt_sha text;
  v_assignment jsonb := null;
  v_time_contract_id uuid := null;
begin
  if auth.uid() is null then
    raise exception 'Sign in required.' using errcode='42501';
  end if;
  if p_institutional_conversation_id is null or p_communication_event_id is null then
    raise exception 'Conversation and message are required.' using errcode='22023';
  end if;
  if nullif(btrim(coalesce(p_title,'')), '') is null then
    raise exception 'Task title is required.' using errcode='22023';
  end if;
  if length(btrim(p_title)) > 220 then
    raise exception 'Task title must be 220 characters or fewer.' using errcode='22023';
  end if;
  ...
  where m.institutional_conversation_id = v_conv.id
    and m.communication_event_id = p_communication_event_id
  limit 1;
  if v_event.id is null then
    raise exception 'Message does not belong to this conversation.' using errcode='22023';
  end if;
  ...
  if v_actor.id is null
     or not atlas.communication_endpoint_membership_has_capability_v1(v_endpoint_id, v_actor.id, 'view') then
    raise exception 'Communication endpoint view authority required.' using errcode='42501';
  end if;

  select * into v_existing
  from atlas.communication_derived_work_links
  where organization_id = v_conv.organization_id
    and idempotency_key = btrim(p_idempotency_key)
  limit 1;
  if v_existing.id is not null then
    select * into v_existing_work from atlas.work_items where id=v_existing.work_item_id;
    return jsonb_build_object(
      'contractVersion','communication_derived_work_create_v1',
      'deduplicated',true,
      'linkId',v_existing.id,
      'workItemId',v_existing.work_item_id,
      'title',v_existing_work.title,
      'workState',v_existing_work.work_state
    );
  end if;

  if v_excerpt is not null then
    v_excerpt_pos := strpos(coalesce(v_event.body,''), v_excerpt);
    if v_excerpt_pos <= 0 then
      raise exception 'Selected text is not present in the canonical message body.' using errcode='22023';
    end if;
    v_excerpt_start := v_excerpt_pos - 1;
    v_excerpt_end := v_excerpt_start + char_length(v_excerpt);
    v_excerpt_sha := encode(extensions.digest(v_event.id::text || ':' || v_excerpt, 'sha256'), 'hex');
  end if;
  ...
  if p_assignee_membership_id is not null then
    ...
    if v_assignee.id is null then
      raise exception 'Choose an active organization member.' using errcode='22023';
    end if;
  ...
  insert into atlas.work_items(
    ...
    created_by_user_id, metadata, stable_key
  ) values (
    v_work_id,
    v_conv.organization_id,
    v_conv.organization_unit_id,
    btrim(p_title),
    ...
    auth.uid(),
    jsonb_build_object(
      'contractVersion','communication_derived_work_v1',
      'institutionalConversationId',v_conv.id,
      'communicationEventId',v_event.id,
      'source','mailroom_selection'
    ),
    'communication-derived-work:' || v_link_id::text
  );

  insert into atlas.communication_derived_work_links(
    id, organization_id, institutional_conversation_id, communication_event_id,
    work_item_id, created_by_membership_id, evidence_excerpt, evidence_start,
    evidence_end, evidence_sha256, idempotency_key, metadata
  ) values (
    v_link_id,
    v_conv.organization_id,
    v_conv.id,
    ...
    jsonb_build_object('source','create_communication_derived_work_self_api_v1')
  );

  if p_due_at is not null or p_expected_duration_minutes is not null then
    insert into atlas.work_time_contracts(
      organization_id, work_item_id, contract_state,
      latest_lawful_at, hard_finish_at, expected_duration_minutes,
      movement_policy, consequence_of_delay, source_kind, source_id,
      source_confidence, metadata
    ) values (
      v_conv.organization_id,
      v_work_id,
      'active',
      ...
      case when p_due_at is null then 'unplaced' else 'bounded' end,
      '{}'::jsonb,
      'communication_derived_work',
      v_link_id,
      1,
  ...
  if v_assignee.id is not null then
    v_assignment := atlas.set_company_work_responsibility_internal_v1(
      ...
      v_assignee.id,
      v_actor.id,
      'communication_derived_work_assignment',
      jsonb_build_object(
        'source','create_communication_derived_work_self_api_v1',
        'institutionalConversationId',v_conv.id,
        'communicationEventId',v_event.id,
        'derivedWorkLinkId',v_link_id
      )
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'title',btrim(p_title),
    'workState','open',
    'responsibleMembershipId',v_assignee.id,
    'timeContractId',v_time_contract_id,
    'assignment',v_assignment,
    'evidence',case when v_excerpt is null then null else jsonb_build_object(
      'excerpt',v_excerpt,
      'start',v_excerpt_start,
      'end',v_excerpt_end,
      'sha256',v_excerpt_sha
    ) end
  );
end;
$$;
...
begin
  ...
  select * into v_actor
  from atlas.organization_memberships
  where organization_id=v_link.organization_id and user_id=auth.uid() and active
  ...
  where work_item_id=p_work_item_id and allocation_role='responsible' and state='active'
  limit 1;
  ...
  v_assignment := atlas.set_company_work_responsibility_internal_v1(
    ...
    v_target.id,
    v_actor.id,
    coalesce(nullif(btrim(p_reason),''),'communication_derived_work_handoff'),
    jsonb_build_object(
      'source','handoff_communication_derived_work_self_api_v1',
      'derivedWorkLinkId',v_link.id,
      'institutionalConversationId',v_link.institutional_conversation_id
    )
  );

  return jsonb_build_object(
    'contractVersion','communication_derived_work_handoff_v1',
...
begin
  ...
  if v_binding.id is null then
    raise exception 'Claim the conversation before adding collaborators.' using errcode='22023';
  end if;
  ...
  limit 1;
  if v_current.assignee_membership_id is distinct from v_actor.id
     and not atlas.communication_endpoint_membership_has_capability_v1(v_endpoint_id,v_actor.id,'handoff') then
  ...
  where work_item_id=v_binding.work_item_id
    and assignee_membership_id=v_target.id
  ...
  if v_existing.id is not null then
    return jsonb_build_object(
      'contractVersion','institutional_conversation_collaborator_v1',
      'deduplicated',true,
  ...
  insert into atlas.work_allocations(
    ...
    v_conv.organization_id,v_binding.work_item_id,v_target.id,v_actor.id,
    p_allocation_role,'active',jsonb_build_object(
      'source','add_institutional_conversation_collaborator_self_api_v1',
      'institutionalConversationId',v_conv.id,
...
begin
  ...
  if v_target_allocation.id is null then
    return jsonb_build_object(
      'contractVersion','institutional_conversation_collaborator_remove_v1',
      'released',false,
      'workItemId',v_binding.work_item_id,
  ...
  set state='released',released_at=now(),release_reason='conversation_collaborator_removed',updated_at=now()
  where id=v_target_allocation.id;

  return jsonb_build_object(
    'contractVersion','institutional_conversation_collaborator_remove_v1',
    'released',true,
    'allocationId',v_target_allocation.id,
    'workItemId',v_binding.work_item_id,
    'membershipId',p_target_membership_id,
    'allocationRole',p_allocation_role
  );
...
create or replace function atlas.institutional_conversation_detail_self_v3(
  p_institutional_conversation_id uuid
)
returns jsonb
language plpgsql
...
begin
    ...
    select
      ...
      (
        select coalesce(jsonb_agg(jsonb_build_object(
          ...
          'address',p.address,
          'addressNormalized',p.address_normalized,
          'addressKind',p.address_kind,
          'isSelf',p.is_self,
          'transportDisplayName',nullif(p.metadata->>'displayName',''),
          'resolutionState',r.resolution_state,
          'externalRelationshipId',r.external_relationship_id,
          'identitySubjectId',er.subject_id,
          'resolvedDisplayName',isp.display_name,
          'resolvedSubjectKind',isp.subject_kind
        ) order by
          case p.participant_role when 'sender' then 0 when 'to' then 1 when 'cc' then 2 when 'bcc' then 3 else 4 end,
          p.address_normalized
        ),'[]'::jsonb)
        from atlas.communication_event_participants p
        left join atlas.communication_participant_relationship_resolutions r
          on r.communication_event_participant_id=p.id
        ...
        left join atlas.identity_subject_projections isp
          on isp.subject_id=er.subject_id and isp.organization_id=v_conv.organization_id
        where p.communication_event_id=e.id
      ) as participants,
      (select min(a.occurred_at) from atlas.communication_attention_events a
        where a.communication_event_id=e.id and a.attention_kind='opened') as first_opened_at,
      (select a.membership_id from atlas.communication_attention_events a
        where a.communication_event_id=e.id and a.attention_kind='opened'
        order by a.occurred_at,a.id limit 1) as first_opened_by_membership_id,
      coalesce((
        select case when a.attention_kind='marked_unread' then false else true end
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'address',q.address,
    'addressNormalized',q.address_normalized,
    'addressKind',q.address_kind,
    'isSelf',q.is_self,
    'displayName',q.display_name,
    'roles',to_jsonb(q.roles),
    'externalRelationshipId',q.external_relationship_id,
    'identitySubjectId',q.subject_id,
    'resolvedDisplayName',q.resolved_display_name,
    'resolvedSubjectKind',q.subject_kind,
    'lastSeenAt',q.last_seen_at
  ) order by q.is_self,q.resolved_display_name nulls last,q.display_name nulls last,q.address_normalized),'[]'::jsonb)
  into v_participants
  from (
    select
      p.address,
      p.address_normalized,
      p.address_kind,
      bool_or(p.is_self) as is_self,
      max(nullif(p.metadata->>'displayName','')) as display_name,
      array_agg(distinct p.participant_role order by p.participant_role) as roles,
      (array_agg(r.external_relationship_id) filter (where r.external_relationship_id is not null))[1] as external_relationship_id,
      (array_agg(er.subject_id) filter (where er.subject_id is not null))[1] as subject_id,
      max(isp.display_name) as resolved_display_name,
      max(isp.subject_kind) as subject_kind,
      max(e.occurred_at) as last_seen_at
    from atlas.institutional_conversation_messages m
    join atlas.communication_events e on e.id=m.communication_event_id
    join atlas.communication_event_participants p on p.communication_event_id=e.id
    left join atlas.communication_participant_relationship_resolutions r on r.communication_event_participant_id=p.id
    left join atlas.external_relationships er on er.id=r.external_relationship_id
    left join atlas.identity_subject_projections isp
      on isp.subject_id=er.subject_id and isp.organization_id=v_conv.organization_id
    where m.institutional_conversation_id=v_conv.id
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'allocationId',a.id,
    'membershipId',a.assignee_membership_id,
    'allocationRole',a.allocation_role,
    'displayName',coalesce(up.display_name,om.metadata->>'displayName',om.role),
    'role',om.role,
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'evidenceStart',l.evidence_start,
    'evidenceEnd',l.evidence_end,
    'createdAt',l.created_at,
    'createdByMembershipId',l.created_by_membership_id,
    'responsibleMembershipId',responsible.assignee_membership_id,
    'responsibleDisplayName',coalesce(resp_profile.display_name,resp_member.metadata->>'displayName',resp_member.role),
    'dueAt',tc.hard_finish_at,
    'expectedDurationMinutes',tc.expected_duration_minutes
  ...
  left join lateral (
    select a.* from atlas.work_allocations a
    where a.work_item_id=w.id and a.allocation_role='responsible' and a.state='active'
    order by a.allocated_at desc limit 1
  ) responsible on true
  left join atlas.organization_memberships resp_member on resp_member.id=responsible.assignee_membership_id
  left join atlas.user_profiles resp_profile on resp_profile.user_id=resp_member.user_id
  left join lateral (
    select t.* from atlas.work_time_contracts t
    where t.work_item_id=w.id and t.contract_state='active'
...
-- Public browser membranes. These add no business authority: the atlas functions
-- continue to enforce auth.uid(), organization membership, endpoint capability,
-- current responsibility, and target-member constraints.

create or replace function public.institutional_conversation_detail_self_v3(
  p_institutional_conversation_id uuid
)
returns jsonb
language sql
...
set search_path = pg_catalog, atlas
as $$
  select atlas.institutional_conversation_detail_self_v3(p_institutional_conversation_id);
$$;
...
set search_path = pg_catalog, atlas
as $$
  select atlas.create_communication_derived_work_self_api_v1(
    p_institutional_conversation_id,p_communication_event_id,p_excerpt,p_title,p_instructions,
    p_assignee_membership_id,p_due_at,p_expected_duration_minutes,p_idempotency_key
  );
$$;

create or replace function public.handoff_communication_derived_work_self_api_v1(
  p_work_item_id uuid,
  p_target_membership_id uuid,
  p_reason text default null
)
...
set search_path = pg_catalog, atlas
as $$
  select atlas.handoff_communication_derived_work_self_api_v1(p_work_item_id,p_target_membership_id,p_reason);
$$;

create or replace function public.add_institutional_conversation_collaborator_self_api_v1(
  p_institutional_conversation_id uuid,
  p_target_membership_id uuid,
  p_allocation_role text default 'participant'
)
...
set search_path = pg_catalog, atlas
as $$
  select atlas.add_institutional_conversation_collaborator_self_api_v1(p_institutional_conversation_id,p_target_membership_id,p_allocation_role);
$$;

create or replace function public.remove_institutional_conversation_collaborator_self_api_v1(
  p_institutional_conversation_id uuid,
  p_target_membership_id uuid,
  p_allocation_role text default 'participant'
)
...
set search_path = pg_catalog, atlas
as $$
  select atlas.remove_institutional_conversation_collaborator_self_api_v1(p_institutional_conversation_id,p_target_membership_id,p_allocation_role);
$$;
...
comment on table atlas.communication_derived_work_links is
  'Append-only provenance linking exact institutional communication evidence to independently governed Company Work.';
comment on function atlas.create_communication_derived_work_self_api_v1(uuid,uuid,text,text,text,uuid,timestamptz,integer,text) is
  'Creates Company Work from an exact institutional communication message or selected excerpt without changing conversation responsibility.';
comment on function atlas.institutional_conversation_detail_self_v3(uuid) is
  'Participant-aware Mailroom detail projection including event recipients, conversation roster, response collaborators, and communication-derived work.';

...
-- This consolidation migration is intentionally schema-standalone against the
-- current production clone. Repairs whose owning provider tables are still
-- unreleased are installed only when those tables exist. They therefore apply
-- in governed release order without pretending an unreleased dependency exists.

-- 1. social_reply currently means Facebook Messenger reply, never public comment.
...
for each row when (new.operation_kind='social_reply')
execute function atlas.guard_social_reply_transport_semantics_v1();

-- 2. Historical one-to-one Messenger continuity is participant based, matching
-- live webhooks. Provider conversation ids remain evidence, not Atlas thread authority.
...
-- 3. Webhook delivery identity and outbound-echo corroboration belong to the
-- provider webhook rail. Current production does not yet contain that rail, so
-- install this repair only when the owning table is present.
do $block$
begin
  if to_regclass('atlas.provider_webhook_deliveries') is null then
    raise notice 'provider webhook rail is not present; webhook audit repair deferred to governed release order';
    return;
  end if;

  execute 'alter table atlas.provider_webhook_deliveries drop constraint if exists provider_webhook_deliveries_provider_key_provider_delivery_key_key';
  if not exists (
    select 1 from pg_catalog.pg_constraint
    where conrelid='atlas.provider_webhook_deliveries'::regclass
      and conname='provider_webhook_deliveries_source_provider_delivery_key_key'
  ) then
    execute 'alter table atlas.provider_webhook_deliveries add constraint provider_webhook_deliveries_source_provider_delivery_key_key unique(connected_source_id,provider_key,provider_delivery_key)';
  end if;

  execute $ddl$
create or replace function atlas.record_provider_webhook_delivery_service_v1(
  p_connected_source_id uuid,p_provider_key text,p_provider_delivery_key text,
  p_payload_sha256 text,p_metadata jsonb default '{}'::jsonb
) returns jsonb
language plpgsql security definer set search_path=pg_catalog,atlas
as $function$
declare
  v_source atlas.connected_sources%rowtype;
...
begin
  ...
  select * into v_delivery from atlas.provider_webhook_deliveries
  where connected_source_id=v_source.id and provider_key=v_provider and provider_delivery_key=v_key for update;
  if found then
    if v_delivery.payload_sha256 is distinct from v_hash then
      update atlas.provider_webhook_deliveries set delivery_state='conflict',attempt_count=attempt_count+1,last_error='provider delivery key replayed for this source with a mismatched payload hash',updated_at=now() where id=v_delivery.id returning * into v_delivery;
...
$function$;
$ddl$;
  execute 'revoke all on function atlas.record_provider_webhook_delivery_service_v1(uuid,text,text,text,jsonb) from public,anon,authenticated';
  execute 'grant execute on function atlas.record_provider_webhook_delivery_service_v1(uuid,text,text,text,jsonb) to service_role';

  execute $ddl$
create or replace function atlas.ingest_provider_webhook_events_service_v1(
  p_provider_webhook_delivery_id uuid,p_events jsonb,p_manifest jsonb default '{}'::jsonb
) returns jsonb
language plpgsql security definer set search_path=pg_catalog,atlas
as $function$
declare
  v_delivery atlas.provider_webhook_deliveries%rowtype;
  v_source atlas.connected_sources%rowtype;
  v_receipt jsonb; v_manifest jsonb; v_event jsonb;
  v_existing atlas.communication_events%rowtype;
  v_events_to_ingest jsonb:='[]'::jsonb; v_corroborated jsonb:='[]'::jsonb;
  v_ref text; v_body text; v_thread text; v_existing_thread text;
begin
  ...
  for v_event in select value from jsonb_array_elements(p_events) loop
    ...
    if v_ref is not null then
      select * into v_existing from atlas.communication_events where connected_source_id=v_source.id and source_event_ref=v_ref;
    end if;
    if v_existing.id is not null and v_existing.direction='outgoing'
      ...
      if v_existing_thread is not distinct from v_thread then
        v_corroborated:=v_corroborated||jsonb_build_array(jsonb_build_object('communicationEventId',v_existing.id,'sourceEventRef',v_ref,'corroboration','provider_webhook_echo'));
        continue;
      end if;
    end if;
  ...
  v_manifest:=coalesce(p_manifest,'{}'::jsonb)||jsonb_build_object('providerWebhookDeliveryId',v_delivery.id,'corroboratedExistingEvents',v_corroborated);
  if jsonb_array_length(v_events_to_ingest)>0 then
    if v_source.custodian_user_id is not null then
      if to_regprocedure('atlas.ingest_principal_communication_events_service_v1(uuid,jsonb,jsonb)') is null then raise exception 'Principal provider communication ingest is unavailable.' using errcode='55000'; end if;
      execute 'select atlas.ingest_principal_communication_events_service_v1($1,$2,$3)' into v_receipt using v_source.id,v_events_to_ingest,v_manifest;
    elsif v_source.custodian_organization_id is not null then
    ...
    v_receipt:=jsonb_build_object('contractVersion','provider_webhook_corroboration_receipt_v1','eventsIngested',0,'corroboratedExistingEvents',v_corroborated);
  end if;
...
$function$;
$ddl$;
  execute 'revoke all on function atlas.ingest_provider_webhook_events_service_v1(uuid,jsonb,jsonb) from public,anon,authenticated';
  execute 'grant execute on function atlas.ingest_provider_webhook_events_service_v1(uuid,jsonb,jsonb) to service_role';
end;
$block$;

-- 4. Temporary provider-user authorization is a credential, not durable history.
-- The discovery tables are still unreleased, so install their cleanup lifecycle
-- only when the owning table exists in governed release order.
do $block$
begin
  if to_regclass('atlas.provider_asset_authorizations') is null
     or to_regclass('atlas.provider_asset_selections') is null then
    raise notice 'provider asset discovery is not present; temporary authorization cleanup deferred to governed release order';
    return;
  end if;

  execute 'alter table atlas.provider_asset_authorizations alter column vault_secret_id drop not null';
  execute $ddl$
create or replace function atlas.cleanup_provider_asset_authorization_secret_v1()
returns trigger
language plpgsql security definer set search_path=pg_catalog,atlas,vault
as $function$
declare v_secret_id uuid:=old.vault_secret_id;
begin
  if v_secret_id is null then return new; end if;
  update atlas.provider_asset_authorizations set vault_secret_id=null,updated_at=now() where id=new.id and vault_secret_id=v_secret_id;
  delete from vault.secrets where id=v_secret_id;
  return new;
end;
$function$;
$ddl$;
  execute 'drop trigger if exists provider_asset_authorization_secret_cleanup on atlas.provider_asset_authorizations';
  execute 'create trigger provider_asset_authorization_secret_cleanup after update of authorization_state on atlas.provider_asset_authorizations for each row when (old.authorization_state is distinct from new.authorization_state and new.authorization_state in (''expired'',''revoked'')) execute function atlas.cleanup_provider_asset_authorization_secret_v1()';

  execute $ddl$
create or replace function atlas.complete_provider_asset_selection_service_v1(
  p_provider_asset_selection_id uuid,p_required_credential_kind text,p_metadata jsonb default '{}'::jsonb
) returns jsonb
language plpgsql security definer set search_path=pg_catalog,atlas
as $function$
declare
...
begin
  ...
  if v_selection.selection_state='connected' then
    update atlas.provider_asset_authorizations set authorization_state='revoked',updated_at=now() where id=v_authorization_id and authorization_state='ready';
  ...
  update atlas.connected_sources set authorization_state='connected',revoked_at=null,metadata=metadata||coalesce(p_metadata,'{}'::jsonb),updated_at=now()
  where id=v_selection.connected_source_id and authorization_state in ('pending','reauthorization_required','error') returning * into v_source;
...
$function$;
$ddl$;
  execute 'revoke all on function atlas.complete_provider_asset_selection_service_v1(uuid,text,jsonb) from public,anon,authenticated';
  execute 'grant execute on function atlas.complete_provider_asset_selection_service_v1(uuid,text,jsonb) to service_role';
...
begin
  update atlas.provider_asset_authorizations set authorization_state='expired',updated_at=now() where authorization_state='ready' and expires_at<=now();
  get diagnostics v_count=row_count;
  return jsonb_build_object('contractVersion','provider_asset_authorization_cleanup_v1','expiredAuthorizations',v_count);
end;
$function$;
$ddl$;
  execute 'revoke all on function atlas.cleanup_expired_provider_asset_authorizations_service_v1() from public,anon,authenticated';
  execute 'grant execute on function atlas.cleanup_expired_provider_asset_authorizations_service_v1() to service_role';
end;
$block$;

-- 5. setup_actor is temporary implementation authority, never a permanent role.
...
as $function$
  select auth.uid() is not null and (
    ...
    or exists (
      ...
      where actor.organization_id=p_organization_id and actor.human_user_id=auth.uid()
        and actor.actor_kind='setup_actor' and actor.active and actor.ended_at is null
        and organization.status='active' and organization.onboarding_state<>'ready'
...
begin
  if new.state='ended' or new.ended_at is not null then
    ...
    where organization_id=new.organization_id and actor_kind='setup_actor' and active
...
for each row when (new.state='ended' or new.ended_at is not null)

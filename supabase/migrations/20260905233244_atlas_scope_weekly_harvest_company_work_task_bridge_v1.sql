...
begin
  ...
  -- This bridge belongs only to the weekly-Harvest compatibility adapter.
  -- Other Company Work domains must reconcile through their own source-owned seams.
  ...
  where a.task_id=new.id
    and a.adapter_kind='legacy_planned_occurrence_task'
    and wi.source_object_type='planned_work_occurrence'
    and wi.operation_class='harvest'
  ...
  if new.status='done' then
    ...
    set state='completed',completed_at=coalesce(completed_at,now()),retired_at=null,updated_at=now()
    where id=v_adapter.id;

    update atlas.work_planning_conflicts
    set state='resolved',resolution_kind='work_completed',
  ...
  if new.status in ('archived','skipped') and v_work.work_state='open' then
    perform atlas.retire_weekly_harvest_company_work_adapter_v1(
      v_adapter.planned_occurrence_id,new.id,

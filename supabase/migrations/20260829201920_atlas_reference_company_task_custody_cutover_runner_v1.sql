insert into atlas.reference_company_scenarios(
  ...
  'task_custody_cutover_v1','worker_day','Task custody survives elapsed Worker Day',
  'Prove that an unresolved overdue obligation preserves its true due date, returns only its elapsed Worker Day placement to Atlas, remains visible as overdue, and is not silently rescheduled.',
  'transactional',1,
  jsonb_build_object('systemFixtureFarm','atlas_reference_farm','requiresExistingAuthUser',true),
  jsonb_build_array(
    'task_due_date_is_immutable_under_elapsed_day_reconciliation',
    'unresolved_task_stays_open',
    'elapsed_placement_returns_to_atlas',
    'overdue_task_remains_visible_as_overdue',
    'generic_rollover_emits_no_reschedule_transition'
  ),true,
...
begin
  ...
  begin
    ...
    values(v_user_id,v_farm.id,'farm_hand','reference_custody_worker_'||replace(v_run_id::text,'-',''),true,
      jsonb_build_object('system_fixture',true,'reference_run_id',v_run_id))

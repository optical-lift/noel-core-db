...
insert into atlas.work_requirements (
  ...
  earliest_relevant_at,latest_satisfactory_at,consequence_of_delay,
  jurisdiction_key,metadata
)
select
  a.organization_id,a.farm_organization_unit_id,
  'legacy_task_adoption:' || a.legacy_task_id::text,
  'operational',a.legacy_title,'legacy_task',a.legacy_task_id,'active',
...
select
  t.organization_id,wr.id,wi.id,'resolves',true,
...
select
  a.organization_id,wi.organization_unit_id,wi.id,'legacy_task',a.legacy_task_id,'active',
  jsonb_build_object(
    'transitional',true,
    'currentCompanyWorkCanonicalization','v1',
...
select
  wi.organization_id,wi.id,'active',
...
select
  wi.organization_id,wi.id,om.id,null,'responsible','active',
  coalesce(t.released_at,t.created_at,now()),
  jsonb_build_object(
...
select
  wi.organization_id,wi.id,null,'no_eligible_assignee',now(),
  case
...
select
  wi.organization_id,wi.id,null,'no_eligible_assignee',now(),null,
...
select
  child.organization_id,child_adapter.work_item_id,parent_adapter.work_item_id,
  'part_of',true,
  jsonb_build_object(
    'source','current_company_work_canonicalization_v1',

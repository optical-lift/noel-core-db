-- Active response-work collaborators may answer a shared conversation without becoming its primary handler.
-- Endpoint send capability remains mandatory; this only extends the existing responsibility gate.
...
begin
  ...
  if v_source_count<>1 then
    raise exception 'Exactly one connected send transport is required for this endpoint; found %.',v_source_count using errcode='55000';
  end if;
  ...
  if v_conv_id is null then
    ...
    insert into atlas.institutional_conversations(
      ...
      v_endpoint.organization_id,v_endpoint.organization_unit_id,v_stable,nullif(btrim(coalesce(p_subject,'')),''),now(),now(),
      jsonb_build_object('createdFromOutboundIntent',true,'initiatedByMembershipId',v_member.id)
    ) returning id into v_conv_id;
    insert into atlas.institutional_conversation_endpoints(institutional_conversation_id,communication_endpoint_id,endpoint_role)
    values(v_conv_id,v_endpoint.id,'primary');
  else
    select * into v_conv from atlas.institutional_conversations
    where id=v_conv_id and organization_id=v_endpoint.organization_id
    ...
    if not exists(select 1 from atlas.institutional_conversation_endpoints ce
      where ce.institutional_conversation_id=v_conv_id and ce.communication_endpoint_id=v_endpoint.id) then
      raise exception 'Conversation is not associated with the sending endpoint.' using errcode='42501';
    end if;
  end if;

  select * into v_case from atlas.institutional_conversation_response_cases
  where institutional_conversation_id=v_conv_id and case_state not in ('complete','informational')
  ...
  if v_case.id is not null then
    ...
    if v_current.id is not null
       ...
       and not v_is_collaborator
       and not atlas.communication_endpoint_membership_has_capability_v1(v_endpoint.id,v_member.id,'admin') then
      raise exception 'Another member currently owns this conversation response. Join the response work or receive a handoff before sending.' using errcode='42501';
    end if;
  ...
  v_hash:=encode(extensions.digest(convert_to(jsonb_build_object(
    'endpointId',v_endpoint.id,'conversationId',v_conv_id,
    'to',coalesce(p_to_recipients,'[]'::jsonb),'cc',coalesce(p_cc_recipients,'[]'::jsonb),'bcc',coalesce(p_bcc_recipients,'[]'::jsonb),
    'subject',p_subject,'bodyText',p_body_text,'bodyHtml',p_body_html,
    'attachments',coalesce(p_attachment_refs,'[]'::jsonb),'replyToCommunicationEventId',p_reply_to_communication_event_id
  )::text,'UTF8'),'sha256'),'hex');

  insert into atlas.communication_outbound_operations(
    id,organization_id,organization_unit_id,communication_endpoint_id,institutional_conversation_id,connected_source_id,
    initiated_by_membership_id,to_recipients,cc_recipients,bcc_recipients,subject,body_text,body_html,attachment_refs,
    reply_to_communication_event_id,idempotency_key,content_sha256,operation_state,metadata
  ) values (
    v_operation_id,v_endpoint.organization_id,v_endpoint.organization_unit_id,v_endpoint.id,v_conv_id,v_source_id,
    v_member.id,coalesce(p_to_recipients,'[]'::jsonb),coalesce(p_cc_recipients,'[]'::jsonb),coalesce(p_bcc_recipients,'[]'::jsonb),
    p_subject,p_body_text,p_body_html,coalesce(p_attachment_refs,'[]'::jsonb),p_reply_to_communication_event_id,
    v_key,v_hash,'authorized',jsonb_build_object('authorizationSource','explicit_authenticated_send_command','actorUserId',auth.uid(),'responseCollaborator',v_is_collaborator)
  ) on conflict (organization_id,idempotency_key) do nothing;

  select id into strict v_operation_id from atlas.communication_outbound_operations
  where organization_id=v_endpoint.organization_id and idempotency_key=v_key;
  return jsonb_build_object(
    'contractVersion','institutional_email_send_intent_v1','outboundOperationId',v_operation_id,
    'institutionalConversationId',v_conv_id,'communicationEndpointId',v_endpoint.id,'connectedSourceId',v_source_id,
    'operationState','authorized','initiatedByMembershipId',v_member.id,'contentSha256',v_hash
  );
end;
$$;

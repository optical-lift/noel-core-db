...
begin
  ...
  select h.farm_id,new.id,h.id,h.bucket_band,h.bucket_equivalent_floor,(h.bucket_band='more_than_one' and h.bucket_halves is null)
  ...
  if v_input_count < 1 then raise exception 'There is no unprepared harvest output in this batch.' using errcode='22023'; end if;
  return new;
end;
$function$;
...
for each row execute function atlas.attach_directed_flower_preparation_inputs_v2();
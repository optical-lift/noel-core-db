...
-- ============================================================================
-- Product RPC membrane: public PostgREST wrappers over canonical Atlas authority.
-- ============================================================================

create or replace function public.record_current_household_claim_evidence_api_v1(
  p_payload jsonb
)
returns jsonb
language sql
...
set search_path = pg_catalog
as $$
  select atlas.record_current_household_claim_evidence_api_v1(p_payload);
$$;

create or replace function public.current_household_claim_evidence_state_api_v1()
returns jsonb
language sql
stable
security definer
set search_path = pg_catalog
as $$
  select atlas.current_household_claim_evidence_state_api_v1();
$$;

create or replace function public.calibrate_personal_laundry_kernel_self_api_v2(
  p_input jsonb
)
returns jsonb
language sql
...
set search_path = pg_catalog
as $$
  select atlas.calibrate_personal_laundry_kernel_self_api_v2(p_input);
$$;

create or replace function public.personal_laundry_kernel_self_api_v2()
returns jsonb
language sql
stable
security definer
set search_path = pg_catalog
as $$
  select atlas.personal_laundry_kernel_self_api_v2();
$$;

create or replace function public.ensure_personal_laundry_consequence_definition_self_api_v1(
  p_need_generation_claim_id uuid
)
returns jsonb
language sql
...
set search_path = pg_catalog
as $$
  select atlas.ensure_personal_laundry_consequence_definition_self_api_v1(p_need_generation_claim_id);
$$;

create or replace function public.evaluate_personal_laundry_consequence_from_household_evidence_self_api_v1(
  p_definition_id uuid,
  p_payload jsonb
)
returns jsonb
...
set search_path = pg_catalog
as $$
  select atlas.evaluate_personal_laundry_consequence_from_household_evidence_self_api_v1(
    p_definition_id,
    p_payload
  );
$$;

create or replace function public.record_person_life_consequence_clock_characterization_self_api_v1(
  p_consequence_instance_id uuid,
  p_input jsonb
)
returns jsonb
...
set search_path = pg_catalog
as $$
  select atlas.record_person_life_consequence_clock_characterization_self_api_v1(
    p_consequence_instance_id,
    p_input
  );
$$;

create or replace function public.person_life_consequence_clock_admission_self_api_v1(
  p_consequence_instance_id uuid
)
returns jsonb
language sql
...
set search_path = pg_catalog
as $$
  select atlas.person_life_consequence_clock_admission_self_api_v1(
    p_consequence_instance_id
  );
$$;

create or replace function public.reconcile_personal_laundry_consequence_axes_self_api_v1(
  p_consequence_instance_id uuid,
  p_input jsonb
)
returns jsonb
...
set search_path = pg_catalog
as $$
  select atlas.reconcile_personal_laundry_consequence_axes_self_api_v1(
    p_consequence_instance_id,
    p_input
  );
$$;

create or replace function public.principal_clock_api_v2(
...
set search_path = pg_catalog
as $$
  select atlas.principal_clock_api_v2(p_day,p_as_of);
$$;

create or replace function public.record_personal_laundry_actual_self_api_v1(
  p_input jsonb
)
returns jsonb
language sql
...
set search_path = pg_catalog
as $$
  select atlas.record_personal_laundry_actual_self_api_v1(p_input);
$$;

create or replace function public.resolve_personal_laundry_consequence_from_actual_self_api_v1(
  p_consequence_instance_id uuid,
  p_actual_claim_id uuid
)
returns jsonb
...
set search_path = pg_catalog
as $$
  select atlas.resolve_personal_laundry_consequence_from_actual_self_api_v1(
    p_consequence_instance_id,
    p_actual_claim_id
  );
$$;

create or replace function public.propose_personal_laundry_weekly_pattern_self_api_v1()
returns jsonb
language sql
security definer
set search_path = pg_catalog
as $$
  select atlas.propose_personal_laundry_weekly_pattern_self_api_v1();
$$;

-- Trusted server surface for explicit human-decision receipts. The browser
-- never receives service_role authority; Atlas's server route verifies the
-- browser bearer token and derives p_owner_user_id before calling this wrapper.
create or replace function public.issue_personal_reality_human_decision_receipt_serv_v1(
  p_owner_user_id uuid,
  p_proposal_id uuid,
  p_decision text,
...
set search_path = pg_catalog
as $$
  select atlas.issue_personal_reality_human_decision_receipt_serv_v1(
    p_owner_user_id,
    p_proposal_id,
    p_decision,
    p_interaction_ref,
...
revoke all on function public.record_current_household_claim_evidence_api_v1(jsonb) from public, anon;
revoke all on function public.current_household_claim_evidence_state_api_v1() from public, anon;
revoke all on function public.calibrate_personal_laundry_kernel_self_api_v2(jsonb) from public, anon;
revoke all on function public.personal_laundry_kernel_self_api_v2() from public, anon;
revoke all on function public.ensure_personal_laundry_consequence_definition_self_api_v1(uuid) from public, anon;
revoke all on function public.evaluate_personal_laundry_consequence_from_household_evidence_self_api_v1(uuid,jsonb) from public, anon;
revoke all on function public.record_person_life_consequence_clock_characterization_self_api_v1(uuid,jsonb) from public, anon;
revoke all on function public.person_life_consequence_clock_admission_self_api_v1(uuid) from public, anon;
revoke all on function public.reconcile_personal_laundry_consequence_axes_self_api_v1(uuid,jsonb) from public, anon;
revoke all on function public.principal_clock_api_v2(date,timestamptz) from public, anon;
revoke all on function public.record_personal_laundry_actual_self_api_v1(jsonb) from public, anon;
revoke all on function public.resolve_personal_laundry_consequence_from_actual_self_api_v1(uuid,uuid) from public, anon;
revoke all on function public.propose_personal_laundry_weekly_pattern_self_api_v1() from public, anon;

grant execute on function public.record_current_household_claim_evidence_api_v1(jsonb) to authenticated, service_role;
grant execute on function public.current_household_claim_evidence_state_api_v1() to authenticated, service_role;
grant execute on function public.calibrate_personal_laundry_kernel_self_api_v2(jsonb) to authenticated, service_role;
grant execute on function public.personal_laundry_kernel_self_api_v2() to authenticated, service_role;
grant execute on function public.ensure_personal_laundry_consequence_definition_self_api_v1(uuid) to authenticated, service_role;
grant execute on function public.evaluate_personal_laundry_consequence_from_household_evidence_self_api_v1(uuid,jsonb) to authenticated, service_role;
grant execute on function public.record_person_life_consequence_clock_characterization_self_api_v1(uuid,jsonb) to authenticated, service_role;
grant execute on function public.person_life_consequence_clock_admission_self_api_v1(uuid) to authenticated, service_role;
grant execute on function public.reconcile_personal_laundry_consequence_axes_self_api_v1(uuid,jsonb) to authenticated, service_role;
grant execute on function public.principal_clock_api_v2(date,timestamptz) to authenticated, service_role;
grant execute on function public.record_personal_laundry_actual_self_api_v1(jsonb) to authenticated, service_role;
grant execute on function public.resolve_personal_laundry_consequence_from_actual_self_api_v1(uuid,uuid) to authenticated, service_role;
grant execute on function public.propose_personal_laundry_weekly_pattern_self_api_v1() to authenticated, service_role;

revoke all on function public.issue_personal_reality_human_decision_receipt_serv_v1(uuid,uuid,text,text,jsonb)
  from public, anon, authenticated;
grant execute on function public.issue_personal_reality_human_decision_receipt_serv_v1(uuid,uuid,text,text,jsonb)
  to service_role;

comment on function public.record_current_household_claim_evidence_api_v1(jsonb) is
  'Public PostgREST membrane for the canonical authenticated Household Claim/Evidence writer.';
comment on function public.current_household_claim_evidence_state_api_v1() is
  'Public PostgREST membrane for the canonical authenticated Household Claim/Evidence read.';
comment on function public.calibrate_personal_laundry_kernel_self_api_v2(jsonb) is
  'Public PostgREST membrane for progressive signed-in Laundry calibration.';
comment on function public.personal_laundry_kernel_self_api_v2() is
  'Public PostgREST membrane for the signed-in Laundry kernel/instance truth read.';
comment on function public.ensure_personal_laundry_consequence_definition_self_api_v1(uuid) is
  'Public PostgREST membrane for deterministic Laundry consequence-definition ensure.';
comment on function public.evaluate_personal_laundry_consequence_from_household_evidence_self_api_v1(uuid,jsonb) is
  'Public PostgREST membrane for governed Laundry consequence evaluation from Household Evidence.';
comment on function public.record_person_life_consequence_clock_characterization_self_api_v1(uuid,jsonb) is
  'Public PostgREST membrane for explicit source-backed Person Life consequence Clock characterization.';
comment on function public.person_life_consequence_clock_admission_self_api_v1(uuid) is
  'Public PostgREST membrane for read-only Person Life consequence Clock admission state.';
comment on function public.reconcile_personal_laundry_consequence_axes_self_api_v1(uuid,jsonb) is
  'Public PostgREST membrane for source-backed Laundry carrier/readiness reconciliation.';
comment on function public.principal_clock_api_v2(date,timestamptz) is
  'Public PostgREST membrane for the signed-in expanded Principal Clock read.';
comment on function public.record_personal_laundry_actual_self_api_v1(jsonb) is
  'Public PostgREST membrane for explicit physical Laundry process actuals.';
comment on function public.resolve_personal_laundry_consequence_from_actual_self_api_v1(uuid,uuid) is
  'Public PostgREST membrane for resolving an open Laundry requirement from a qualifying physical actual.';
comment on function public.propose_personal_laundry_weekly_pattern_self_api_v1() is
  'Public PostgREST membrane for conservative Laundry weekly-pattern proposal learning.';
comment on function public.issue_personal_reality_human_decision_receipt_serv_v1(uuid,uuid,text,text,jsonb) is
  'Service-role-only public PostgREST membrane for trusted Personal Reality human-decision receipt issuance.';

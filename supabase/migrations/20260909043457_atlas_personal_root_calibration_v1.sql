...
declare v_household_id uuid; v_dimensions jsonb; v_context jsonb;
begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object('dimensionKey',d.dimension_key,'title',d.title,'prompt',d.prompt,'selectionKind',d.selection_kind,'options',(select coalesce(jsonb_agg(jsonb_build_object('optionKey',o.option_key,'label',o.label) order by o.position),'[]'::jsonb) from atlas.root_calibration_options o where o.dimension_key=d.dimension_key and o.active),'selected',coalesce(c.selections,'[]'::jsonb),'quantity',c.quantity,'source',c.source,'confirmed',coalesce(c.confirmed,false)) order by d.position),'[]'::jsonb) into v_dimensions from atlas.root_calibration_dimensions d left join atlas.household_root_calibrations c on c.household_id=v_household_id and c.dimension_key=d.dimension_key where d.active;
 select coalesce(jsonb_agg(distinct x),'[]'::jsonb) into v_context from atlas.household_root_calibrations c, lateral jsonb_array_elements_text(c.selections) t(x) where c.household_id=v_household_id and c.confirmed;
...
declare v_household_id uuid; v_dimension text := nullif(trim(p_input->>'dimensionKey'),''); v_selections jsonb := coalesce(p_input->'selections','[]'::jsonb); v_quantity integer := case when nullif(p_input->>'quantity','') is null then null else (p_input->>'quantity')::integer end; v_row atlas.household_root_calibrations%rowtype;
begin
 if auth.uid() is null then raise exception 'Sign in required.' using errcode='42501'; end if;
 ...
 insert into atlas.household_root_calibration_history(household_id,dimension_key,selections,quantity,source,confirmed,changed_by,metadata) select household_id,dimension_key,selections,quantity,source,confirmed,auth.uid(),metadata from atlas.household_root_calibrations where household_id=v_household_id and dimension_key=v_dimension;
 insert into atlas.household_root_calibrations(household_id,dimension_key,selections,quantity,source,confirmed,confirmed_by,confirmed_at,metadata) values(v_household_id,v_dimension,v_selections,v_quantity,'self_reported',true,auth.uid(),now(),jsonb_build_object('enteredFrom','personal_root_calibration_v1')) on conflict(household_id,dimension_key) do update set selections=excluded.selections,quantity=excluded.quantity,source=excluded.source,confirmed=true,confirmed_by=excluded.confirmed_by,confirmed_at=now(),metadata=atlas.household_root_calibrations.metadata||excluded.metadata,updated_at=now() returning * into v_row;
...
revoke all on function public.personal_root_calibration_self_api_v1() from public,anon; revoke all on function public.upsert_personal_root_calibration_self_api_v1(jsonb) from public,anon; revoke all on function public.personal_kernel_models_self_api_v1(text) from public,anon;
grant execute on function public.personal_root_calibration_self_api_v1() to authenticated,service_role; grant execute on function public.upsert_personal_root_calibration_self_api_v1(jsonb) to authenticated,service_role; grant execute on function public.personal_kernel_models_self_api_v1(text) to authenticated,service_role;
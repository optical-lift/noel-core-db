...
begin
  ...
  if p_actual_lines is null or jsonb_typeof(p_actual_lines) <> 'array'
    ...
    raise exception 'Final tally requires between 1 and 12 requested lines.' using errcode = '22023';
  end if;

  if p_worker_added_lines is null or jsonb_typeof(p_worker_added_lines) <> 'array'
     or jsonb_array_length(p_worker_added_lines) > 12 then
    raise exception 'Worker-added final tally must be an array of at most 12 lines.' using errcode = '22023';
  end if;

  if p_remaining_stems is null or jsonb_typeof(p_remaining_stems) <> 'array'
     or jsonb_array_length(p_remaining_stems) > 24 then
    raise exception 'Remaining stems must be an array of at most 24 lines.' using errcode = '22023';
  end if;
  ...
  v_fingerprint := md5(
    p_task_id::text || '|' || v_directive.id::text || '|' ||
    p_actual_lines::text || '|' || p_worker_added_lines::text || '|' || p_remaining_stems::text
  );
  ...
  if v_existing.id is not null then
    ...
    return jsonb_build_object(
      ...
      'workerAddedLineCount', jsonb_array_length(coalesce(v_existing.metadata->'workerAddedLines', '[]'::jsonb)),
      'readyInventoryLotsCreated', (select count(*) from atlas.flower_ready_inventory_lots lot where lot.metadata->>'directiveResultId' = v_existing.id::text),
      'deduplicated', true
    );
  end if;
  ...
  for v_line in select value from jsonb_array_elements(p_worker_added_lines) loop
    if jsonb_typeof(v_line) <> 'object' then
      raise exception 'Each worker-added tally line must be an object.' using errcode = '22023';
    end if;
    ...
    if v_product_label is null or char_length(v_product_label) > 160 then
      raise exception 'Each worker-added tally line requires a flower label of 160 characters or fewer.' using errcode = '22023';
    end if;

    v_output_kind := btrim(coalesce(v_line->>'outputKind', ''));
    if v_output_kind not in ('bundle', 'posy', 'bouquet', 'lobby_arrangement') then
      raise exception 'Worker-added tally line has an unsupported pack type.' using errcode = '22023';
    end if;

    v_actual_text := btrim(coalesce(v_line->>'actualQuantity', ''));
    if v_actual_text !~ '^[1-9][0-9]*$' then
      raise exception 'Worker-added actual quantity must be a positive whole number.' using errcode = '22023';
    end if;
    v_actual_quantity := v_actual_text::integer;
    if v_actual_quantity > 10000 then
      raise exception 'Worker-added actual quantity is outside the supported range.' using errcode = '22023';
    end if;

    if v_output_kind = 'bundle' then
      if coalesce(v_line->>'stemsPerUnit', '') !~ '^[1-9][0-9]*$' then
        raise exception 'Worker-added bunches require a positive stems-per-bunch count.' using errcode = '22023';
      end if;
      v_stems_per_unit := (v_line->>'stemsPerUnit')::integer;
      if v_stems_per_unit > 1000 then
        raise exception 'Stems per bunch is outside the supported range.' using errcode = '22023';
      end if;
    end if;
  end loop;
  ...
  insert into atlas.flower_preparation_directive_results(
    ...
    jsonb_build_object(
      ...
      'readyInventoryCreated', true,
      'workerAddedLines', p_worker_added_lines,
      'remainingStems', p_remaining_stems
    )
  ...
  for v_line in select value from jsonb_array_elements(p_actual_lines) loop
    ...
    insert into atlas.flower_preparation_directive_result_lines(
      farm_id, result_id, directive_line_id, actual_quantity, note
    ) values (
      v_task.farm_id, v_result.id, v_line_id, v_actual_quantity, v_note
    );
  ...
  for v_line in select value from jsonb_array_elements(p_worker_added_lines) loop
    v_total_saleable := v_total_saleable + (v_line->>'actualQuantity')::integer;
  end loop;
  ...
  insert into atlas.flower_preparation_batches(
    ...
    v_task.farm_id,
    v_directive.harvest_batch_id,
    v_task.id,
    v_ready_date,
    v_membership.id,
    case when v_total_saleable > 0 then 'ready' else 'no_saleable_output' end,
    'Final tally from Owner-directed flower preparation.',
    left(v_key || ':ready', 160),
    jsonb_build_object(
      'source', 'flower_preparation_directive_result_v2',
      'directiveResultId', v_result.id,
  ...
  for v_line in select value from jsonb_array_elements(p_actual_lines) loop
    ...
    if v_actual_quantity > 0 then
      ...
      from atlas.flower_preparation_directive_lines
      where id = v_line_id;

      v_inventory_kind := case v_directive_line.output_kind
        when 'bundle' then 'bunch'
        when 'posy' then 'posy'
        when 'bouquet' then 'bouquet'
        when 'lobby_arrangement' then 'lobby_arrangement'
        else null
      end;
      v_unit := case v_inventory_kind
      ...
      if v_inventory_kind is null then
        raise exception 'Requested preparation line has an unsupported sellable output kind.' using errcode = '22023';
      end if;
      ...
      insert into atlas.flower_ready_inventory_lots(
        ...
        'exact',
        v_ready_date,
        left('directive-result:' || v_result.id::text || ':' || v_line_id::text, 160),
        jsonb_build_object(
          'source', 'worker_confirmed_preparation',
          'directiveResultId', v_result.id,
          'directiveLineId', v_line_id,
          'requestedQuantity', v_directive_line.requested_quantity,
          'stemsPerUnit', v_directive_line.stems_per_unit,
          'outputKind', v_directive_line.output_kind
        ),
        v_directive_line.crop_profile_id,
        v_directive_line.product_label
      );
  ...
  v_added_index := 0;
  for v_line in select value from jsonb_array_elements(p_worker_added_lines) loop
    v_added_index := v_added_index + 1;
    v_actual_quantity := (v_line->>'actualQuantity')::integer;
    v_product_label := btrim(v_line->>'productLabel');
    v_output_kind := btrim(v_line->>'outputKind');
    v_stems_per_unit := case when v_output_kind = 'bundle' then (v_line->>'stemsPerUnit')::integer else null end;
    v_inventory_kind := case v_output_kind
      when 'bundle' then 'bunch'
      when 'posy' then 'posy'
      when 'bouquet' then 'bouquet'
      when 'lobby_arrangement' then 'lobby_arrangement'
    ...
    insert into atlas.flower_ready_inventory_lots(
      ...
      'exact',
      v_ready_date,
      left('directive-result:' || v_result.id::text || ':added:' || v_added_index::text, 160),
      jsonb_build_object(
        'source', 'worker_added_actual',
        'directiveResultId', v_result.id,
        'lineNumber', coalesce(v_line->>'lineNumber', (v_expected_count + v_added_index)::text),
        'stemsPerUnit', v_stems_per_unit,
        'outputKind', v_output_kind
      ),
      null,
      v_product_label
    );
  ...
  v_transition := atlas.record_task_transition_v1(
    p_task_id => v_task.id,
    p_transition => 'done',
    p_idempotency_key => 'flower-prep-final-tally-v2:' || v_result.id::text,
    p_target_date => null,
    p_note => null,
    p_reason => null,
    p_lane_key => null,
    ...
    p_payload => jsonb_build_object(
      ...
      'flowerPreparationDirectiveResultVersion', 2,
      ...
      'workerAddedLineCount', jsonb_array_length(p_worker_added_lines),
      'readyInventoryLotsCreated', v_ready_count,
      'truthBoundary', 'worker_finished_preparation_tally'
    ),
    p_existing_field_log_id => null
  );

  update atlas.flower_preparation_directive_results
  set metadata = metadata || jsonb_build_object(
    'readyInventoryLotsCreated', v_ready_count,
    'preparationBatchId', v_batch.id
  )

...
declare
  ...
  v_context_id uuid;
  v_name text; v_address text; v_city text; v_state text; v_postal text;
  v_phone text; v_phone_norm text; v_website text; v_category text;
  v_source_url text; v_source_title text; v_publisher text; v_source_kind text;
  v_fingerprint text; v_source_id uuid; v_ingestion_source_id uuid; v_candidate_id uuid; v_entity_id uuid;
  v_match_count integer; v_name_city_count integer; v_disposition text; v_stable_key text;
begin
  ...
  if v_work.status not in ('in_process','queued','retry') then raise exception 'Business census work item % is in status %',p_work_item_id,v_work.status; end if;
  v_context_id:=v_work.local_context_id;
  if v_context_id is null then raise exception 'Business census work item % has no Local context',p_work_item_id; end if;
  ...
  v_publisher:=coalesce(nullif(btrim(p_record->>'publisher'),''),'Atlas Local public business discovery');
  ...
  if v_source_id is null then
    ...
    values(v_source_url,v_source_kind,v_publisher,v_source_title,now(),'Atlas Local business census durable identity/category discovery; does not establish current availability.',
  ...
  select id into v_ingestion_source_id from local_intel.ingestion_sources where source_id=v_source_id and source_role='organization_directory';
  ...
  insert into local_intel.entity_ingestion_candidates(
    local_context_id,ingestion_source_id,source_record_key,proposed_entity_type,proposed_name,website_url,phone,address_line1,city,state,postal_code,category_text,confidence,review_state,metadata
  ) values(
    v_context_id,v_ingestion_source_id,v_fingerprint,'business',v_name,v_website,v_phone,v_address,v_city,v_state,v_postal,v_category,0.85,'pending',
  ...
  on conflict(local_context_id,ingestion_source_id,source_record_key) do update set
    ...
    metadata=local_intel.entity_ingestion_candidates.metadata||excluded.metadata,updated_at=now()
  returning id into v_candidate_id;

  select count(*),(array_agg(e.id order by e.id::text))[1] into v_match_count,v_entity_id
  from local_intel.entities e
  where e.local_context_id=v_context_id and e.entity_type='business' and e.status<>'inactive'
    and ((v_phone_norm<>'' and regexp_replace(coalesce(e.phone,''),'[^0-9]','','g')=v_phone_norm)
      or (v_address is not null and lower(btrim(e.name))=lower(btrim(v_name))
        and lower(regexp_replace(coalesce(e.address_line1,''),'[^a-zA-Z0-9]','','g'))=lower(regexp_replace(v_address,'[^a-zA-Z0-9]','','g'))));
  ...
  if v_match_count=1 then
    ...
    from local_intel.entities e
    where e.local_context_id=v_context_id and e.entity_type='business' and e.status<>'inactive'
      and lower(btrim(e.name))=lower(btrim(v_name)) and lower(btrim(coalesce(e.city,'')))=lower(btrim(coalesce(v_city,'')));

    if v_match_count>1 or v_name_city_count>0 then
      v_disposition:='needs_review';
      update local_intel.entity_ingestion_candidates set review_state='needs_review',
        ...
        resolver_algorithm_key='business_census_identity_v1',resolver_algorithm_version='1',
        ...
        resolver_recommended_at=now(),updated_at=now() where id=v_candidate_id;
      ...
      insert into local_intel.entities(local_context_id,stable_key,entity_type,name,website_url,phone,address_line1,city,state,postal_code,status,verification_state,last_verified_at,metadata)
      values(v_context_id,v_stable_key,'business',v_name,v_website,v_phone,v_address,v_city,v_state,v_postal,'active','public_indexed',now(),
        jsonb_build_object('business_census',true,'identity_basis','publicly indexed named business discovery','first_census_source_id',v_source_id,'query_family',v_work.query_family_key,'publication_state','identity_publishable','category',coalesce(v_category,'business')))
  ...
  insert into local_intel.business_census_discovery_records(work_item_id,discovery_fingerprint,business_name,address_line1,city,state,postal_code,phone,website_url,category_text,source_url,source_title,publisher,source_kind,source_id,ingestion_source_id,ingestion_candidate_id,canonical_entity_id,disposition,metadata)
  values(p_work_item_id,v_fingerprint,v_name,v_address,v_city,v_state,v_postal,v_phone,v_website,v_category,v_source_url,v_source_title,v_publisher,v_source_kind,v_source_id,v_ingestion_source_id,v_candidate_id,v_entity_id,v_disposition,jsonb_build_object('raw',coalesce(p_record->'raw','{}'::jsonb),'local_context_id',v_context_id))
  on conflict(work_item_id,discovery_fingerprint) do update set source_id=excluded.source_id,ingestion_source_id=excluded.ingestion_source_id,ingestion_candidate_id=excluded.ingestion_candidate_id,canonical_entity_id=coalesce(excluded.canonical_entity_id,local_intel.business_census_discovery_records.canonical_entity_id),disposition=excluded.disposition,observed_at=now(),updated_at=now(),metadata=local_intel.business_census_discovery_records.metadata||excluded.metadata;
  ...
  return jsonb_build_object('work_item_id',p_work_item_id,'local_context_id',v_context_id,'disposition',v_disposition,'entity_id',v_entity_id,'candidate_id',v_candidate_id,'source_id',v_source_id);
end;
$$;

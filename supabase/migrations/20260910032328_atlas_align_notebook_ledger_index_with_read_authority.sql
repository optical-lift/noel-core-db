-- Do not advertise an Organization Ledger page unless the same caller can open the owner Ledger read membrane.
-- Index is retrieval metadata only and must never imply broader authority than the target spread actually accepts.

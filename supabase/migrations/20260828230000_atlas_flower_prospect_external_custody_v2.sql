...
--
-- A prospect route is actual custody of Ready flower inventory before sale truth.
-- The custodian may be an Atlas farm member or an external person such as a
-- commissioned route seller. Exactly one custody identity is required.

alter table atlas.flower_prospect_routes
  add column if not exists custodian_label text;

alter table atlas.flower_prospect_routes
  alter column assigned_membership_id drop not null;

alter table atlas.flower_prospect_routes
  drop constraint if exists flower_prospect_routes_custodian_identity_check;

alter table atlas.flower_prospect_routes
  add constraint flower_prospect_routes_custodian_identity_check
  check (
    (assigned_membership_id is not null)
    <> (nullif(btrim(coalesce(custodian_label, '')), '') is not null)
  );
...
as $function$
declare
  v_assignee_farm uuid;
  v_recorder_farm uuid;
begin
  select farm_id into v_recorder_farm
  ...
  if new.assigned_membership_id is not null then
    select farm_id into v_assignee_farm
    from atlas.farm_memberships
...
declare
  ...
  v_existing atlas.flower_prospect_routes%rowtype;
  v_route atlas.flower_prospect_routes%rowtype;
  v_external_custodian text := nullif(btrim(coalesce(p_custodian_label, '')), '');
  v_line jsonb;
...
begin
  if v_key is null then
    raise exception 'Prospect-route idempotency key is required.' using errcode = '22023';
  end if;
  ...
  if (p_assigned_membership_id is null) = (v_external_custodian is null) then
  ...
  if nullif(btrim(coalesce(p_route_label, '')), '') is null then
    raise exception 'Prospect route requires a label.' using errcode = '22023';
  end if;
  ...
  from atlas.flower_prospect_routes
  ...
  if v_existing.id is not null then
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'id', l.id,
          'readyLotId', l.ready_lot_id,
          'quantity', l.quantity,
          'buyerRelationshipId', l.buyer_relationship_id,
    ...
    where l.prospect_route_id = v_existing.id;

    return jsonb_build_object(
      'prospectRouteId', v_existing.id,
      'assignedMembershipId', v_existing.assigned_membership_id,
      'custodianLabel', v_existing.custodian_label,
      'lines', v_rows,
      'deduplicated', true
    );
  end if;
  ...
  insert into atlas.flower_prospect_routes(
    ...
    jsonb_build_object(
      ...
      'truthBoundary', 'actual_prospect_custody',
      'saleTruth', false,
  ...
  for v_line in select value from jsonb_array_elements(p_lines) loop
    begin
      v_ready_id := nullif(v_line ->> 'readyLotId', '')::uuid;
    exception when others then
      raise exception 'Invalid readyLotId in prospect route.' using errcode = '22023';
    end;
    begin
      v_buyer := nullif(v_line ->> 'buyerRelationshipId', '')::uuid;
    exception when others then
      raise exception 'Invalid buyerRelationshipId in prospect route.' using errcode = '22023';
    end;
    begin
      v_quantity := (v_line ->> 'quantity')::numeric;
    exception when others then
      raise exception 'Invalid quantity in prospect route.' using errcode = '22023';
    end;
    ...
    if v_ready.id is null or v_ready.farm_id is distinct from p_farm_id then
      raise exception 'Prospect Ready lot is outside this farm.' using errcode = '22023';
    end if;
    if v_quantity is null or v_quantity <= 0 then
      raise exception 'Prospect quantity must be positive.' using errcode = '22023';
    end if;
    if v_ready.unit = 'bucket_equivalent' then
      if mod(v_quantity * 4, 1) <> 0 then
        raise exception 'Prospect bucket quantity must use quarter-bucket increments.' using errcode = '22023';
      end if;
    elsif mod(v_quantity, 1) <> 0 then
      raise exception 'Prospect counted units must be whole numbers.' using errcode = '22023';
    end if;
    ...
    if v_quantity > coalesce(v_available, 0) then
      raise exception 'Prospect route would exceed Ready quantity still Available.' using errcode = '22023';
    end if;

    insert into atlas.flower_prospect_route_lines(
      farm_id,
      prospect_route_id,
      ready_lot_id,
      ...
      v_dest,
      v_quantity,
      jsonb_build_object('truthBoundary', 'on_prospect_route', 'saleTruth', false)
    ) returning * into v_inserted;

    v_rows := v_rows || jsonb_build_array(jsonb_build_object(
      'id', v_inserted.id,
      'readyLotId', v_inserted.ready_lot_id,
      'quantity', v_inserted.quantity,
      'buyerRelationshipId', v_inserted.buyer_relationship_id,
      'destinationLabel', v_inserted.destination_label
    ));
  end loop;

  return jsonb_build_object(
    'prospectRouteId', v_route.id,
    'routeDate', v_route.route_date,
    'assignedMembershipId', v_route.assigned_membership_id,
    'custodianLabel', v_route.custodian_label,
    'lines', v_rows,
    'deduplicated', false
  );
end;
...
begin
  ...
  return atlas.record_flower_prospect_route_core_v2(
    p_farm_id,
    v_membership,
    v_role,
    p_assigned_membership_id,
...
begin
  ...
  v_farm := nullif(v_context ->> 'farmId', '')::uuid;
  return atlas.record_flower_prospect_route_core_v2(
    v_farm,
...
with target as (
  select
    ...
    case when p.proname like 'owner_operator_%' then 'owner_admin_endpoint' else 'app_endpoint' end as classification,
  ...
  where n.nspname = 'atlas'
    and p.proname in (
      'record_flower_prospect_route_for_member_v2',
      'owner_operator_record_flower_prospect_route_v2'
    )
)
...
select
  ...
  policy_reference_count,
  jsonb_build_object(
    'source', 'atlas_flower_prospect_external_custody_v2',
    'reason', 'allow_truthful_external_prospect_route_custodian',
    'functionOid', oid,
    'classificationRuleVersion', 3,
    'truthBoundary', 'Prospect route is actual custody before sale. Exactly one internal member or external custodian is named.'
  ),
  now(),
  now()
from target
...
begin
  ...
  if v_drift <> 0 then
    raise exception 'External prospect-route custody reconciliation ended with % drift rows.', v_drift;
  end if;
end
$verification$;

...
--
-- Purpose:
--   Preserve a worker/Owner assertion about when a known live crop cycle is
--   expected to need its next harvest/check without pretending that future
--   physical harvest has already happened.
--
-- Truth boundary:
--   crop_harvest_expectations = forward-looking human field knowledge
--   crop_harvest_events       = physical harvest/watch facts that already happened
--
-- System-derived biological expectations remain derived by the Harvest reader;
-- this table records only explicit human assertions and is append-only.
...
create table atlas.crop_harvest_expectations (
  ...
  constraint crop_harvest_expectations_quantity_unit_check
    check (
      (estimated_quantity is null and unit is null)
...
comment on table atlas.crop_harvest_expectations is
  'Append-only human forecast assertions for the next harvest/check of a canonical crop cycle. These rows never claim a future physical observation occurred.';
comment on column atlas.crop_harvest_expectations.expected_date is
  'The date the recording person expects this crop cycle to need its next harvest/check.';
comment on column atlas.crop_harvest_expectations.source_kind is
  'Role-derived provenance: worker_assertion, owner_assertion, or manager_assertion.';
...
begin
  ...
  if p_estimated_quantity is not null and v_unit is null then
    raise exception 'Choose a unit for the expected quantity.' using errcode = '22023';
  end if;
  if p_estimated_quantity is null and v_unit is not null then
    raise exception 'Do not choose a unit without an expected quantity.' using errcode = '22023';
  end if;
  ...
  if v_existing.id is not null then
    ...
    return jsonb_build_object(
      'contractVersion', 'crop_harvest_expectation_v1',
      'deduplicated', true,
      'expectationId', v_existing.id,
      'cropCycleId', v_existing.crop_cycle_id,
      'expectedDate', v_existing.expected_date,
      'estimatedQuantity', v_existing.estimated_quantity,
  ...
  return jsonb_build_object(
    'contractVersion', 'crop_harvest_expectation_v1',
    'deduplicated', false,
    'expectationId', v_row.id,
    'cropCycleId', v_row.crop_cycle_id,
    'expectedDate', v_row.expected_date,
    'estimatedQuantity', v_row.estimated_quantity,
    'unit', v_row.unit,
...
select
  ...
  jsonb_build_object(
    ...
    'truthBoundary', 'Authenticated farm members may append forward-looking harvest expectations for active crop cycles they can operate. The write does not create a physical harvest event, Ready inventory, or task completion.'
  ),
  now(),
  now()
from targets
...
begin
  ...
  where n.nspname = 'atlas'
    and p.proname in (
      'record_crop_harvest_expectation_for_member_v1',
      'owner_operator_record_crop_harvest_expectation_v1'
    )
  ...
  if v_drift <> 0 then
    raise exception 'Harvest expectation custody ended with % authenticated RPC drift rows.', v_drift;
  end if;
end
$verification$;

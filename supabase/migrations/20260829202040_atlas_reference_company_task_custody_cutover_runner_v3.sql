...
declare
  ...
  v_error text;
begin
  select * into v_scenario from atlas.reference_company_scenarios where stable_key='task_custody_cutover_v1' and active;
  if v_scenario.id is null then raise exception 'Reference company task custody scenario is unavailable.' using errcode='P0002'; end if;

  select * into v_farm from atlas.farms where stable_key='atlas_reference_farm';
  if v_farm.id is null or not atlas.is_system_fixture_farm_v1(v_farm.id) then raise exception 'Atlas Reference Farm isolation contract is unavailable.' using errcode='23514'; end if;
  v_org_id := v_farm.organization_id;

  select fm.user_id into v_user_id from atlas.farm_memberships fm where fm.active=true order by fm.created_at,fm.id limit 1;
  if v_user_id is null then raise exception 'Reference custody scenario requires one existing auth-backed Atlas membership.' using errcode='23514'; end if;
  ...
  begin
    ...
    values(v_user_id,v_farm.id,'manager','reference_custody_worker_'||replace(v_run_id::text,'-',''),true,
    ...
    select state,service_date into v_placement_state,v_placement_date from atlas.worker_day_task_placements where id=v_placement_id;
    select visibility_reason into v_visibility_reason from atlas.worker_day_visibility_floor_v1(v_farm.id,v_membership_id,v_today) where task_id=v_task_id limit 1;
    select count(*) into v_reschedule_count from atlas.task_transitions where task_id=v_task_id and transition='rescheduled';

    v_assertions := v_assertions || jsonb_build_array(jsonb_build_object('key','overdue_due_date_preserved','passed',v_due_after=v_due,'expected',v_due,'actual',v_due_after));
    v_assertions := v_assertions || jsonb_build_array(jsonb_build_object('key','unresolved_task_remains_open','passed',v_status_after='open','expected','open','actual',v_status_after));
    v_assertions := v_assertions || jsonb_build_array(jsonb_build_object(
      ...
      'expected',jsonb_build_object('state','returned_to_atlas','serviceDate',v_service_date),'actual',jsonb_build_object('state',v_placement_state,'serviceDate',v_placement_date)));
    v_assertions := v_assertions || jsonb_build_array(jsonb_build_object('key','overdue_visibility_survives_midnight','passed',v_visibility_reason='assigned_overdue','expected','assigned_overdue','actual',v_visibility_reason));
    v_assertions := v_assertions || jsonb_build_array(jsonb_build_object('key','generic_rollover_emits_no_reschedule_transition','passed',v_reschedule_count=0,'expected',0,'actual',v_reschedule_count));
  ...
  if not v_fixture_completed then
    ...
    return jsonb_build_object('contractVersion','run_reference_company_task_custody_cutover_v1','runId',v_run_id,'scenarioKey','task_custody_cutover_v1','status','failed','error',v_error,'fixtureRolledBack',true);
  end if;

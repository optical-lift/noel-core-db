-- Elm Local public calendar Atlas status + privacy correction v1
-- Publish active Elm events as well as planned ones, and keep proposed/unconfirmed instructors private.
...
set title = 'Clay + Color: Handmade Clay Jewelry Workshop',
    updated_at = now()
where stable_key = 'thursdays_at_elm_2026_10_22_evening'
  and coalesce(metadata ->> 'instructor_status','') <> 'confirmed';

-- Re-project all currently publishable Atlas rows so the public cache reflects
-- the safer detail whitelist and catches any already-active events.
update atlas.community_events

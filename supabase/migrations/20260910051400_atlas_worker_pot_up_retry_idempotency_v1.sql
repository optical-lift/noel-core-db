...
-- against the same Worker Day projection should return the already-recorded
-- institutional result rather than fail because Company Work is now closed.
do $repair$
declare
  v_oid oid;
  v_def text;
  v_old text:=$needle$  select * into v_work from atlas.work_items w
  ...
  if v_work.id is null then raise exception 'This Worker Day item is not open governed production pot-up work.' using errcode='23514'; end if;$needle$;
  v_new text:=$needle$  select * into v_result
  from atlas.work_execution_results r
  where r.work_item_id=v_work_ids[1]
    and r.organization_id=(v_context->>'organizationId')::uuid
    and r.result_contract_key='production_pot_up_v1'
  ...
  if v_result.id is not null then
    ...
    if v_acceptance.id is not null then
      return jsonb_build_object(
        'ok',true,'status','pot_up_completed','deduplicated',true,
        'projectionId',p_projection_id,'workItemId',v_result.work_item_id,
        'taskId',v_result.task_id,'executionResultId',v_result.id,

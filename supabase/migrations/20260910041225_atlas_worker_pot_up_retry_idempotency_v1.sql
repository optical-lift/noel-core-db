...
begin
  select p.oid into v_oid from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='atlas' and p.proname='worker_record_production_pot_up_self_api_v1' and pg_get_function_identity_arguments(p.oid)='p_delivery_membership_id uuid, p_projection_id uuid, p_outputs jsonb, p_care_date date, p_note text, p_idempotency_key text';

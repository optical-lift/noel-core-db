...
-- Access, identity, billing seat, institutional placement, responsibility, authority,
-- and semantic exposure remain separate truths.
...
create table if not exists atlas.organization_access_invitations (
  ...
  access_class text not null default 'employee' check (access_class='employee'),
  ...
  check ((issuer_kind='organization_owner' and issued_by_membership_id is not null)
...
comment on table atlas.organization_access_invitations is
  'Typed institutional access offers. Acceptance may establish an identity anchor, organization membership, paid employee seat, and credential only. It never implies position, responsibility, Principal authority, execution authority, or semantic exposure.';
...
begin
  ...
  -- Bounded bootstrap rule only for this command membrane. This is not a general
  -- inference that every role name grants every similarly named capability.
  ...
  if exists(select 1 from atlas.organization_onboarding_actors a
            where a.organization_id=p_organization_id and a.human_user_id=v_uid
              and a.active and a.ended_at is null) then
...
create or replace function atlas.prepare_organization_employee_invitation_self_api_v1(
  ...
  p_email text,
  p_display_name text,
  p_identity_subject_id uuid default null
)
...
begin
  ...
  if p_identity_subject_id is not null and not exists(
    select 1 from atlas.identity_subjects s
    where s.id=p_identity_subject_id and s.organization_id=p_organization_id and s.state='active'
  ) then raise exception 'Intended institutional identity is outside this organization.' using errcode='23514'; end if;

  if exists(
    select 1 from atlas.organization_member_credentials c
    join auth.users u on u.id=c.auth_user_id
    where c.organization_id=p_organization_id and c.credential_kind='auth_user'
      and c.status='active' and lower(u.email)=v_email
  ) then raise exception 'This authenticated account already has active organization access.' using errcode='23505'; end if;

  select * into v_invite from atlas.organization_access_invitations i
  where i.organization_id=p_organization_id and i.invitee_email=v_email
    and i.status in ('draft','issued') for update;

  if v_invite.id is null then
    insert into atlas.organization_access_invitations(
      organization_id,invitee_email,display_name,issuer_kind,issued_by_user_id,
      issued_by_membership_id,intended_identity_subject_id,metadata
    ) values(
      p_organization_id,v_email,v_name,v_ctx->>'issuerKind',v_uid,
      nullif(v_ctx->>'membershipId','')::uuid,p_identity_subject_id,
      jsonb_build_object('contractVersion','organization_access_invitation_v1')
    ) returning * into v_invite;
  else
    update atlas.organization_access_invitations set
      display_name=v_name,status='draft',issuer_kind=v_ctx->>'issuerKind',issued_by_user_id=v_uid,
      issued_by_membership_id=nullif(v_ctx->>'membershipId','')::uuid,
      intended_identity_subject_id=p_identity_subject_id,issued_at=null,expires_at=null,
      revoked_at=null,updated_at=now()
    where id=v_invite.id returning * into v_invite;
  end if;

  return jsonb_build_object('ok',true,'contractVersion','organization_access_invitation_v1',
    'invitationId',v_invite.id,'organizationId',v_invite.organization_id,'email',v_invite.invitee_email,
    'displayName',v_invite.display_name,'status',v_invite.status,'accessClass',v_invite.access_class,
    'intendedIdentitySubjectId',v_invite.intended_identity_subject_id);
end;
$function$;

create or replace function atlas.issue_organization_employee_invitation_service_v1(
  p_invitation_id uuid,p_expires_at timestamptz default null,p_auth_user_id uuid default null
)
returns jsonb language plpgsql security definer
set search_path=pg_catalog,atlas,auth
as $function$
...
begin
  select * into v_invite from atlas.organization_access_invitations i
  where i.id=p_invitation_id and i.status='draft' for update;
  ...
  update atlas.organization_access_invitations set status='issued',issued_at=now(),expires_at=p_expires_at,
    metadata=metadata||jsonb_strip_nulls(jsonb_build_object('providerAuthUserId',p_auth_user_id)),updated_at=now()
  where id=v_invite.id returning * into v_invite;
...
begin
  ...
  select jsonb_build_object('ok',true,'contractVersion','organization_access_invitation_v1',
  ...
  into v_result from atlas.organization_access_invitations i join atlas.organizations o on o.id=i.organization_id
  where i.id=p_invitation_id and i.status in ('issued','accepted') and i.invitee_email=v_email
...
as $function$
declare
  v_uid uuid:=auth.uid(); v_email text; v_invite atlas.organization_access_invitations%rowtype;
  v_issuer_ok boolean:=false; v_subject_id uuid; v_membership_id uuid; v_seat_id uuid; v_credential_id uuid;
  v_existing_credential atlas.organization_member_credentials%rowtype; v_candidate_count integer:=0; v_candidate_subject uuid;
  v_review_id uuid; v_source_record_id uuid; v_email_claim_id uuid;
begin
  if v_uid is null then raise exception 'Authentication required.' using errcode='42501'; end if;
  select lower(u.email) into v_email from auth.users u where u.id=v_uid and u.deleted_at is null
  ...
  if v_invite.status='accepted' and v_invite.accepted_by_user_id=v_uid then
    select * into v_existing_credential from atlas.organization_member_credentials c
    where c.organization_id=v_invite.organization_id and c.auth_user_id=v_uid
      and c.credential_kind='auth_user' and c.status='active' order by c.created_at limit 1;
    if v_existing_credential.id is not null then
      return jsonb_build_object('ok',true,'replayed',true,'invitationId',v_invite.id,
        ...
        'employeeSeatId',v_existing_credential.employee_seat_id,'credentialId',v_existing_credential.id);
    end if;
  end if;
  ...
  if v_invite.issuer_kind='organization_owner' then
    v_issuer_ok:=exists(select 1 from atlas.organization_memberships m
      where m.id=v_invite.issued_by_membership_id and m.organization_id=v_invite.organization_id
        and m.user_id=v_invite.issued_by_user_id and m.active and m.role='owner');
  else
    v_issuer_ok:=exists(select 1 from atlas.organization_onboarding_actors a
      where a.organization_id=v_invite.organization_id and a.human_user_id=v_invite.issued_by_user_id
  ...
  if v_existing_credential.id is not null then
    ...
    from atlas.organization_memberships m where m.organization_id=v_invite.organization_id
      and m.user_id=v_uid and m.active order by m.created_at limit 1;

    if v_subject_id is null and v_invite.intended_identity_subject_id is not null then
      if not exists(select 1 from atlas.identity_subjects s where s.id=v_invite.intended_identity_subject_id
                    and s.organization_id=v_invite.organization_id and s.state='active') then
    ...
    if v_subject_id is null then
      select count(distinct c.subject_id) into v_candidate_count
      from atlas.identity_claims c
      where c.organization_id=v_invite.organization_id and c.claim_kind in ('email','email_address')
        and lower(btrim(case when jsonb_typeof(c.claim_value)='string' then c.claim_value#>>'{}'
                         else coalesce(c.claim_value->>'value',c.claim_value->>'email',c.claim_value->>'address','') end))=v_email;

      if v_candidate_count>0 then
        select c.subject_id into v_candidate_subject from atlas.identity_claims c
        where c.organization_id=v_invite.organization_id and c.claim_kind in ('email','email_address')
          and lower(btrim(case when jsonb_typeof(c.claim_value)='string' then c.claim_value#>>'{}'
                           else coalesce(c.claim_value->>'value',c.claim_value->>'email',c.claim_value->>'address','') end))=v_email
        ...
        ) values(v_invite.organization_id,'source_binding',v_candidate_subject,'open','high',
          jsonb_build_object('reason','employee_invitation_email_matches_existing_identity_evidence',
            'invitationId',v_invite.id,'email',v_email,'candidateCount',v_candidate_count),
          'organization_access_invitation_v1') returning id into v_review_id;
        return jsonb_build_object('ok',false,'reason','identity_reconciliation_required',
          'invitationId',v_invite.id,'reviewId',v_review_id,'candidateCount',v_candidate_count);
      end if;

      insert into atlas.identity_subjects(organization_id,state,created_by_user_id,creation_basis)
      values(v_invite.organization_id,'active',v_uid,
    ...
    insert into atlas.identity_source_records(
      organization_id,source_system_key,source_record_kind,source_record_key,source_observed_at,
      source_authority,custody_ref,metadata
    ) values(v_invite.organization_id,'atlas_access','organization_employee_invitation',v_invite.id::text,now(),
      'authoritative_source',jsonb_build_object('invitationId',v_invite.id),
    ...
    select r.id into v_source_record_id from atlas.identity_source_records r
      where r.organization_id=v_invite.organization_id and r.source_system_key='atlas_access'
        and r.source_record_kind='organization_employee_invitation' and r.source_record_key=v_invite.id::text;

    if not exists(select 1 from atlas.identity_claims c where c.source_record_id=v_source_record_id
                  and c.subject_id=v_subject_id and c.claim_kind='email') then
      ...
      values(v_invite.organization_id,v_subject_id,v_source_record_id,'email',to_jsonb(v_email),1.0,
        'Authenticated account accepted an invitation addressed to this email.',jsonb_build_object('credentialVerified',true))
      returning id into v_email_claim_id;
      insert into atlas.identity_reconciliation_adjudications(
        organization_id,decision_kind,source_record_id,subject_id,claim_id,evidence_snapshot,basis,
        adjudicated_by_user_id,adjudicated_by_label
      ) values(v_invite.organization_id,'accept_claim',v_source_record_id,v_subject_id,v_email_claim_id,
        jsonb_build_object('invitationId',v_invite.id,'authenticatedUserId',v_uid,'email',v_email),
        'The authenticated credential carrier accepted the invitation addressed to the same email.',
        v_uid,'organization_access_invitation_v1');
    end if;

    if not exists(select 1 from atlas.identity_claims c where c.source_record_id=v_source_record_id
                  and c.subject_id=v_subject_id and c.claim_kind='display_name') then
      insert into atlas.identity_claims(organization_id,subject_id,source_record_id,claim_kind,claim_value,confidence,basis)
      values(v_invite.organization_id,v_subject_id,v_source_record_id,'display_name',to_jsonb(v_invite.display_name),0.9,
        'Display name supplied by the authorized organization inviter.');
    end if;

    if not exists(select 1 from atlas.identity_source_subject_assertions a where a.source_record_id=v_source_record_id
                  and a.subject_id=v_subject_id and a.assertion_kind='supports') then
      ...
      ) values(v_invite.organization_id,v_source_record_id,v_subject_id,'supports',1.0,
        'Authenticated invitation acceptance binds this access record to the institutional subject.',
        'access_invite:'||v_invite.id::text,v_uid);
    end if;

    insert into atlas.identity_subject_projections(
      subject_id,organization_id,subject_kind,display_name,contact_points,unresolved_identity,confidence,projection_basis
    ...
    on conflict(subject_id) do update set
      subject_kind=case when atlas.identity_subject_projections.subject_kind='unknown' then 'person'
                        else atlas.identity_subject_projections.subject_kind end,
      display_name=coalesce(atlas.identity_subject_projections.display_name,excluded.display_name),
      contact_points=case when atlas.identity_subject_projections.contact_points='[]'::jsonb then excluded.contact_points
                          else atlas.identity_subject_projections.contact_points end,
    ...
    if v_membership_id is null then
      ...
      on conflict(organization_id,user_id) do update set active=true,
        identity_subject_id=coalesce(atlas.organization_memberships.identity_subject_id,excluded.identity_subject_id),updated_at=now()
      returning id into v_membership_id;
    else
      update atlas.organization_memberships set identity_subject_id=coalesce(identity_subject_id,v_subject_id),updated_at=now()
    ...
    if v_seat_id is null then
      ...
      ) values(v_invite.organization_id,v_membership_id,v_subject_id,'employee','active','active',700,
        jsonb_build_object('sourceInvitationId',v_invite.id)) returning id into v_seat_id;
    else
      update atlas.organization_employee_seats set identity_subject_id=v_subject_id,status='active',
        billing_state=case when billing_state='ended' then 'active' else billing_state end,updated_at=now()
      where id=v_seat_id;
    end if;
    ...
    ) values(v_invite.organization_id,v_membership_id,v_seat_id,v_subject_id,'auth_user',v_uid,'active',
      v_invite.organization_id,jsonb_build_object('sourceInvitationId',v_invite.id,'verifiedEmail',v_email))
    returning id into v_credential_id;
  end if;

  update atlas.organization_access_invitations set status='accepted',accepted_by_user_id=v_uid,
    accepted_at=now(),updated_at=now() where id=v_invite.id;

  return jsonb_build_object('ok',true,'replayed',false,'contractVersion','organization_access_invitation_v1',
    'invitationId',v_invite.id,'organizationId',v_invite.organization_id,'identitySubjectId',v_subject_id,
    'organizationMembershipId',v_membership_id,'employeeSeatId',v_seat_id,'credentialId',v_credential_id,
...
('atlas.prepare_organization_employee_invitation_self_api_v1(uuid,text,text,uuid)','owner_admin_endpoint','verified','active',true,true,true,1,1,
...
('atlas.pending_organization_employee_invitation_self_api_v1(uuid)','app_endpoint','verified','active',true,true,true,1,1,
 jsonb_build_object('source','atlas_organization_access_invitation_v1','purpose','Read an invitation addressed to the signed-in verified email.','truthBoundary','Read only and email-bound.','classificationRuleVersion',3),false),
...
on conflict(signature) do update set classification=excluded.classification,confidence=excluded.confidence,
  ...
  caller_count=excluded.caller_count,policy_reference_count=excluded.policy_reference_count,evidence=excluded.evidence,

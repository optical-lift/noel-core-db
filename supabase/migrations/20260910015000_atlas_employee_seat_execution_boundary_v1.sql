...
-- Gate 2: an ordinary employee membership is not enough to receive Atlas work.
-- The employee must still have the separately established active employee seat
-- and an active credential bound to that same institutional identity. Owners
-- and consultants remain separate institutional access classes and are not
-- converted into employee seats by this rule.
...
begin
  ...
  for v_plan in
    ...
    where p.assignee_membership_id=v_membership_id
  ...
  loop
    ...
    if not coalesce((v_delivery->>'allowed')::boolean,false) then
      ...
      set plan_state='needs_replan',
          plan_reason='Employee Atlas access changed; manager re-planning is required.'
      where id=v_plan.id;
      ...
      insert into atlas.work_execution_plan_events(
        ...
        'Employee Atlas access changed; manager re-planning is required.',

-- Atlas Person Goal Claim Count v1
--
-- Widens the existing person Goal Claim/Evidence resolver by one reducer kind.
-- claim_threshold continues to compare a numeric value at an explicit path and
-- optional exact unit contract. claim_count instead counts the already-governed
-- current Claim set selected by the requirement and applies canonical Atlas
-- counted-set progress semantics: target met = satisfied, nonzero progress =
-- partial, zero progress = unmet.
--
-- This does not widen requirement authorization, Evidence authority, subject or
-- Claim selection, unit conversion, Rhythm occurrence capture, task creation,
-- consequence creation, carrier selection, or Clock authority.
...
begin
  ...
  loop
    ...
    elsif v_requirement_kind not in ('claim_threshold','claim_count') then
    ...
    if v_reason is null then
      ...
      if v_requirement_kind='claim_count' then
        if jsonb_typeof(v_criterion)<>'object'
           ...
           or v_operator<>'>=' then
          v_reason := 'invalid_claim_count_criterion';
        else
    ...
    if v_reason is null then
      ...
      loop
        ...
        -- claim_count reduces the already-filtered current Claim set itself. It
        -- deliberately does not inspect Claim values, paths, or units and does
        -- not select one Claim as aggregate provenance.
        if v_requirement_kind='claim_count' then
          continue;
        end if;
      ...
      if v_requirement_kind='claim_count' then
        v_state := case
          when v_candidate_count>=v_threshold then 'satisfied'
          when v_candidate_count>0 then 'partial'
          else 'unmet'
        end;
        v_reason := case v_state
          when 'satisfied' then 'current_claim_count_meets_target'
          when 'partial' then 'current_claim_count_partially_meets_target'
          else 'no_current_claim_matches_explicit_evidence_selector'
        end;
      elsif v_state<>'satisfied' then
    ...
    v_results := v_results || jsonb_build_array(jsonb_strip_nulls(jsonb_build_object(
      ...
      'state',v_state,
      'detail',case when v_requirement_kind='claim_count' then
        jsonb_strip_nulls(jsonb_build_object(
          'reason',v_reason,
          'observedCount',v_candidate_count,
          'targetCount',case when v_reason not in (
            'missing_requirement_key','unsupported_requirement_kind','missing_current_requirement_authorization',
            'invalid_requirement_authorization_reference','requirement_authorization_is_not_current',
            'invalid_evidence_selector','invalid_claim_count_criterion'
          ) then v_threshold else null end,
          'operator',case when v_reason not in (
            'missing_requirement_key','unsupported_requirement_kind','missing_current_requirement_authorization',
            'invalid_requirement_authorization_reference','requirement_authorization_is_not_current',
            'invalid_evidence_selector','invalid_claim_count_criterion'
        ...
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'operator',case when v_reason not in (
            'missing_requirement_key','unsupported_requirement_kind','missing_current_requirement_authorization',
            'invalid_requirement_authorization_reference','requirement_authorization_is_not_current',
            'invalid_evidence_selector','invalid_claim_threshold_criterion','invalid_claim_threshold_unit_contract'
          ) then v_operator else null end,
          'threshold',case when v_reason not in (
            'missing_requirement_key','unsupported_requirement_kind','missing_current_requirement_authorization',
            'invalid_requirement_authorization_reference','requirement_authorization_is_not_current',
            'invalid_evidence_selector','invalid_claim_threshold_criterion','invalid_claim_threshold_unit_contract'
          ) then v_threshold else null end,
      ...
      'source',case when v_requirement_kind='claim_count' then
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'matchingClaimCount',case when v_reason not in (
            'missing_requirement_key','unsupported_requirement_kind','missing_current_requirement_authorization',
            'invalid_requirement_authorization_reference','requirement_authorization_is_not_current',
            'invalid_evidence_selector','invalid_claim_count_criterion'
          ) then v_candidate_count else null end
        ))
      else
        jsonb_strip_nulls(jsonb_build_object(
          'resolver','atlas.resolve_person_goal_requirement_results_v1',
...
comment on function atlas.resolve_person_goal_requirement_results_v1(uuid,jsonb) is
  'Internal evidence resolver for person Goal claim_threshold and claim_count requirements. Requires the exact current person-accepted requirement authorization and explicit Evidence selector. claim_threshold compares current Claim values under an explicit path/unit criterion; claim_count counts the already-filtered current Claim set and emits aggregate provenance.';
...
comment on function atlas.evaluate_person_goal_from_claim_evidence_api_v1(uuid,text) is
  'Signed-in person Goal evidence-backed evaluator. Resolves explicit claim_threshold or claim_count requirements from current governed Claim/Evidence truth, records the resulting Goal evaluation event, and grants no task, carrier, consequence, unit-conversion, or Clock authority.';

update atlas.authenticated_rpc_registry
set evidence = coalesce(evidence,'{}'::jsonb) || jsonb_build_object(
      'purpose','Evaluate the signed-in person active Goal from current governed Claim/Evidence truth using only explicit person-authorized requirement selectors and criteria, including numeric claim_threshold and aggregate claim_count reducers.',
      'authorizationBoundary','SECURITY DEFINER fixes custody to auth.uid(); each requirement must retain its exact current person-accepted goal_requirement authorization Claim. Evidence selection remains requirement-defined. claim_threshold may inspect only its explicit path/unit criterion; claim_count counts only the already-filtered current Claim set and selects no witness Claim as aggregate provenance. Missing or malformed evidence does not widen authority. No unit conversion, inference of customary requirements, task, carrier, consequence, or Clock authority is granted.'
    ),
    reviewed_at = now()
where signature='atlas.evaluate_person_goal_from_claim_evidence_api_v1(uuid, text)';

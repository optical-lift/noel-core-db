...
-- Presentation must not invent an Organization Ledger window. The source/read
-- membrane owns the rolling recency policy and returns the same governed Ledger
-- payload with that policy made explicit.
...
begin
  ...
  v_payload := atlas.organization_ledger_owner_window_api_v1(
    p_organization_id,
    v_start_at,
    v_end_at,
    0,
...
-- Adopt the remaining default Home pages as durable spreads. These rows describe
-- notebook identity/composition only. The household kernels remain the truth owners.
do $body$
declare
  r record;
  v_spread_id uuid;
...
-- Organization Ledger pages become durable composed pages. The recent-window
-- contract is source-owned; no browser dates are stored in the binding.
do $body$
declare
  r record;
  v_spread_id uuid;
...
-- Existing institutional correspondence books become ordinary durable notebook
-- pages. Endpoint membership remains the authority for what can be read.
do $body$
declare
  r record;
  v_spread_id uuid;
...
-- Index is navigation only: Today/Index are notebook infrastructure; every
-- data-bearing page now comes from the durable spread registry.
...
begin
  ...
  with descriptors as (
    select 0 as section_order,0 as item_order,
      jsonb_build_object(
        'addressKind','today','spreadKey','today','templateKey','today','title','Today','section','Notebook',
        'scope',jsonb_build_object('kind','person','id',v_user_id),
        'subject',jsonb_build_object('kind','day','id',current_date::text)
      ) as item
    union all
    select 0,1,jsonb_build_object(
      'addressKind','index','spreadKey','index','templateKey','index','title','Index','section','Notebook',
      'scope',jsonb_build_object('kind','person','id',v_user_id),

-- Refine lifecycle audit evidence: archived tasks are not live continuity, and
-- destination truth may be carried by linked transplant task objects or production bed assignments.
  ...
  select cc.id as crop_cycle_id,
    ...
    count(pwo.id) filter (
      where pwo.state in ('planned','eligible','releasing','released')
        and coalesce(cc.metadata ->> 'next_action','') <> ''
        and lower(coalesce(pwo.task_payload ->> 'action_key','')) = lower(cc.metadata ->> 'next_action')
...
select *,
  ...
  case
    ...
    when 'next_operation_missing' = any(gap_types) or 'declared_next_action_unwired' = any(gap_types) or 'sow_occurrence_missing' = any(gap_types) then 'continuity_broken'

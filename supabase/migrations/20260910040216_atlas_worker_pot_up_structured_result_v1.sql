...
begin
  ...
  select * into v_org_member from atlas.organization_memberships om where om.id=v_allocation.assignee_membership_id and om.organization_id=v_work.organization_id and om.active;
  select * into v_farm_member from atlas.farm_memberships fm where fm.id=new.assigned_membership_id and fm.farm_id=new.farm_id and fm.active;
  if v_org_member.id is null or v_farm_member.id is null or v_org_member.user_id is distinct from v_farm_member.user_id or auth.uid() is null or auth.uid() is distinct from v_org_member.user_id then raise exception 'Production pot-up result actor must be the currently responsible signed-in worker.' using errcode='42501'; end if;
...
create trigger aa0_accept_company_work_pot_up_domain_result_v1 after update of status on atlas.tasks for each row when (new.status='done' and old.status is distinct from new.status) execute function atlas.accept_company_work_pot_up_domain_result_v1();

create or replace function atlas.worker_record_production_pot_up_self_api_v1(p_delivery_membership_id uuid,p_projection_id uuid,p_outputs jsonb,p_care_date date default null,p_note text default null,p_idempotency_key text default null)
returns jsonb language plpgsql security definer set search_path=pg_catalog,atlas,auth as $function$
declare
  v_uid uuid:=auth.uid(); v_farm_id uuid; v_context jsonb; v_projection atlas.worker_week_projection%rowtype; v_work_ids uuid[]; v_work atlas.work_items%rowtype; v_allocation atlas.work_allocations%rowtype; v_plan atlas.work_execution_plans%rowtype; v_adapter atlas.work_execution_adapters%rowtype; v_task atlas.tasks%rowtype; v_group jsonb; v_tray jsonb; v_cycle_id uuid; v_tray_number integer; v_living numeric; v_group_living numeric; v_seen integer[]; v_aggregate jsonb:='[]'::jsonb; v_normalized jsonb:='[]'::jsonb; v_domain jsonb; v_result atlas.work_execution_results%rowtype; v_acceptance atlas.work_result_acceptances%rowtype; v_key text:=coalesce(nullif(btrim(coalesce(p_idempotency_key,'')),''),'employee-pot-up:'||p_projection_id::text||':'||clock_timestamp()::text);
begin
  ...
  select fm.farm_id into v_farm_id from atlas.farm_memberships fm where fm.id=p_delivery_membership_id; if v_farm_id is null then raise exception 'Worker delivery membership was not found.' using errcode='P0002'; end if;
  v_context:=atlas.organization_employee_worker_context_self_v1(v_farm_id,p_delivery_membership_id); if not coalesce((v_context->>'ok')::boolean,false) then raise exception 'Employee Worker Day authority required: %',coalesce(v_context->>'reason','unknown') using errcode='42501'; end if;
  select * into v_projection from atlas.worker_week_projection p where p.id=p_projection_id and p.membership_id=p_delivery_membership_id and p.organization_id=(v_context->>'organizationId')::uuid and p.organization_membership_id=(v_context->>'organizationMembershipId')::uuid; if v_projection.id is null then raise exception 'Worker Day projection does not belong to this employee relationship.' using errcode='42501'; end if;
  select array_agg(s.work_item_id order by s.work_item_id) into v_work_ids from atlas.worker_week_projection_sources s where s.projection_id=v_projection.id and s.source_role='required'; if coalesce(array_length(v_work_ids,1),0)<>1 then raise exception 'Structured pot-up requires exactly one required Company Work identity.' using errcode='23514'; end if;
  select * into v_work from atlas.work_items w where w.id=v_work_ids[1] and w.organization_id=(v_context->>'organizationId')::uuid and w.organization_unit_id=(v_context->>'organizationUnitId')::uuid and w.work_state='open' and w.result_contract_key='production_pot_up_v1'; if v_work.id is null then raise exception 'This Worker Day item is not open governed production pot-up work.' using errcode='23514'; end if;
  select * into v_allocation from atlas.work_allocations a where a.organization_id=v_work.organization_id and a.work_item_id=v_work.id and a.state='active' and a.allocation_role='responsible' and a.assignee_membership_id=(v_context->>'organizationMembershipId')::uuid limit 1; if v_allocation.id is null then raise exception 'Current delegated Responsibility is required.' using errcode='23514'; end if;
  select * into v_plan from atlas.work_execution_plans p where p.organization_id=v_work.organization_id and p.work_item_id=v_work.id and p.plan_state='active' and p.responsible_allocation_id=v_allocation.id and p.assignee_membership_id=(v_context->>'organizationMembershipId')::uuid and p.exposure_service_date=(v_context->>'serviceDate')::date limit 1; if v_plan.id is null then raise exception 'Pot-up may only be returned from the currently exposed governed Worker Day.' using errcode='23514'; end if;
  select * into v_adapter from atlas.work_execution_adapters a where a.organization_id=v_work.organization_id and a.work_item_id=v_work.id and a.state='active' and a.task_id is not null order by a.created_at desc limit 1; if v_adapter.id is null then raise exception 'Pot-up execution carrier is not materialized.' using errcode='23514'; end if;
  select * into v_task from atlas.tasks t where t.id=v_adapter.task_id and t.farm_id=v_farm_id and t.assigned_membership_id=p_delivery_membership_id and t.status in ('open','blocked'); if v_task.id is null then raise exception 'Pot-up execution task does not belong to the routed employee.' using errcode='42501'; end if;
  ...
  for v_group in select value from jsonb_array_elements(p_outputs) loop
    ...
    for v_tray in select value from jsonb_array_elements(v_group->'physicalTrays') loop
  ...
  update atlas.production_tray_batches b set metadata=coalesce(b.metadata,'{}'::jsonb)||jsonb_build_object('physical_trays',g.value->'physicalTrays','physical_trays_recorded_by',v_uid),updated_at=now() from jsonb_array_elements(v_normalized) g(value) where b.source_task_id=v_task.id and b.crop_cycle_id=(g.value->>'cropCycleId')::uuid;
  select * into v_result from atlas.work_execution_results r where r.organization_id=v_work.organization_id and r.work_item_id=v_work.id and r.task_id=v_task.id and r.result_contract_key='production_pot_up_v1' order by r.reported_at desc limit 1;
  ...
  if v_result.id is null or v_acceptance.decision<>'accepted' or v_work.work_state<>'completed' then raise exception 'Pot-up domain result did not close Company Work atomically.' using errcode='55000'; end if;
  if exists(select 1 from atlas.worker_delivery_pilot_active_attention a where a.delivery_membership_id=p_delivery_membership_id and a.projection_id=p_projection_id) then insert into atlas.worker_delivery_pilot_events(organization_id,organization_membership_id,delivery_membership_id,projection_id,session_id,actor_user_id,event_kind,effective_at,metadata) values(v_work.organization_id,(v_context->>'organizationMembershipId')::uuid,p_delivery_membership_id,p_projection_id,null,v_uid,'stop',clock_timestamp(),jsonb_build_object('source','structured_pot_up_completion','employeeSeatId',v_context->>'employeeSeatId')); delete from atlas.worker_delivery_pilot_active_attention where delivery_membership_id=p_delivery_membership_id and projection_id=p_projection_id; end if;
...
values('atlas.worker_record_production_pot_up_self_api_v1(uuid, uuid, jsonb, date, text, text)','app_endpoint','verified','active',true,true,true,1,1,jsonb_build_object('source','atlas_worker_pot_up_structured_result_v1','purpose','Return physical pot-up outputs from the current governed Worker Day.','truthBoundary','Requires current employee identity, seat, credential, unit appointment, delegated Responsibility, active execution plan, exact Worker Day projection, exact materialized pot-up task, and per-physical-tray living counts. Domain evidence automatically advances Company Work; no manager approval is required.','classificationRuleVersion',3),false)

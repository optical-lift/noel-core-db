...
-- The model extracts notebook intent. It does not decide persistence and it does not
-- establish source-domain authority. Those are separate governed decisions.
...
-- No runtime uses the v1 interpretation writer yet. Retire it before any request rows exist
-- so there is only one authoritative route from interpretation to persistence.
...
as $function$
  select jsonb_build_object(
    ...
    'utteranceModes',jsonb_build_array(
      'point_lookup','broad_retrieval','fact_statement','sustained_intent',
      'explicit_persistence','continuation','unclear'
    ),
    'retrievalNeedKinds',jsonb_build_array(
      'point_lookup','history','pattern','progress','cadence','collection','sequence',
      'comparison','coordination','reference','monitoring'
    ),
    'retrievalRecurrence',jsonb_build_array('once','repeated','ongoing'),
    'basisKinds',jsonb_build_array(
      'literal_span','existing_reality','conversation_context','derived_notebook_identity',
      'calendar_resolution','established_thread'
    ),
    ...
    'truthBoundary',jsonb_build_object(
      ...
      'interpreterDoesNotCreateSourceTruth',true
    )
  );
$function$;
...
begin
  if nullif(btrim(p_literal),'') is null then
    return jsonb_build_object(
      'validationState','rejected','violations',jsonb_build_array(jsonb_build_object('key','literal_request_required')),
      'warnings','[]'::jsonb,'blockingAmbiguityCount',0
    );
  end if;
  if p_interpretation is null or jsonb_typeof(p_interpretation)<>'object' then
    return jsonb_build_object(
      'validationState','rejected','violations',jsonb_build_array(jsonb_build_object('key','interpretation_object_required')),
      'warnings','[]'::jsonb,'blockingAmbiguityCount',0
    );
  end if;
  ...
  if p_interpretation->>'contractVersion'<>'notebook_spread_intent_v2' then
    v_violations := v_violations || jsonb_build_array(jsonb_build_object(
      'key','unsupported_contract_version','value',p_interpretation->>'contractVersion'
    ));
  end if;
  ...
  if jsonb_typeof(coalesce(p_interpretation->'retrievalNeeds','[]'::jsonb))<>'array' then
    ...
    for v_item in select value from jsonb_array_elements(coalesce(p_interpretation->'retrievalNeeds','[]'::jsonb)) loop
      ...
      if jsonb_typeof(v_item)<>'object'
         or v_kind is null or v_kind not in ('point_lookup','history','pattern','progress','cadence','collection','sequence','comparison','coordination','reference','monitoring')
         or v_recurrence is null or v_recurrence not in ('once','repeated','ongoing') then
  ...
  if jsonb_typeof(coalesce(p_interpretation->'sourceCandidates','[]'::jsonb))<>'array' then
    ...
    for v_binding in select value from jsonb_array_elements(coalesce(p_interpretation->'sourceCandidates','[]'::jsonb)) loop
      ...
      if jsonb_typeof(v_binding)<>'object'
         or nullif(btrim(v_binding->>'sourceDomain'),'') is null
         or nullif(btrim(v_binding->>'sourceKind'),'') is null
         or v_relationship is null or v_relationship not in (
      ...
      if v_binding ? 'bindingState' or v_binding ? 'established' then
        v_violations := v_violations || jsonb_build_array(jsonb_build_object(
          'key','interpreter_may_not_establish_source_binding','item',v_binding
        ));
      end if;
    end loop;
  end if;
  ...
  if v_mode='continuation' then
    if jsonb_typeof(p_interpretation->'continuation')<>'object'
       or nullif(btrim(p_interpretation#>>'{continuation,threadKey}'),'') is null then
  ...
  if p_interpretation ? 'resolutionAction' or p_interpretation ? 'creationAuthority' then
    v_violations := v_violations || jsonb_build_array(jsonb_build_object(
      'key','interpreter_may_not_decide_persistence'
    ));
  end if;
...
begin
  ...
  if v_validation->>'validationState'<>'passed' then
    return jsonb_build_object(
      'policyVersion','notebook_spread_creation_policy_v1','resolutionAction','rejected',
      'creationAuthority','none','reason','invalid_interpretation','validation',v_validation
    );
  end if;
  ...
  if v_blocking>0 or v_mode='unclear' then
    v_action := 'needs_clarification';
    v_authority := 'none';
    v_reason := case when v_blocking>0 then 'blocking_ambiguity' else 'unclear_utterance' end;
  elsif v_mode='continuation' then
    v_action := 'resolve_or_create';
    v_authority := 'established_continuation';
    v_reason := 'existing_spread_thread';
  elsif v_mode='explicit_persistence' then
    v_action := 'resolve_or_create';
    v_authority := 'explicit_persistence';
    v_reason := 'human_explicitly_requested_persistence';
  elsif v_mode in ('sustained_intent','broad_retrieval') and v_durable_count>0 then
    v_action := 'resolve_or_create';
    v_authority := 'durable_retrieval_need';
    v_reason := 'repeated_or_ongoing_notebook_retrieval';
  else
    v_action := 'no_spread';
    v_authority := 'none';
    v_reason := case
      when v_mode='point_lookup' then 'narrow_point_lookup'
      when v_mode='fact_statement' then 'fact_without_durable_retrieval_need'
      when v_mode in ('sustained_intent','broad_retrieval') then 'no_durable_retrieval_need_extracted'
      else 'no_persistence_authority'
    end;
  end if;
  ...
  if v_action='resolve_or_create' then
    ...
    if jsonb_array_length(v_missing)>0 then
      v_action := 'needs_clarification';
      v_authority := 'none';
      v_reason := 'durable_page_identity_incomplete';
    end if;
  end if;

  return jsonb_build_object(
    'policyVersion','notebook_spread_creation_policy_v1',
    'resolutionAction',v_action,'creationAuthority',v_authority,'reason',v_reason,
    'durableRetrievalNeedCount',v_durable_count,'missingFields',v_missing,'validation',v_validation,
    'truthBoundary',jsonb_build_object(
      'policyNotModelDecidesPersistence',true,
      'pointLookupDoesNotCreateSpread',true,
      'durableRetrievalNeedCanCreateSpread',true,
      'sourceAuthorityResolvedSeparately',true
    )
  );
end;
$function$;
...
create or replace function atlas.interpret_notebook_spread_intent_serv_v2(
  p_request_id uuid,
...
begin
  ...
  if v_request.request_state<>'captured' then
    ...
    return jsonb_build_object(
      'ok',true,'changed',false,'contractVersion','notebook_spread_intent_resolution_v2',
      'requestId',v_request.id,'requestState',v_request.request_state,
      'spreadInstanceId',v_request.spread_instance_id,'policyDecision',v_request.policy_decision,
      'validation',v_request.validation
    );
  end if;
  ...
  if v_action in ('rejected','no_spread','needs_clarification') then
    ...
    set request_state=case v_action when 'rejected' then 'rejected' when 'no_spread' then 'no_spread' else 'needs_clarification' end,
        ...
        validation=v_policy->'validation',
        policy_decision=v_policy,
    ...
    return jsonb_build_object(
      'ok',true,'changed',true,'contractVersion','notebook_spread_intent_resolution_v2',
      'requestId',v_request.id,'requestState',v_request.request_state,'spreadInstanceId',null,
      'policyDecision',v_policy,'validation',v_request.validation,
      'truthBoundary',jsonb_build_object('noSourceBindingCreatedByInterpreter',true,'sourceTruthUnchanged',true)
    );
  end if;
  ...
  v_creation_mode := case v_policy->>'creationAuthority'
  ...
  v_thread_key := case
    when v_creation_mode='continuation' then p_interpretation#>>'{continuation,threadKey}'
    else 'thread:'||gen_random_uuid()::text
  end;
  ...
  v_result := atlas.resolve_notebook_spread_instance_v2(
    ...
    v_subject->>'domain',v_subject->>'kind',v_subject->>'id',
    v_purpose->>'key',v_horizon->>'key',v_thread_key,
    ...
    jsonb_build_object(
      ...
      'creationAuthority',v_policy->>'creationAuthority',
    ...
    jsonb_build_object(
      ...
      'unresolvedSourceCandidates',coalesce(p_interpretation->'sourceCandidates','[]'::jsonb)
    )
  );
  v_spread_id := (v_result->>'spreadInstanceId')::uuid;

  update atlas.notebook_spread_intent_requests
  set request_state='resolved',interpretation=p_interpretation,validation=v_policy->'validation',
      policy_decision=v_policy,interpreter_kind=p_interpreter_kind,interpreter_ref=btrim(p_interpreter_ref),
  ...
  return jsonb_build_object(
    ...
    'spread',v_result,'policyDecision',v_policy,'validation',v_request.validation,
    'sourceCandidates',coalesce(p_interpretation->'sourceCandidates','[]'::jsonb),
    'truthBoundary',jsonb_build_object(
      'policyNotInterpreterGrantedCreationAuthority',true,
      'interpreterCreatedNoSourceBindings',true,
      'sourceCandidatesRequireSeparateDomainResolution',true,
...
create or replace function public.interpret_notebook_spread_intent_service_v2(
  p_request_id uuid,
  p_interpretation jsonb,
  p_interpreter_kind text,
  p_interpreter_ref text
...
as $function$
  select jsonb_build_object(
    ...
    'interpretation',r.interpretation,'validation',r.validation,'policyDecision',r.policy_decision,

...
type DetailEvent = { communicationEventId?: string; presentationAvailable?: boolean };
type DetailPacket = {
  identityRoot?: string;
...
async function presentationForEvent(communicationConversationId: string, eventId: string, allowRemoteImages: boolean): Promise<Presentation> {
  ...
  if (!html) {
    return { ok: true, communicationConversationId, eventId, kind: "text", text: parsed.text?.trim() ?? "", remoteImagesBlocked: false, inlineImageCount: 0 };
  }
...
Deno.serve(async (request: Request) => {
  ...
  try {
    const body = await request.json() as { communicationConversationId?: string; eventId?: string; allowRemoteImages?: boolean };
  ...
  if (!UUID.test(communicationConversationId) || !UUID.test(eventId)) return json({ error: "Conversation or Event reference is invalid." }, 400);

  try {
    const detail = await rpc<DetailPacket>(
      "organization_correspondence_conversation_self_api_v4",
    ...
    if (
      ...
      return json({ error: "That Communication Event is not available to this Atlas account." }, 403);
    }
    const authorizedEvent = (detail.events ?? []).find((event) => event.communicationEventId === eventId);
    if (!authorizedEvent) return json({ error: "That Communication Event is not available to this Atlas account." }, 403);
    if (authorizedEvent.presentationAvailable !== true) {
      return json({ error: "Sender-designed presentation is not available for this Communication Event." }, 422);
    }
  } catch (error) {
    console.error("Communication Event presentation authorization failed", error instanceof Error ? error.message : String(error));
    return json({ error: "That Communication Event is not available to this Atlas account." }, 403);
  }

  try {
    return json(await presentationForEvent(communicationConversationId, eventId, allowRemoteImages));
  } catch (error) {
    console.error("Communication Event presentation projection failed", { eventId, error: error instanceof Error ? error.message : String(error) });

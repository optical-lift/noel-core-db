...
function isPageNumber(text: string) {
  ...
  return /^\d{1,4}$/.test(t) || /^[ivxlcdm]{1,8}$/i.test(t);
}
...
function startsLower(text: string) {
  const m = cleanText(text).match(/[A-Za-z]/);
  return !!m && m[0] === m[0].toLowerCase();
}

function startsQuote(text: string) {
  return /^[“‘"']/.test(cleanText(text));
}

function makeLocator(surface: Surface): Json {
  const s = surface.source_locator ?? {};
  const out: Json = { source_asset_id: surface.id, source_asset_key: surface.asset_key };
  for (const key of [
    "sequence_index", "printed_page", "source_pdf_page", "pdf_page", "loc_image",
    "iiif_canvas_uri", "iiif_image_service_uri", "image_uri", "pdf_uri", "item_uri",
  ]) {
...
function lineUnits(surface: Surface, surfaceIndex: number): Unit[] {
  ...
  if (regions.length) {
    return regions
      .sort((a, b) => (a.ordinal ?? 1e9) - (b.ordinal ?? 1e9) || (a.y ?? 1e9) - (b.y ?? 1e9) || (a.x ?? 1e9) - (b.x ?? 1e9))
      .map((o, i) => ({
        text: cleanText(o.text_candidate ?? ""), observationIds: [o.id], locator: makeLocator(surface), assetKey: surface.asset_key,
        surfaceIndex, x: o.x, y: o.y, width: o.width, height: o.height, confidence: o.confidence,
        explicitBoundaryBefore: i > 0, sourceMode: "region" as const,
      }));
  }
  ...
  if (lines.length) {
    return lines
      ...
      .map((o) => ({
        text: cleanText(o.text_candidate ?? ""), observationIds: [o.id], locator: makeLocator(surface), assetKey: surface.asset_key,
        surfaceIndex, x: o.x, y: o.y, width: o.width, height: o.height, confidence: o.confidence,
        explicitBoundaryBefore: false, sourceMode: "line" as const,
      }));
  }
  ...
  const page = [...pages].sort((a, b) => {
    ...
    const bn = (b.text_candidate ?? "").split(/\r?\n/).length;
  ...
  for (const part of raw) {
    ...
    out.push({
      text: t, observationIds: [page.id], locator: makeLocator(surface), assetKey: surface.asset_key,
      surfaceIndex, x: null, y: null, width: null, height: null, confidence: page.confidence,
      explicitBoundaryBefore: blankBefore, sourceMode: "page_text",
    });
    blankBefore = false;
  }
  if (out.length && isPageNumber(out[out.length - 1].text)) out.pop();
  return out;
}
...
function addUnit(draft: ParagraphDraft, unit: Unit) {
  ...
  draft.assetKeys.add(unit.assetKey);
  draft.sourceModes.add(unit.sourceMode);
...
function newDraft(kind: "paragraph" | "heading"): ParagraphDraft {
  return {
    text: "", observationIds: new Set(), locators: [], assetKeys: new Set(), reviewReasons: new Set(),
    structuralScores: [], sourceConfidences: [], sourceModes: new Set(), kind,
  };
}
...
function buildDrafts(packet: SourcePacket): ParagraphDraft[] {
  ...
  for (let si = 0; si < surfaceUnits.length; si++) {
    const units = surfaceUnits[si];
    if (!units.length) continue;

    const xVals = units.map((u) => u.x).filter((v): v is number => typeof v === "number");
    const baselineX = median(xVals);
    const pageWidth = Number(packet.surfaces[si].source_locator?.pixel_width ?? 0) || null;
    ...
    for (let ui = 0; ui < units.length; ui++) {
      const unit = units[ui];
      if (isPageNumber(unit.text)) continue;

      if (isHeading(unit.text)) {
        if (current?.text) drafts.push(current);
        current = newDraft("heading");
        addUnit(current, unit);
        current.reviewReasons.add("heading_or_chapter_boundary_requires_parentage_review");
        current.structuralScores.push(0.99);
        drafts.push(current);
        current = null;
        previousUnit = unit;
        continue;
      ...
      if (!current || !previousUnit) {
        ...
        } else if (!terminal(previousUnit.text)) {
          breakBefore = false; score = 0.82;
          reason = "cross_page_continuation_inferred_without_terminal_punctuation";
        } else {
        ...
        if (typeof previousUnit.y === "number" && typeof previousUnit.height === "number" && typeof unit.y === "number" && typeof medianGap === "number") {
          const gap = unit.y - (previousUnit.y + previousUnit.height);
          largeGap = gap > Math.max(4, medianGap * 1.6);
        }
        if (indented || largeGap) {
          breakBefore = true; score = indented && largeGap ? 0.99 : 0.94;
        } else if (startsQuote(unit.text) && terminal(previousUnit.text)) {
          breakBefore = true; score = 0.72;
          reason = "dialogue_paragraph_boundary_inferred_without_layout_signal";
        } else {
          breakBefore = false; score = unit.sourceMode === "page_text" ? 0.58 : 0.82;
          if (unit.sourceMode === "page_text") reason = "physical_line_joined_without_layout_coordinates";
        }
      }
      ...
      addUnit(current, unit);
      current.structuralScores.push(score);
      if (reason) current.reviewReasons.add(reason);
      previousUnit = unit;
    }
  }
  if (current?.text) drafts.push(current);
...
Deno.serve(async (req: Request) => {
  ...
  const claims = decodeJwtPayload(auth.replace(/^Bearer\s+/i, ""));
  ...
  try {
    ...
    for (const draft of drafts) {
      ...
      const reasons = new Set(draft.reviewReasons);

      const proposedReadingState = desiredState;
      ...
      let proposedBlockKey: string;
      let proposedBlockType: string;
      let proposedSemanticRole: string;
      if (draft.kind === "heading") {
        proposedBlockKey = `${prefix}:heading:${String(headingNo++).padStart(3, "0")}`;
        proposedBlockType = "heading";
        proposedSemanticRole = /^(chapter)\b/i.test(draft.text) ? "chapter_heading" : "section_heading";
      } else {
        proposedBlockKey = `${prefix}:${String(paragraphNo++).padStart(3, "0")}`;
        proposedBlockType = "paragraph";
        proposedSemanticRole = "body_paragraph";
      }
      ...
      proposals.push({
        proposal_key: `${reconstructionKey}:${proposedBlockKey}`,
        target_parent_block_id: packet.target_parent_block.id,
        proposed_block_key: proposedBlockKey,
        proposed_ordinal: blockOrdinal,
        proposed_block_type: proposedBlockType,
        proposed_semantic_role: proposedSemanticRole,
        proposed_text_content: draft.text,
        proposed_reading_state: proposedReadingState,
        source_observation_ids: [...draft.observationIds],
        confidence,
        disposition,
    ...
    const stats = {
      ...
      auto_admit: proposals.filter((p) => p.disposition === "auto_admit").length,
    ...
    const committed = await rpc("wnph_commit_reconstruction_batch_v1", {
      p_source_package_key: sourcePackageKey,
      p_reconstruction_key: reconstructionKey,
